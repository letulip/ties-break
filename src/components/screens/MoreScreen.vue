    ...
    <table v-if="namedSlots.length" style="margin-top: 12px">
      ...
      <tbody>
        <tr v-for="s in namedSlots" :key="s.slot">
          ...
          <td>
            <button :disabled="game.busy" @click="tracked(() => game.load(s.slot))">Load</button>
            <!-- TB-19: routed through the shared ConfirmDialog – this was the screen's one
                 unconfirmed irreversible action. See askDeleteSlot for the reasoning. -->
            <IconButton
              variant="bare"
              icon="close"
              :icon-size="14"
