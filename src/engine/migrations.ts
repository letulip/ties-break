...
export function migrateSave(raw: unknown): WorldState {
  ...
  // ⭐⭐ v64 -> v65: WHO WON EACH AI TOURNAMENT. `fieldSeasonTitles` is the season's titles by rung
  ...
  // by construction.
  //
  // ⚠⚠ AND IT WAS BUILT AS `v === 63` ON ITS OWN BRANCH – THE COLLISION THIS PLACEMENT RESOLVES. The
  // wave branched off round 28's ledger while that branch still read 63, so 63 -> 64 was the only
  // step it could see; `main` had meanwhile taken 64 for the call-up reveal directly above. Two
  // different v64 migrations existed, both correct against their own chain, and a save written by
  // either was unreadable by the other. ⚠ THE CHAIN IS APPEND-ONLY AND THAT IS WHY THIS SITS AFTER
  // THE REVEAL RATHER THAN BESIDE IT: the two are independent – one writes a world key, the other a
  // field on `save.college` – so the ORDER is free but the POSITION is not. A v63 save must reach 65
  // by running both, in the order the versions shipped, and `tests/migrations.test.ts` drives that
  // 63 -> 64 -> 65 walk end to end rather than checking each step alone.
  if (v === 64) {
  ...
  // every career in the corpus, and no evidence anywhere in a save could say otherwise.
  //
  ...
  //
  // ⚠ IDEMPOTENT AND DRAW-FREE: two `??=` on two world keys, gated on `v === 84`, writing two
