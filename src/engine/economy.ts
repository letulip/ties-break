...
export const ECONOMY = {
  ...
  // honest slice of it – one offer, results-gated, cash only, no cost at all – and stops there on
  // the plan's own order: the recovery cost (step 2) is the next slice, fame (step 3+) is paused
  // upstream with the private life.
  //
  ...
  advertising: {
     ...
     *  At Ines' stage it is 8% of her interest alone – noise, exactly as §3 predicts. Step 2's
     *  bench (the recovery cost) is where this number gets a measured counterweight. */
    cashCents: 20_000_00,
    /** Twelve months of her face, from the week the paper is signed. The term is the deal's only
     *  ongoing fact in step 1: while it runs no second advertising letter arrives – one deal at a
     *  time, the plan's §4.1 – and it asks nothing of her, so it survives a college enrolment by
     *  simply running out on its own clock (plan §4c: no penalty, ever). */
    termWeeks: 52,
