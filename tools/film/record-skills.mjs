// Film the skills panel growing, once per talent setup, then the logo card.
//
...
const OUT = process.argv[2] || '/tmp/skillfilm'
...
await page.goto(`${URL}/tools/film/skill-film.html`, { waitUntil: 'networkidle' })
...
for (const m of marks) {
  log.push({ what: 'tier', index: m.index, tier: m.tier, t: filmStart + m.start, end: filmStart + m.end })
}
...
await ctx.close()
await browser.close()
console.log('tiers:', marks.length, '| per tier:', plan.perTier + 's', '| film ends', stamp().toFixed(1) + 's')
for (const m of marks) console.log('  ', m.index, m.tier)
