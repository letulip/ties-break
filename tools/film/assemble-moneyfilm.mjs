...
const DIR = process.argv[2] || '/tmp/moneyfilm'
const OUT = process.argv[3] || '/Users/letulip/Downloads/she_won_lost_money.mp4'
...
const r = data.report
const $ = (c) => `$${(Math.abs(c) / 100).toFixed(2)}`
console.log(`\n${OUT}  ${(fs.statSync(OUT).size / 1024 / 1024).toFixed(1)} MB  ${total.toFixed(1)}s  828x1792`)
console.log(`seed ${r.seed} | ${r.event.label} week ${r.event.week} (deadline ${r.event.deadlineWeek})`)
console.log(`before ${$(r.fundsBefore)} -> after ${$(r.fundsAfter)}  NET -${$(r.net)}`)
console.log(`entry ${$(r.entryFeeCents)} | travel ${$(r.travelCents)} | prize ${$(r.prizeForChampionCents)}`)
console.log(`income ${$(r.incomeCents)} | spend ${$(r.spendCents)} | points ${r.points} | matches ${r.matches}`)
