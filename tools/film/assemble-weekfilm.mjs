...
const DIR = process.argv[2] || '/tmp/weekfilm'
const OUT = process.argv[3] || '/Users/letulip/Downloads/one_week_two_futures.mp4'
...
console.log(`seed ${r.seed} | week ${r.startWeek} | age ${r.ageYears} | start condition ${r.startCondition}`)
console.log(`BUILD   ${r.build.start} -> ${r.build.end}   ${r.build.weeks.map((w) => w.condition).join(' -> ')}`)
console.log(`RECOVER ${r.recover.start} -> ${r.recover.end}   ${r.recover.weeks.map((w) => w.condition).join(' -> ')}`)
