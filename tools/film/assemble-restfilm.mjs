...
const DIR = process.argv[2] || '/tmp/restfilm'
const OUT = process.argv[3] || '/Users/letulip/Downloads/when_rest_becomes_training.mp4'
...
console.log(`seed ${r.seed} | age ${r.ageYears} | ${r.centralLabel} wk ${r.centralWeek} | score ${r.centralScore}`)
console.log(`  condition ${r.conditionAtEntry} -> ${r.conditionAfterMatch} (drain ${r.drain}) | ${r.pointsFromCentral} pts | clearance ${r.clearanceAfterMatch}`)
console.log(`  resort ${$(r.resortPriceCents)} | +${r.resortGain} programme +${r.matchFreeRecovery} match-free -> ${r.afterResort} -> ${r.afterLight}`)
console.log(`  factor ${r.factorAtEntry.toFixed(3)} -> ${r.factorAfter.toFixed(3)} | return ${r.returned ? r.returned.label + ' @' + r.returned.condition : 'none'}`)
