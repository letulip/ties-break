// Film ONE WEEK, TWO FUTURES. The rig plays its own timeline (with its own 300ms crossfades) and the
...
const OUT = process.argv[2] || '/tmp/weekfilm'
...
step('goto rig')
await page.goto(`${URL}/tools/film/week-film.html`, { waitUntil: 'networkidle' })
...
console.log('BUILD  ', report.build.start, '->', report.build.end, '| RECOVER', report.recover.start, '->', report.recover.end)
