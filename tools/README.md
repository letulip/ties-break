...
178 TypeScript files: **26 live**, **152 archival**.
...
which ones are supported. The live set stays in `tsconfig.app.json`; the whole directory is
typechecked by `npm run check:tools` (`tsconfig.tools.json`) – which `npm run check` and CI have
run on every change since round 34, because "on demand" meant nine errors nobody saw (QA-34).
...
- `r34-brand-foot.ts` · `r34-calendar-tiers.ts` · `r34-domestic-reset.ts` · `r34-reachable-ceiling.ts`
- `r34-savings-income.ts` · `r34-zero-lock.ts` · `reach-sweep.ts` · `real-vs-bench.ts`
