...
const HEADER = [
  ...
  `THE PRACTICE GATE (owner 26.07): the veto now also covers the FRIENDLY – under condition ${ECONOMY.availability.medicalFloor} a practice match cannot be`,
  '  BOOKED (same predicate as the entry gate) and one already booked is called off if its week arrives under the',
  '  floor (court rental refunded, and the freed week pays the full free-week recovery). It closes the friendly',
  '  treadmill the match-base raise opened: the practice columns below are the read – a policy that books every',
  '  plannable week now loses the weeks it spends under the floor, while a load-managed one never notices.',
  '',
