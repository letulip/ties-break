...
function weeklyBurnCents(world: WorldState): number | null {
...
const signedPct = (delta: number, base: number) => (base === 0 ? '–' : `${delta >= 0 ? '+' : ''}${((delta / base) * 100).toFixed(1)}%`)
const num = (x: number | null, d = 1) => (x === null ? '–' : x.toFixed(d))

function corridor(title: string, before: Read[], after: Read[]): void {
  console.log(`\n${title}  (${before.length} careers per arm)\n`)
  console.log(`  ${padR('', 40)}${padL('BEFORE 5pp/50@26', 20)}${padL('AFTER 10pp/60@23', 20)}${padL('delta', 20)}`)
  ...
  console.log('')
  const runway = (rows: Read[]) => median(rows.map((c) => c.runwayWeeks).filter((x): x is number => x !== null))
  console.log(
