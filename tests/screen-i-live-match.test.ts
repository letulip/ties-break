...
describe('one header slot per match screen, and it says where it takes you', () => {
  ...
  it('the surface pill stands down only while a match is on screen', () => {
    ...
    const markup = markupOf(flow)
    const sub = markup.slice(markup.indexOf('class="tf-sub"'), markup.indexOf('</header>'))
    expect(sub).toMatch(/v-if="!replayOpen" class="pill"/)
    expect(sub).toContain('SURFACE_EMOJI[pending.surface]')
    // and the friendly's header keeps its own pill unconditionally – its surface is stated once.
    expect(markupOf(read('../src/components/PracticeFlow.vue'))).toContain('SURFACE_EMOJI[match.surface]')
  })
...
describe('live and replay open the same way – the popup, which is the one he likes', () => {
  ...
  it('exactly one cross is left in the match flow, and it is the replay\'s', () => {
    ...
    // ⚠ ADOPTION POINT for the icon system's cross SVG – this is the last bare glyph in the flow.
    expect(markupOf(replay)).toContain('Close ✕')
    expect(markupOf(practice), 'the friendly no longer offers a close').not.toContain('Close ✕')
  })
})
