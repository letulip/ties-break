...
describe('save migrations', () => {
  ...
  it('⭐⭐ v63 -> v64 -> v65: a v63 save walks BOTH of the day\'s schema moves and arrives correct', () => {
    ...
    // ⚠ RE-AIMED AT v79 (16.09, the chemistry wave C1 + round 42 #48's third sparring key), NOT
    // WEAKENED, for the FOURTEENTH time and with the claim unchanged once more. What moved is the
    // LADDER'S HEAD and nothing else: 79 was taken by that wave's own full move (bump, append-only
    // v78 -> v79 step, golden fixture v79, e2e fixtures regenerated, and the frozen-career peel
    // rung), the walk below is the same walk, and the claim this line makes – that the chain runs
    // PAST the colliding 64 and through 65 – now has one more rung to cross before it arrives.
    // ⚠ v79 is the first step in this chain whose reader is reachable IN A FROZEN CAREER: every
    // frozen career hires a coach, so `coachPairs` accrues in all of them, where v78's
