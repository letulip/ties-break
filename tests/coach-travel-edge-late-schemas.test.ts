// THE FROZEN CAREERS, THE MIDDLE RUNGS – v82 down to v77.
//
// ⚠⚠ WHY THIS FILE EXISTS – THE FIFTH CUT OF THE SAME FAMILY FOR THE SAME REASON. birpc's RPC
// window is a hard 60 s and it is not raisable; a file whose tests cross it fails the job with
// every test GREEN and one unhandled `Timeout calling "onTaskUpdate"`. On 23.09 the wave-12 PR's
// runner hit it in -recent-schemas: **12 passed (12)**, `Errors 1`, `Duration 66.67s (tests
// 64.45s)`, and scripts/units.mjs's classifier called it – «1 stalled twice (runner, not tests)»
// – exit 1 with every test green. Local solo the same day: 34.34 s on ten cores, and the family's
// own 1.9x multiplier predicts the observed 64 s exactly.
//
// ⚠⚠ AND THE FOURTH CUT PREDICTED THIS ONE AND ASKED FOR IT EARLY. Its header said: «at ~2.5 s a
// rung it crosses again near twelve, six moves from now. Cut it then, not after the red run.»
// Waves 7 through 12 added exactly six rungs (v83..v88), the file reached twelve, and the cut was
// not made – the red run arrived first. A sentence in a header is not an instrument, so the
// instrument now exists: tests/coach-travel-edge-rungs-ratchet.test.ts pins every `-schemas` file
// in this family to at most TEN rungs, and the eleventh goes red locally, one wave before any
// runner can.
//
// WHAT THE SEAM IS. The bottom six rungs of -recent-schemas (v82 down to v77) moved here; the top
// six (v88 down to v83) stayed, and that file is where every future schema move adds its rung.
// Nothing was trimmed on the way across – same walk, same 156 weeks, same constants, same test
// names, and the ORIGINAL describe name deliberately unchanged, exactly as the four earlier cuts
// did (tests/coach-travel-edge-{mid,older,deepest,prior}-schemas.test.ts).
//
// ⚠ Before cutting anything below v61, read round 42's open question about whether the deep rungs
// are the right instrument at all: nothing else in this repo mechanically enforces append-only
// down there, and a pin on the migration bodies would do the same job in milliseconds. That is a
// design decision for the owner, not a cut to make under time pressure.
