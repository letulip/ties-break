...
const template = regionToLast(wizard, '<template>', '</template>')
...
const css = after(wizard, '<style scoped>')
...
/** A named region of the WIZARD, by its two boundary markers.
 *
 *  ⚠ THE THROW USED TO LIVE HERE, ALONE. This file wrote its own marker helper because a raw
 *  `slice(indexOf(a), indexOf(b))` returns -1 and widens instead of failing – and it was right, but
 *  it was one file's private answer to a repository-wide hazard (176 raw slices across 42 test
 *  files, R2-12). The shared `region` in tests/helpers/source.ts is that helper, for everyone; this
 *  is now a one-line binding of it to `wizard` so the call sites below need no edit. */
const wizardRegion = (from: string, to: string): string => region(wizard, from, to)

// ===========================================================================
// 1. WHAT ONBOARDING COLLECTS — the contract with the rest of the game
// ===========================================================================
describe('the wizard still writes a whole PlayerProfile', () => {
  const literal = wizardRegion('const profile = reactive<PlayerProfile>({', 'const countryChosen')
  ...
  it('gates the two steps that can be left empty, and only those two', () => {
    const gate = wizardRegion('const nextDisabled', 'const head =')
  ...
  it('trims the names and falls back rather than starting a career called ""', () => {
    const start = wizardRegion('function start()', '</script>')
  ...
  it('offers the two coach rungs it always did – `self` and the STANDARD private coach', () => {
    const options = wizardRegion('const COACH_OPTIONS', 'const PLAY_STYLES')
...
describe('R: every play style has a pose that ships, and a colour to paint it', () => {
  const styles = wizardRegion('const PLAY_STYLES: {', 'const RADAR_OUTER')
  ...
  // ⚠ RE-AIMED TWICE IN ONE WAVE, AND BOTH HALVES ARE KEPT (collected 24.08). R2-09 made
  // `shared/protocol` a BARREL, so reading protocol.ts alone would slice a file of re-export lines;
  // `engineModuleSource('../shared/protocol')` is the barrel PLUS every `src/shared/protocol/*.ts`
  // part, which makes the pin location-independent and the remaining moves test-free. R2-12 then
  // replaced the raw `indexOf` slice with `lineAt`, which THROWS on an absent marker instead of
  // returning -1 and silently widening – the exact failure this file would otherwise have inherited
  // from the split. One reads the right text, the other refuses to read the wrong text.
  const protocol = engineModuleSource('../shared/protocol')
  const IDS = [...lineAt(protocol, 'export type PlayStyle').matchAll(/'([^']+)'/g)].map((m) => m[1])
