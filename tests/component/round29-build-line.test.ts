...
describe('the build line renders at the foot of Settings, with the values the build baked in', () => {
  ...
  it('prints the build date and this build\'s save schema, so a report carries both numbers', () => {
    ...
    // ⚠ AND AGAIN AT v70 (03.09, round 35 #14 – the published draw becomes a fact), NOT WEAKENED,
    ...
    // ⚠ AND AGAIN AT v76 (13.09, the psychologist's year – the seat's four keys and the two walls
    // keys of who-she-is §2a), NOT WEAKENED, for the ELEVENTH time and for the reason verbatim: 76 was
    // moved by that wave with its full four-part move (bump, append-only migration back-filling six
    ...
    // ⚠ AND AGAIN AT v79 (16.09, the chemistry wave C1 – `coachPairs`, plus round 42 #48's
    // `sparringTravels` riding with it), NOT WEAKENED, for the FOURTEENTH time and for the reason
    // verbatim: 79 was moved by that wave with its full move (bump, append-only migration, golden
    // fixture v79, e2e fixtures regenerated, the frozen-career peel rung, plus the spec's own
    ...
    // ⚠ AND AGAIN AT v82 (17.09, round 42 #51 – `coachDeal`, the coach's fee fixed at hire and his
    // annual ask), NOT WEAKENED, for the SIXTEENTH time and for the reason verbatim: 82 was moved by
    // that item with its full move (bump, append-only migration, golden fixture v82 and its README
    // row, e2e fixtures regenerated, the doc-facts sentence, the frozen-career peel rung and the
