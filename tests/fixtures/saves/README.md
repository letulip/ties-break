...
| v22 | profile `coachSetup` → `coachTier` (the coach ladder: `'parent' \| 'hired'` becomes `'self' \| 'budget' \| 'middle' \| 'high' \| 'elite'`. The back-fill lands the career on the rung closest to **what it was paying** – old and new are both weekly bills, so they compare directly, computed at the plan the save is running and the age she is at. `parent` is the one exception and a deliberate correction: priced literally it would migrate onto a *paid* Middle or High coach, because the old `parent` band was a catch-all base cost with a coach-sized number on it rather than a coach's fee. What that family bought was no coach, so it lands on `self` – which also keeps it development-neutral at 0.82. The `hired` mapping is background-dependent, because the old bill was wealth-corridor scaled and the new one is not) |
