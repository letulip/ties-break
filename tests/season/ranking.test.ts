...
describe('computeRanking — rolling 52-week window', () => {
  it('keeps a week-1 result at week 53 and drops it at week 54', () => {
    ...
    expect(computeRanking(results, 53, 6).find((x) => x.playerId === 'a')?.points).toBe(30)
    expect(computeRanking(results, 54, 6).find((x) => x.playerId === 'a')?.points).toBe(0)
  })

  it('ignores results in the future relative to currentWeek', () => {
    const results = [r('a', 10, 30)]
    expect(computeRanking(results, 5, 6).find((x) => x.playerId === 'a')?.points).toBe(0)
  })
})
...
describe('computeRanking — best 6 of the window', () => {
  it('sums the best six results, ignoring the weakest when there are seven', () => {
    ...
    const row = computeRanking(results, 20, 6).find((x) => x.playerId === 'a')
...
describe('computeRanking — tie-break by recency', () => {
  it('ranks equal-point players by the more recent best result, sharing the rank', () => {
    ...
    const ranking = computeRanking(results, 50, 6)
...
describe('computeRanking — competition ranks', () => {
  it('assigns competition ranks (equal points share a rank; next distinct skips by the tie count)', () => {
    ...
    const ranking = computeRanking(results, 5, 6)
...
describe('computeRanking — totality over a roster', () => {
  it('includes every roster member, zero-point players after pointed ones in stable order', () => {
    ...
    const ranking = computeRanking(results, 6, 6, roster)
  ...
  it('is pure — does not mutate the results array', () => {
    ...
    computeRanking(results, 6, 6, ['a', 'b'])
...
describe('windowedBestSum — the value the standings show', () => {
  it('equals the player points computeRanking assigns (sum shown = standings points)', () => {
    const results = [10, 20, 30, 40, 50, 60, 70].map((p, i) => r('a', 2 + i, p))
    const standingsPoints = computeRanking(results, 20, 6).find((x) => x.playerId === 'a')!.points
    expect(windowedBestSum(results, 20, 'a', 6)).toBe(standingsPoints)
    expect(windowedBestSum(results, 20, 'a', 6)).toBe(20 + 30 + 40 + 50 + 60 + 70) // weakest (10) dropped
  })

  it('respects the 52-week window (drops a result that ages out)', () => {
    const results = [r('a', 1, 30)]
    expect(windowedBestSum(results, 53, 'a', 6)).toBe(30)
    expect(windowedBestSum(results, 54, 'a', 6)).toBe(0)
  })

  it('is zero for a player with no counted results', () => {
    expect(windowedBestSum([r('a', 4, 30)], 6, 'kid', 6)).toBe(0)
  })
})
...
describe("owner's ranking scenarios — effective delta of a new result", () => {
  ...
  it('DISPLACED result: net delta = new − displaced (the pushed-out 6th best)', () => {
    const before = windowedBestSum(six(), 6, 'kid', 6)
    const withNew = [...six(), r('kid', 6, 48)] // beats the weakest counted result (15)
    const after = windowedBestSum(withNew, 6, 'kid', 6)
    expect(after - before).toBe(48 - 15) // +33: only the improvement over the displaced 15 counts
    expect(rankingDeltaSuffix(48, after - before, 6)).toBe(' (ranking total +33)')
  })
  ...
  it('BELOW the 6th best: net delta 0, "does not improve best 6"', () => {
    ...
    expect(rankingDeltaSuffix(10, after - before, 6)).toBe(' (does not improve best 6)')
  })

  it('FRESH add (fewer than 6 counted): full points count, no suffix', () => {
    const before = windowedBestSum([r('kid', 1, 30)], 2, 'kid', 6)
    const withNew = [r('kid', 1, 30), r('kid', 2, 48)]
    const after = windowedBestSum(withNew, 2, 'kid', 6)
    expect(after - before).toBe(48)
    expect(rankingDeltaSuffix(48, after - before, 6)).toBe('') // delta === points → nothing extra
  })
})

describe('rankingDeltaSuffix — pure formatter', () => {
  it('covers displaced / net-zero / full / degenerate cases', () => {
    expect(rankingDeltaSuffix(48, 48, 6)).toBe('') // nothing displaced
    expect(rankingDeltaSuffix(48, 33, 6)).toBe(' (ranking total +33)') // displaced
    expect(rankingDeltaSuffix(10, 0, 6)).toBe(' (does not improve best 6)') // below 6th
    expect(rankingDeltaSuffix(0, 0, 6)).toBe('') // no points awarded (degenerate)
  })
})
