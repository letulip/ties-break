...
// =================================================================================================
// ⭐⭐⭐ RE-STAMPED 19.09.2026 – THE STAFF'S YEAR-END POST, WAVE 7: A BEHAVIOUR CHANGE AND NOT A KEY
// APPEND, WHICH IS WHY IT REACHES THE WHOLE LADDER. **SEVENTY CELLS OF A HUNDRED AND FOUR, AND THE
// PATTERN IS THE PROOF: EVERY COACHED CELL MOVED AND EVERY SELF-COACHED ONE HELD, IN ALL THIRTY-FOUR
// REGISTERS.** SCHEMA UNMOVED AT v83.
// =================================================================================================
//
// WHAT MOVED THEM. `world/staffLetters.ts` – the owner, 18.09, having played a career to its end:
// «письмо от тренера по итогу года мне так и не пришло, да и ни от одного специалиста не пришло» –
// puts one `Offer` of `kind: 'staff'` into `world.offers` on every season-wrap week for every
// employed seat. So a career WITH a seat now carries letters it did not carry before and a career
// without one carries none, which is exactly what the ladder printed when the 104 cells were
// recomputed, without anybody having to arrange it.
//
// ⚠⚠ AND IT MOVES EVERY `PRE_V*` RUNG, WHICH THE v83 APPEND ABOVE DID NOT – the difference is the
// whole reason this block is a separate entry rather than a line added to that one. A key APPEND is
// peeled off by `careerHashAtSchema` and the old identities come back; a BEHAVIOUR change is not
// peelable, because the peel drops schema keys and never touches `offers`. A letter that is in the
// live world is therefore in every rolled-back serialisation of it, and «this save at v50» is now a
// v50-shaped save of a career whose coach wrote home. `SAVE_SCHEMA_VERSION` did not move.
//
// ⚠ PER-KEY DIFF TAKEN FIRST, AND THE CONTROL WAS THIS TREE WITH THE TICK CALL NEUTRALISED IN PLACE
// – `settleStaffLetters(world)` in `world/phaseAiWeek.ts`, the single line, commented out in a
// DETACHED WORKTREE at this branch's head `f27b5cdc`. Never in the shared checkout, never by a staged
// revert, and restored by `git checkout --` only because that worktree's index was untouched.
// ⭐ AND THE A ARM WAS CHECKED FOR ITS ABSENT WRITER before it was believed, which is CLAUDE.md's
// null-arm check run in the negative direction: `raiseStaffLetter` has exactly ONE caller anywhere in
// `src/` – `settleStaffLetters` – and on the neutralised tree nothing in `src/` calls that, so the A
// arm is genuinely the old inbox rather than a writer sitting in a tree with nothing to reach it.
// `tools/frozen-key-diff.ts` on all five cells, both arms, **ten explicit commands** – never
// `set -- $cell`, the zsh word-split trap this file has now watched fire three times:
//
//   · 5/0 (25k middle, grinder)            – **1 key of 94: `offers`** (`f2c132e68a2e` -> `fd3be3d48351`)
//   · 8/0 (120k wealthy, elite, grinder)   – **1 of 94: `offers`** (`739b55428e3c` -> `eb68fef34393`)
//   · 0/1 (8k working, SELF-COACHED)       – **0 keys. THE WHOLE PRINT-OUT BYTE-IDENTICAL.** No seat,
//                                            no letter: `offers` stands at `a304b048e5e7` on both arms.
//   · 6/1 (25k middle, HIGH coach, player) – **1 of 93: `offers`** (`ab4d6f6cd38d` -> `735c5e6f9c4c`)
//   · 5/1 (25k middle, middle, player)     – **1 of 94: `offers`** (`a39a78730453` -> `27dd721cf429`)
//
// ⚠ `rngMain` REPRODUCES THE THREE CANONICAL FINGERPRINTS – 5/0 `1dbff28caca2` · 8/0 `aebc8101d6df`
// · 0/1 `d84bcbf0c481`, and the two witnesses carry 5/0's – identical between the arms on every one
// of the five cells. `results`, `careerTotals`, `seasonHistory`, `events` and `schemaVersion` were
// checked BY NAME on every arm and held: the post is a REPORT, and a report rolls no dice, so it
// moves the inbox and nothing else. ⚠ The frozen capture was RE-RUN rather than assumed – count
// **41550**, hash **e6b0c709**, `tests/condition.test.ts` 51 passed – so the draw count has not moved
// and the shape of the week is what it was.
//
// ⚠ AND THE INBOX DIFF IS COUNTED RATHER THAN DESCRIBED, because «offers moved» is the key and not
// the fact. Every letter in it is the COACH's – the other three seats are never hired inside 156
// weeks, which `walkFrozenCareer` has asserted since v59:
//
//   · 5/0 and 8/0 – **three letters each, at weeks 49, 101 and 153**, the three wrap weeks a
//     156-week walk contains (`week % 52 === 49`, the first off-season week). Both careers begin with
//     a founding coach and no `coach-since-` mark ever appears, so `coachSinceWeek` falls back to
//     week 0 and he has served every season in full.
//   · 6/1 and 5/1 – **one letter each, at week 49 only**, and the two empty wraps are the gate doing
//     its job rather than a miss. The seat is EMPTY at 101 and at 153: 6/1 lets the coach go at week
//     66 and 5/1 at week 90, and both re-hire at week 154 – one week past the last wrap.
//   · 0/1 – **none**, and it could not be otherwise. `coachTier: 'self'` is the parent on the court,
//     `coachId` is null, and a parent does not write the family a letter.
//
// ⚠ WHICH CELLS. `middleGrinder` (5/0) and `eliteGrinder` (8/0) in all thirty-four registers, plus
// `PRE_R28B`'s two witnesses `highPlayer` (6/1) and `middlePlayer` (5/1) – seventy cells. Every
// `selfTravelling` (0/1) cell is UNTOUCHED, all thirty-four of them. Named in full, so that nothing
// re-stamped here has to be inferred from a count: `FROZEN`, `PRE_R28B`, `PRE_NAME_VERA`, and the
// rungs `PRE_V83` `PRE_V82` `PRE_V81` `PRE_V80` `PRE_V79` `PRE_V78` `PRE_V77` `PRE_V76` `PRE_V75`
// `PRE_V74` `PRE_V73` `PRE_V72` `PRE_V71` `PRE_V69` `PRE_V68` `PRE_V67` `PRE_V66` `PRE_V65`
// `PRE_V64` `PRE_V63` `PRE_V62` `PRE_V61` `PRE_V60` `PRE_V59` `PRE_V58` `PRE_V57` `PRE_V56`
// `PRE_V55` `PRE_V52` `PRE_V51` `PRE_V50`.
//
// ⚠ EVERY NEW VALUE WAS COMPUTED BY RUNNING THE EXPORTED HELPERS INSIDE A THROW-AWAY VITEST PROBE
// that printed constant, cell, old and new on one line each (the 17.09 pass's own instrument, kept),
// all 104 cells in a single run – and the prediction, that only self-coached cells hold, was ASSERTED
// inside that run before a character was stamped. Stamping was exact-hash replacement with the
// occurrence count printed per hash: **2 for each shared FROZEN/PRE_R28B cell, 1 for each witness,
// old-name and rung cell, 68 distinct hashes covering 70 cells, 0 missing, 0 left over**, and the 34
// held hashes were confirmed still present afterwards. ⚠ The probe's own exit code was 1 with
// `Tests 1 passed` – the birpc `onTaskUpdate` timeout on an 83 s synchronous test, the all-green-
// non-zero shape this file's header was written about – so its verdict was read from the assertions
// and not from the code.
//
// =================================================================================================
// ⭐⭐⭐ RE-STAMPED 18.09.2026 – THE WEDDING, WAVE 7 T1: A PURE KEY APPEND, AND THE PREDICTION HELD.
// **ELEVEN LIVE CELLS OF A HUNDRED AND FOUR, AND NOT ONE `PRE_V*` RUNG MOVED.** SCHEMA v82 -> v83.
// =================================================================================================
//
// WHAT MOVED THEM. v83 appends `latchedWeek` and `partnerName` to every `LoveEpisode` row, both null
// at birth (`docs/plans/life-wave-7-builder-2026-09.md` §2 T1), and ships NO WRITER for either value
// on the T1 tree – the hazard is T2's, the latch write and the naming are T3's, and every one of
// them is gated on 23+, which a 156-week walk (age 16.6) can never reach. So every register that
// hashes the LIVE world moved – `FROZEN`'s trio, `PRE_R28B`'s five (its trio shares `FROZEN`'s
// cells, the shared-cell rule), `PRE_NAME_VERA`'s three – and every rollback rung HELD, because the
// v83 peel drops the two row fields ahead of all its older peels: `PRE_V83` holds the VERBATIM v82
// `FROZEN` constants, the identity the wave's §0 predicted.
//
// ⚠ PER-KEY DIFF TAKEN FIRST, control = this branch's head `1b9eb2b1` with a clean tree (the wave
// absent – T1 is the wave's first commit, so the head IS the control and no neutralisation was
// needed). `tools/frozen-key-diff.ts` on all five cells, both arms, **five explicit commands** –
// never `set -- $cell`, the zsh word-split trap this pass watched fire a THIRD time on its own
// first control attempt (files named `control-5 0-.txt`, policy silently defaulted; re-run
// explicitly before anything was read):
//
//   · 5/0 (25k middle, grinder)            – **1 key of 95: `schemaVersion`** (`a46e37632fa6` -> `bbb965ab0c80`)
//   · 8/0 (120k wealthy, elite, grinder)   – **2 of 94: `schemaVersion`, `loveEpisodes`**
//                                            (`8c5884c15bcd` -> `444ca2a71789`) – its one row,
//                                            `p:137`, v77's own witness cell, gaining the two null
//                                            fields: the key append itself, never a behaviour
//   · 0/1 (8k working, SELF-COACHED)       – **1 of 96: `schemaVersion`**
//   · 6/1 (25k middle, high, player)       – **1 of 94: `schemaVersion`**
//   · 5/1 (25k middle, middle, player)     – **1 of 95: `schemaVersion`**
//
// ⚠ `rngMain` REPRODUCES THE THREE CANONICAL FINGERPRINTS – 5/0 `1dbff28caca2` · 8/0 `aebc8101d6df`
// · 0/1 `d84bcbf0c481` – identical between the arms on every cell. Zero MAIN draws anywhere in the
// wave: the wedding hazard (`seed:life:wedding:<week>`) and the name (`seed:life:partner-name:
// <episodeId>`) are purpose-scoped sub-streams, and neither even exists on the T1 tree. The frozen
// capture (41550 / e6b0c709) is untouched by construction.
//
// ⚠ EVERY NEW VALUE WAS COMPUTED BY RUNNING THE EXPORTED HELPERS INSIDE A THROW-AWAY VITEST PROBE
// that printed helper, cell and hash on one line each (the 17.09 pass's own instrument, kept), and
// stamped by exact-hash replacement with the occurrence count printed per hash – 2 for each shared
// FROZEN/PRE_R28B cell, 1 for each witness and old-name cell, 0 left over. The peel identity was
// asserted in the same probe run, against the pre-wave constants, before any stamping.
//
// =================================================================================================
// ⭐⭐⭐ RE-STAMPED 17.09.2026, THE FIFTH OF THE DAY – ROUND 42 #51, THE COACH'S FEE FIXED AT HIRE.
// **SEVENTY-TWO CELLS OF A HUNDRED AND ONE, AND THE PATTERN IS THE PROOF: EVERY `PRE_V*` RUNG MOVED
// ITS TWO COACHED CELLS AND HELD ITS SELF-COACHED ONE, ALL TWENTY-NINE OF THEM.** SCHEMA v81 -> v82.
// =================================================================================================
//
// WHAT MOVED THEM. v82 changes what a hired coach COSTS (`docs/specs/the-coachs-raise-2026-09.md`).
// The shipped till re-derived his hourly figure every week from her age band and her ranking; v82
// bills the LABOUR that was agreed at hire plus the court at today's price. So a career with a coach
// is a different career and a career without one is not – which is exactly what the ladder printed
// when the 101 cells were recomputed, without anybody having to arrange it.
//
// ⚠⚠ AND THE BUILD PREDICTED A PURE KEY APPEND AND WAS WRONG BY ONE WEEK. Its reasoning: «a frozen
// career is 156 weeks, she ends at `coachAgeBand` 0 with no WTA rank, so the agreed labour equals the
// market's, the ceiling leaves no room and no ask can fire». `ageAtWeek` returns WHOLE YEARS – 16
// from week 104, 17 at week 156 exactly – and this walk's final tick runs AT 156. So the last week of
// every coached frozen career is billed across an age-band step, and 156 is also `3 x 52`, so the
// annual ask fires on the same week. **The per-key diff caught it before a constant was touched**,
// which is the whole of why this file demands the diff FIRST.
//
// ⚠ PER-KEY DIFF TAKEN FIRST, AND THE CONTROL WAS THIS TREE WITH MY OWN COMMIT NEUTRALISED IN PLACE
// by a reverse application, restored with a hard reset and never a checkout-with-pathspec.
// ⭐ AND THE A ARM WAS CHECKED FOR ITS ABSENT READER before it was believed: a search for `coachDeal`
// across `src/` on the neutralised tree returns nothing at all, so it is the old till and not a new
// key sitting in a tree with nothing to read it. `tools/frozen-key-diff.ts` on all five cells, both
// arms, and **the five pairs were run as five explicit commands** – never `set -- $cell`, the zsh
// word-split trap this file has now watched fire twice:
//
//   · 5/0 (25k middle, grinder)            – **7 keys of 93**: `careerTotals`, `coachDeal`, `events`,
//                                            `financeWeeks`, `fundsCents`, `nextEventId`, `schemaVersion`
//   · 8/0 (120k wealthy, elite, grinder)   – **the same 7 of 93**
//   · 6/1 (25k middle, high, player)       – **6 of 92** – the same, without `nextEventId`
//   · 5/1 (25k middle, middle, player)     – **6 of 93** – the same, without `nextEventId`
//   · 0/1 (8k working, SELF-COACHED)       – **2 of 94: `coachDeal` and `schemaVersion`.** Nothing
//     else, and it could not be otherwise: there is no coach, so there is no fee to fix.
//
// ⚠ `rngMain` REPRODUCES THE THREE CANONICAL FINGERPRINTS THE LADDER HAS CARRIED FOR EIGHTEEN WAVES –
// 5/0 `1dbff28caca2` · 8/0 `aebc8101d6df` · 0/1 `d84bcbf0c481` – identical between the A and B arms on
// every one of the five. **Neither the fee nor the ask can take a draw**: the fee is integer
// arithmetic over a persisted figure and the score is a weighted mean over state the tick has already
// written, so MAIN cannot move and the frozen capture (41550 / e6b0c709) is untouched.
//
// ⚠⚠ EVERY NEW VALUE WAS COMPUTED BY RUNNING THE EXPORTED HELPERS INSIDE A THROW-AWAY VITEST PROBE
// THAT PRINTED CONSTANT, CELL AND HASH ON ONE LINE EACH – never transcribed from a failure message,
// which elides a hash with an ellipsis, and never by a global search-and-replace, which cannot tell
// two rungs that happen to share a cell apart. ⭐ The probe's (helper, args, constant, cell) table was
// DERIVED FROM THE ASSERTIONS THEMSELVES by regex over the six test files rather than hand-written.
// It reached 99 of the ladder's 101 positive cells; the two it cannot see are asserted through a
// LOCAL (`const high = windowRuleWitness(6, 1)` … `high.underTheWindowRule`) so no single `expect`
// line carries the call, and they were run explicitly and named in the probe rather than left silent.
// **101 cells visited, 72 rewritten, 29 already correct, 0 unmatched**, and the 58-case ladder is
// green on the result.
//
// ⚠⚠ AND A COMPARISON SCRIPT WROTE A FALSE ALARM INTO THIS PASS, WHICH IS WORTH THE LINES BECAUSE IT
// IS THIS FILE'S OWN RECURRING FAILURE IN A NEW COSTUME. A throw-away regex reading the literal after
// `export const FROZEN = {` reported that `PRE_V82.selfTravelling` did not reproduce the v81 career –
// and the block it had actually read was `PRE_V78`'s, because **`FROZEN` is the one register in this
// file whose doc comment sits INSIDE the literal**, so its cells are two hundred lines below its
// declaration and a non-greedy span stops long before them. The code was right and the instrument was
// broken, which is the `indexOf`-returning-minus-one family exactly. Settled by running
// `careerHash(0, 1)` and a hand-rolled walk in ONE process on the neutralised tree: all three numbers
// agreed, and `PRE_V82.selfTravelling` IS the verbatim v81 live career.
//
// ⭐⭐ WHICH RUNGS MOVED IS THE PROOF RATHER THAN THE CONSEQUENCE, in this file's established shape.
// `coachDeal` is a v82 world key, so `careerHashAtSchema(…, <= 81)` peels it away – and every rung
// therefore returns «the v81 SHAPE» while the CAREER inside it is the new one wherever a coach was
// paid. The eleven live-shaped cells (`FROZEN`, `PRE_R28B`, `PRE_NAME_VERA`) moved because they carry
// the key and the version; the twenty-nine `PRE_V*` self-coached cells held because the peel is exact
// and nothing else about that career changed.
//
// =================================================================================================
// ⭐⭐⭐ RE-STAMPED 17.09.2026, THE FOURTH OF THE DAY – ROUND 44's SECOND HALF. **THIRTY-FIVE CELLS OF
// NINETY-EIGHT, AND THE OTHER SIXTY-THREE HELD BYTE FOR BYTE. EXACTLY ONE KEY MOVED ON EVERY CAREER:
// `lifeLog`.** NO SCHEMA MOVED, NO MIGRATION IS OWED, AND `rngMain` IS BYTE-IDENTICAL EVERYWHERE.
// =================================================================================================
//
// WHAT MOVED THEM, AND THE BLOCK ABOVE PREDICTED IT IN AS MANY WORDS. Its own sentence was: «THE DAY
// THAT CHANGES, THIS WHOLE FILE RE-ANCHORS, and it will change for a reason that looks unrelated: a
// longer `FREEZE_WEEKS`, or **one corpus row given a `school` stage**.» That day is the same day. The
// eight situations that had been hand-written in `SMALL_TALK_SHIPPED` moved INTO the corpus document
// and came back four-voiced – 11 voice columns to 32 – and six of the eight declare `school`. A
// frozen career is 156 weeks from its own start and **never leaves `school`**, so before this she
// reached a handful of single-voice rows and now she reaches every one her gates allow. Her pool
// grew, `drawSmallTalkSubject` sees more subjects, `pickInt` lands elsewhere, and the `lifeLog` row
// records a different conversation. ⭐ The prediction was worth writing down: it turned a re-anchor
// that would have looked like a mechanic into ten minutes of confirming arithmetic.
//
// ⚠ PER-KEY DIFF TAKEN FIRST, as this file's protocol demands, and THE CONTROL WAS THIS TREE WITH MY
// OWN COMMIT REVERTED IN PLACE – `git revert --no-commit 3e291ee5`, restored with `git reset --hard`
// and never `git checkout -- src` (the staged revert has already overwritten the index, so a checkout
// runs the A arm twice and yields the byte-identical diff that looks like a null result).
// ⭐ AND THE A ARM WAS CHECKED FOR ITS READER before it was believed: the neutralised tree really does
// declare `SMALL_TALK_SHIPPED` again and really does compose `[...SHIPPED, ...CORPUS]`, so it is the
// old catalogue and not a new constant sitting in a tree with nothing to read it.
// `tools/frozen-key-diff.ts` on all five cells, both arms, headers checked against the invocation,
// and **the five pairs were run as five explicit commands** – never `set -- $cell`, the zsh
// word-split trap this file has now watched fire twice:
//
//   · 5/0 (25k middle, grinder)            – **1 key of 93: `lifeLog`.** Nothing else.
//   · 8/0 (120k wealthy, elite, grinder)   – **1 key of 93: `lifeLog`.** Nothing else.
//   · 0/1 (8k working, self-coached)       – **1 key of 94: `lifeLog`.** Nothing else.
//   · 6/1 (25k middle, high, player)       – **1 key of 92: `lifeLog`.** Nothing else.
//   · 5/1 (25k middle, middle, player)     – **1 key of 93: `lifeLog`.** Nothing else.
//
// ⚠⚠ `schemaVersion` DID NOT MOVE, which is the difference between this re-stamp and the one below
// it. Round 44's first half appended `LifeBeatRecord.frame` and owed v81; this half writes no key at
// all – it changes which SITUATION a row records, inside a field that already existed. `5316ca1c5ddc`
// on every cell in both arms.
//
// ⚠ `rngMain` REPRODUCES THE THREE CANONICAL FINGERPRINTS THE LADDER HAS CARRIED FOR EIGHTEEN WAVES –
// 5/0 `1dbff28caca2` · 8/0 `aebc8101d6df` · 0/1 `d84bcbf0c481` – identical between the A and B arms on
// every one of the five. **A bigger catalogue cannot take a draw**: `rollSmallTalk` derives
// `:smalltalk:<week>`, `:smalltalk:subject:<week>` and `:smalltalk:situation:<week>` in that order
// whatever the pool holds, so the COUNT of draws is unchanged and only the index the last one lands
// on differs. The per-key diff says so from the other side.
//
// ⭐⭐ AND WHICH RUNGS MOVED IS THE PROOF RATHER THAN THE CONSEQUENCE, in the exact shape the round-43
// block below established: **`lifeLog` became a required world key at v73, so
// `careerHashAtSchema(…, ≤ 72)` peels it away – and every `PRE_V*` rung at or below `PRE_V73` HELD,
// character for character.** What moved is `PRE_V74`–`PRE_V81` (24 cells) plus the three LIVE-shaped
// registers (`FROZEN`, `PRE_R28B`, `PRE_NAME_VERA`, 11 cells): exactly the cells whose serialisation
// still contains the key. A change that had touched anything else would have moved a rung below 74.
//
// ⚠⚠ EVERY NEW VALUE WAS COMPUTED BY RUNNING THE EXPORTED HELPERS INSIDE A THROW-AWAY VITEST PROBE
// THAT PRINTED CONSTANT, CELL AND HASH ON ONE LINE EACH – never transcribed from a failure message,
// which elides a hash with an ellipsis, and never by a global search-and-replace, which cannot tell
// two rungs that happen to share a cell apart. ⭐ The probe's (helper, args, constant, cell) table was
// DERIVED FROM THE ASSERTIONS THEMSELVES by regex over the six test files rather than hand-written,
// because a hand-written table is a second copy of the ladder and the copy is what rots. The rewrite
// is block-scoped: it finds `export const <NAME> = {`, cuts at the next top-level `export `, and
// inside that span replaces one key's 64-hex value, refusing the whole file if any cell matches other
// than exactly once. **96 cells visited, 33 rewritten, 63 already correct, 0 unmatched.**
//
// ⚠⚠ AND THE DERIVATION WAS INCOMPLETE BY TWO, WHICH IS RECORDED BECAUSE THE TEST CAUGHT IT AND THE
// METHOD DID NOT. `PRE_R28B.highPlayer` and `PRE_R28B.middlePlayer` are asserted through
// `windowRuleWitness(6, 1).underTheWindowRule` rather than through one of the four named hash
// helpers, so the regex that reads `expect(helper(args), …).toBe(CONST.cell)` could not see them and
// the first re-run went red on one of the two. ⭐ The check that closes this hole for good is one
// command and it is the one to run next time BEFORE stamping: grep every `CONST.cell` reference in
// `tests/` and compare the set against what the probe printed – **98 referenced, 96 covered**, and
// the two it names are the two. They were then computed by a second probe from the helper itself,
// never copied out of the failure message, and the value the probe read for `highPlayer` agreeing
// with the one the red had printed is a confirmation rather than the source.
//
// ⚠ THE FROZEN MAIN CAPTURE IS UNMOVED AND NOT RE-PINNED: 41550 draws / hash `e6b0c709`,
// tests/condition.test.ts, re-run GREEN on this tree (51 tests). It holds by construction – no draw
// was added or removed – and the per-key `rngMain` identity confirms it from the other side.
//
// =================================================================================================
// ⭐⭐⭐ RE-STAMPED 17.09.2026, THE THIRD OF THE DAY – ROUND 44, v80 → v81. **ELEVEN CELLS OF
// NINETY-FIVE, AND THE OTHER EIGHTY-FOUR HELD BYTE FOR BYTE.** TWO KEYS MOVED ON ALL FIVE CAREERS –
// `lifeLog` AND `schemaVersion` – AND `rngMain` IS BYTE-IDENTICAL ON EVERY ONE OF THEM.
// =================================================================================================
//
// WHAT MOVED THEM. `LifeBeatRecord.frame`, the frame-pool spec's third mechanical ruling: the id of
// the delivery scene a `'small-talk'` beat was raised in, PERSISTED on the row because a frame «must
// not change after a save, a reload, or the array growing» and only the third of those needs state.
// The key lands on every small-talk row these careers already held, so `lifeLog` moves; the version
// moves with it. Nothing else in the serialisation is touched.
//
// ⭐⭐ AND THE ROUND'S BIG HALF DOES NOT REACH THESE CAREERS AT ALL, WHICH IS THE FINDING WORTH
// CARRYING AND THE ONE THAT PREDICTS WHEN IT WILL STOP BEING TRUE. Round 44 also lands the corpus –
// the small-talk catalogue goes from **8 situations to 51**, four voices each, which is exactly the
// «a behaviour change, not a key append» shape that re-anchored all 84 rungs at v79 and again at v80.
// It changed nothing here because of a CALENDAR fact: `FREEZE_WEEKS` is 156, a frozen career is three
// years from its own start, **so the girl in it never leaves `school`** – and all 43 corpus rows
// declare `after-school`, `college` and `independent` with no `school` among them. Her reachable pool
// did not grow by one row, so not one `detail` differs.
//
// ⚠⚠ THE DAY THAT CHANGES, THIS WHOLE FILE RE-ANCHORS, and it will change for a reason that looks
// unrelated: a longer `FREEZE_WEEKS`, or one corpus row given a `school` stage. Whoever meets that
// red should read this paragraph before looking for a mechanic – the same service the 17.09 block
// below performed for the copy rewrite that followed it eight cases later.
//
// ⚠ PER-KEY DIFF TAKEN FIRST, as this file's protocol demands, and THE CONTROL WAS THIS TREE WITH
// ROUND 44's OWN CHANGE NEUTRALISED IN PLACE by reverse edit – `SMALL_TALK_SITUATIONS` narrowed back
// to the shipped eight, the frame stamp removed from `rollSmallTalk`'s raise, `SAVE_SCHEMA_VERSION`
// back to 80 – never `git checkout --`, and never the previous commit (the shared-checkout rule).
// ⭐ AND THE A ARM WAS CHECKED FOR ITS READER before it was believed: `grep -c SMALL_TALK_CORPUS` on
// the neutralised tree returns 1, the import alone, with no reader in the catalogue – which is the
// «a constant without its reader is a null arm that looks like a null result» check of 17.08.
// `tools/frozen-key-diff.ts` on all five cells, both arms, headers checked against the invocation:
//
//   · 5/0 (25k middle, grinder)            – **2 keys of 93: `lifeLog`, `schemaVersion`.**
//   · 8/0 (120k wealthy, elite, grinder)   – **2 keys of 93: `lifeLog`, `schemaVersion`.**
//   · 0/1 (8k working, self-coached)       – **2 keys of 94: `lifeLog`, `schemaVersion`.**
//   · 6/1 (25k middle, high, player)       – **2 keys of 92: `lifeLog`, `schemaVersion`.**
//   · 5/1 (25k middle, middle, player)     – **2 keys of 93: `lifeLog`, `schemaVersion`.**
//
// ⚠ `rngMain` REPRODUCES THE THREE CANONICAL FINGERPRINTS THE LADDER HAS CARRIED FOR SEVENTEEN WAVES
// – 5/0 `1dbff28caca2` · 8/0 `aebc8101d6df` · 0/1 `d84bcbf0c481` – identical between the A and B arms
// on every cell. The frame is drawn on `seed:smalltalk:frame:<week>`, a purpose-scoped sub-stream
// re-derived at the call site and persisting nothing, so MAIN cannot see it; the per-key diff says so
// from the other side.
//
// ⚠⚠ EVERY NEW VALUE WAS COMPUTED BY RUNNING THE EXPORTED HELPERS (`careerHash`,
// `careerHashAtSchema` at all twenty-nine rungs, `careerHashUnderTheOldName`,
// `careerHashUnderTheWindowRule`, `windowRuleWitness`) INSIDE A THROW-AWAY VITEST PROBE THAT PRINTED
// CONSTANT, CELL AND HASH ON ONE LINE EACH – never transcribed from a failure message, which elides a
// hash with an ellipsis, and never by a global search-and-replace of one value with another, which
// cannot tell two rungs that happen to share a cell apart. The rewrite is block-scoped: it finds
// `export const <NAME> = {`, cuts at the next top-level `export `, and inside that span replaces one
// key's 64-hex value after asserting the value found is the one the probe read. **95 cells visited,
// 11 rewritten, 84 already correct, 0 unmatched** – and the eleven are exactly the cells that hash a
// LIVE-shaped world (`FROZEN`, `PRE_R28B`, `PRE_NAME_VERA`), because those carry the new key and the
// new version while every `PRE_V*` rung peels both away. ⚠ The probe exited 1 on birpc's 60 s RPC
// window with the test itself GREEN – this file's own all-green-non-zero shape, met once more – and
// it was deleted the moment the stamp was applied.
//
// ⚠ THE FROZEN MAIN CAPTURE IS UNMOVED AND NOT RE-PINNED: 41550 draws / hash `e6b0c709`,
// tests/condition.test.ts, re-run green on this tree. It holds BY CONSTRUCTION and the per-key
// `rngMain` identity confirms it.
//
// =================================================================================================
// ⭐⭐⭐ RE-STAMPED 17.09.2026, THE SECOND OF THE DAY – THE SHEET APPLIED TO THE EXISTING LETTERS.
// SIXTY-TWO CELLS AGAIN, AND THE WHOLE MOVE IS **ONE SENTENCE OF ENGLISH**. `rngMain` IS
// BYTE-IDENTICAL ON ALL FIVE CAREERS AND THE FROZEN MAIN CAPTURE IS UNMOVED.
// =================================================================================================
//
// WHAT MOVED THEM. The owner's terminology sheet, applied to the copy that shipped BEFORE round 43
// («the review could be applied to our existing letters too»): «a professional operation» became his
// own unlock sentence, «the payroll» became the team, and the travel cost became ONE ADDITIONAL FARE
// on every surface that names it. Twenty-seven strings across five engine modules and six
// components, plus five hyphens promoted to the house short dash. No constant, no predicate, no
// draw, no schema key – `SAVE_SCHEMA_VERSION` is still 80 and no migration was added.
//
// ⚠⚠ AND THE BLOCK BELOW PREDICTED THIS ONE IN AS MANY WORDS: «the wave that ships the next
// re-wording should expect this file to go red and should not go looking for a mechanic». It went
// red eight cases later the same day, and that is the only sentence of it worth repeating – **A LINE
// OF PLAYER-FACING COPY IS PERSISTED STATE THE MOMENT IT REACHES `addEvent`.**
//
// ⚠ PER-KEY DIFF TAKEN FIRST, as this file's protocol demands, and the CONTROL WAS THIS TREE WITH
// THE TWELVE ENGINE SENTENCES REVERTED IN PLACE by a reverse edit – never `git checkout --`, and
// never the previous commit (the shared-checkout rule). `tools/frozen-key-diff.ts` on all three
// cells, A and B arms, headers checked against the filenames:
//
//   · 5/0 (25k middle, grinder)          – **1 key of 93: `events`.** Nothing else.
//   · 8/0 (120k wealthy, elite, grinder) – **1 key of 93: `events`.** Nothing else.
//   · 0/1 (8k working, self-coached)     – **0 keys of 94. BYTE-IDENTICAL.**
//
// ⭐⭐ AND WHICH SENTENCE DID IT WAS CENSUSED RATHER THAN INFERRED FROM A GUARD, which is worth the
// extra probe because the obvious answer is wrong. Twenty-seven strings moved and **exactly one of
// them reaches a frozen career**: `markCoachTravelOpen`'s notice in `world/milestones.ts` («…a trip
// with the coach costs one additional fare»), which is `keep: true` and so survives the 400-row
// prune that every one of these careers hits. A throw-away census walked all five careers and
// grepped their feeds for all nine candidate sentences:
//
//   middleGrinder 5/0   coach hired, stance off   -> coach-travel-open
//   eliteGrinder  8/0   coach hired, stance off   -> coach-travel-open
//   selfTravelling 0/1  no coach,    stance on    -> NONE
//   highPlayer     6/1  coach hired, stance ON    -> NONE
//   middlePlayer   5/1  coach hired, stance ON    -> NONE
//
// ⭐ THE THREE «NONE» CELLS ARE THE PROOF RATHER THAN A CONVENIENCE, and they fail for two different
// reasons. `markCoachTravelOpen` opens `if (world.coachId === null) return` – there is nobody to
// send – which is 0/1; and then `if (world.coachOnEventWeeks) return` – the notice is «you can now
// do this», so a family already doing it is never told – which is 6/1 and 5/1 under `POLICIES[1]`.
// The sponsors rows that DID move («Coach travel to …», «Masseur travel to …») reach none of the
// five, because inside 156 weeks these careers enter no prize-money rung and the junior stance is
// off. **Not one `selfTravelling`, `highPlayer` or `middlePlayer` cell changed in this file**, which
// is what that reasoning predicts before the rewrite and what `git diff` counts after it: 62 lines,
// every one of them `middleGrinder` or `eliteGrinder`.
//
// ⚠ `rngMain` REPRODUCES THE THREE CANONICAL FINGERPRINTS THE LADDER HAS CARRIED FOR SIXTEEN WAVES –
// 5/0 `1dbff28caca2` · 8/0 `aebc8101d6df` · 0/1 `d84bcbf0c481` – identical between the A and B arms.
// A wording change cannot take a draw, and the per-key diff says so from the other side.
//
// ⚠⚠ EVERY NEW VALUE WAS COMPUTED BY RUNNING THE EXPORTED HELPERS (`careerHash`,
// `careerHashAtSchema` at all twenty-eight rungs, `careerHashUnderTheOldName`,
// `careerHashUnderTheWindowRule`, `windowRuleWitness`) INSIDE A THROW-AWAY VITEST PROBE THAT PRINTED
// CONSTANT, CELL AND HASH ON ONE LINE EACH – never transcribed from a failure message, which elides
// a hash with an ellipsis, and never by a global search-and-replace of one value with another, which
// cannot tell two rungs that happen to share a cell apart. The rewrite is block-scoped: it finds
// `export const <NAME> = {`, cuts at the next top-level `export `, and inside that span replaces one
// key's 64-hex value after asserting the value found is the one the probe read. **95 cells visited,
// 62 rewritten, 33 already correct, 0 unmatched** – and the 62 are only **60 unique hashes**, because
// FROZEN and PRE_R28B share both grinder cells (the window rewrite is a no-op on a career whose kit
// letters all landed on the window's opening week, which `underTheWindowRule`'s own note explains).
// ⚠ The probe exited 1 on birpc's 60 s RPC window (the run is 75.9 s) with the test itself GREEN –
// this file's own all-green-non-zero shape, met once more – and both probes were deleted the moment
...
//
// =================================================================================================
// ⭐⭐⭐ RE-STAMPED 17.09.2026 – HIS COPY REVIEW OF ROUND 43'S NEW STRINGS. SIXTY-TWO CELLS, AND THE
// WHOLE MOVE IS **TWO SENTENCES OF ENGLISH**. `rngMain` IS BYTE-IDENTICAL ON ALL THREE CAREERS AND
// THE FROZEN MAIN CAPTURE IS UNMOVED.
// =================================================================================================
//
// WHAT MOVED THEM. The owner re-worded the coach's eye on her form (`world/form.ts`,
// `coachFormNote`): «She is striking the ball clean.» → «She is striking the ball **cleanly**.» and
// «She needs matches under her.» → «She needs **match play**.» Nothing else. No constant, no
// predicate, no draw, no schema key – `SAVE_SCHEMA_VERSION` is still 80 and no migration was added.
//
// ⚠⚠ AND THAT IS THE LESSON THIS BLOCK EXISTS FOR, because it is not obvious and it will happen
// again: **A LINE OF PLAYER-FACING COPY IS PERSISTED STATE THE MOMENT IT REACHES `addEvent`.** The
// sentence is written into `world.events[].text`, `events` is serialised, and the serialisation is
// what these hashes are over – so a pure wording change is a byte change on every career that ever
// saw the line. The wave that ships the next re-wording should expect this file to go red and should
// not go looking for a mechanic.
//
// ⚠ PER-KEY DIFF TAKEN FIRST, as this file's protocol demands, and the CONTROL WAS THIS TREE WITH
// THE TWO SENTENCES REVERTED IN PLACE by a reverse edit – never `git checkout --`, and never the
// previous commit (the shared-checkout rule). `tools/frozen-key-diff.ts` on all three cells, A and B
// arms, headers checked against the filenames:
//
//   · 5/0 (25k middle, grinder)          – **1 key of 93: `events`.** Nothing else.
//   · 8/0 (120k wealthy, elite, grinder) – **1 key of 93: `events`.** Nothing else.
//   · 0/1 (8k working, self-coached)     – **0 keys of 94. BYTE-IDENTICAL.**
//
// ⭐ THE THIRD CELL IS THE PROOF RATHER THAN A CONVENIENCE. `coachFormNote` opens with
// `if (!world.coachId) return null` – the eye belongs to the man – and 0/1 is the one frozen career
// with nobody in the corner, so she never saw either sentence and her hash could not move. **Not one
// `selfTravelling` cell changed in this file**, which is what that reasoning predicts before the
// rewrite and what `git diff` counts after it: 62 lines, every one of them `middleGrinder` or
// `eliteGrinder`.
//
// ⚠ `rngMain` REPRODUCES THE THREE CANONICAL FINGERPRINTS THE LADDER HAS CARRIED FOR FIFTEEN WAVES –
// 5/0 `1dbff28caca2` · 8/0 `aebc8101d6df` · 0/1 `d84bcbf0c481` – identical between the A and B arms.
// A wording change cannot take a draw, and the per-key diff says so from the other side.
//
// ⚠⚠ EVERY NEW VALUE WAS COMPUTED BY RUNNING THE EXPORTED HELPERS (`careerHash`,
// `careerHashAtSchema` at all twenty-eight rungs, `careerHashUnderTheOldName`,
// `careerHashUnderTheWindowRule`) INSIDE A THROW-AWAY VITEST PROBE THAT PRINTED CONSTANT, CELL AND
// HASH ON ONE LINE EACH – never transcribed from a failure message, which elides a hash with an
// ellipsis, and never by a global search-and-replace of one value with another, which cannot tell
// two rungs that happen to share a cell apart. The rewrite is block-scoped: it finds
// `export const <NAME> = {` and replaces only that block's three value lines. **93 cells visited,
// 62 rewritten, 31 already correct, 0 unmatched.** ⚠ The probe exited 1 on birpc's 60 s RPC window
// (the run is 71.9 s) with the test itself GREEN – this file's own all-green-non-zero shape, met
// once more – and it was deleted the moment the stamp was applied.
//
// ⚠ THE FROZEN MAIN CAPTURE IS UNMOVED AND NOT RE-PINNED: 41550 draws / hash `e6b0c709`,
// tests/condition.test.ts. It holds BY CONSTRUCTION and the per-key `rngMain` identity confirms it.
//
// =================================================================================================
// ⭐⭐⭐ RE-STAMPED 16.09.2026 – ROUND 42 #47, SECOND READING. FIFTY-SIX CELLS, AND THE STAMP IS A
// REVERT RATHER THAN A CHANGE: the cameo's cheque went back to the flat `amountCents` band it had
// before #47's first reading turned it into a fraction of the next unpayable trip's gap.
// `rngMain` IS BYTE-IDENTICAL ON EVERY CAREER. THE FROZEN MAIN CAPTURE IS UNMOVED.
// =================================================================================================
//
// WHAT MOVED THEM, and why a sizing revert reaches this far. The owner read #47's own ship note and
// answered «вообще всё не так»: «60-80% закрытия» is a FREQUENCY – «помощь должна срабатывать в
// 80% случаев» – and not a fraction of a sum; the size was never the broken part («механизм
// нормально давал денег, нормальными суммами, просто смотрел только на кошелек»). So the cheque is
// $500–1,500 again against a J trip's measured $1,100–3,600, where the gap fraction paid a median
// $129 – and the trip probe stays as the GATE, which is the half that was right.
//
// ⚠⚠ PER-KEY PROTOCOL RUN FIRST, ON ALL THREE CAREERS, AND THE CONTROL WAS THIS TREE WITH THE
// REVERT NEUTRALISED IN PLACE (`tools/frozen-key-diff.ts --preset N --policy M`, both arms, then the
// B files copied back – never `git checkout --` under another agent's edits). Of **91 keys, 42 moved
// and 49 held**, and the split is the whole reading:
//
//   MOVED    fundsCents and kidFundsCents, then everything the wallet steers – entries, results,
//            seasonHistory, kidRank and its three registers, skills, condition, spirit, bond,
//            careerTotals, trophiesByTier, offers, cohort. A J-series family with more money in the
//            hardest years enters different tournaments, and the career diverges from there.
//   ⭐ HELD  `rngMain`, `seed`, `schemaVersion`, `potential`, `temperament`, `profile`, `plan`,
//            `coachId`, every seat key (`masseur*`, `psychologist*`, `sparring*`), `composureBonus`.
//
// ⭐⭐ `rngMain` HOLDING IS THE INVARIANT-2 PROOF FOR THIS CHANGE, TAKEN PER KEY RATHER THAN ASSERTED.
// The cameo spends exactly one `rng()` on `seed:sponsor:cameo:gift:<week>` under BOTH readings –
// `pickInt` spent one and the share draw spent one – so the sub-stream's shape never moved and MAIN
// never saw the difference. A revert that changed a draw COUNT would have shown here first.
//
// ⚠ AND ONE HONEST NOTE ABOUT THE STAMP THIS ONE REPLACES. Bundle 15's fixture stamp was committed
// under the #34 message (`fc856e17`), but the working tree already carried #47's gap fraction, so
// that stamp silently baked in an economic change its commit message never mentioned. It is moot now
// – this stamp supersedes it – but the split-by-pathspec habit does not protect a FIXTURE from a
// change that is in the tree but not in the commit, and that is worth knowing before the next wave.
//
//
// =================================================================================================
// ⭐⭐⭐ RE-STAMPED FOR ROUND 43 #8(a) (16.09.2026, SCHEMA v79 – UNMOVED) – TWENTY CONSTANTS, AND
// **EXACTLY ONE KEY MOVED ON EVERY CAREER: `lifeLog`.** NO SCHEMA MOVED, NO MIGRATION IS OWED,
// `rngMain` IS BYTE-IDENTICAL ON ALL THREE CAREERS AND THE FROZEN MAIN CAPTURE IS UNMOVED.
// =================================================================================================
//
// WHAT MOVED THEM. `rollSmallTalk` excludes her last two conversations from the draw now
// (`withoutRecentSituations`, `world/lifeBeat.ts` – round 43 #8(a), his «они точно не должны так
// часто повторяться»). The pool the two sub-streams pick from is narrower, so a career that would
// have repeated a situation brings a different one, and the `lifeLog` row records the different one.
//
// ⚠⚠ PER-KEY PROTOCOL RUN FIRST, ON ALL THREE CAREERS, AND THE CONTROL WAS THIS TREE WITH BOTH OF
// THE BUNDLE'S CHANGES NEUTRALISED **IN PLACE** – `withoutRecentSituations` returning
// `[...reachable]` and the masseur's drift returning `ECONOMY.masseur.perSessionCents` with
// `resolveMasseurRaise` short-circuited, restored by copying the saved B files back, never by a
// checkout under another agent's edits. `tools/frozen-key-diff.ts` on 5/0, 8/0 and 0/1:
//
//   MOVED    `lifeLog`, and nothing else. 5/0 `888cb1de403b` -> `68ff23458562`,
//            0/1 `a04152ae3c9c` -> `124063e1b55c`.
//   ⭐ HELD  EVERY other key on every career – `rngMain`, `events`, `nextEventId`, `fundsCents`,
//            `kidFundsCents`, `entries`, `results`, `skills`, `condition`, `spirit`, `bond`,
//            `careerTotals`, and every seat key including all seven `masseur*`.
//
// ⭐⭐ THE ELITE GRINDER (8/0) DID NOT MOVE AT ALL, and that is a measurement rather than luck: her
// 156 weeks hold no small-talk row whose draw the exclusion could narrow. She is the third career
// and she stayed where she was, which is what a one-key change is supposed to look like.
//
// ⭐⭐⭐ AND THE IDENTITY THAT MAKES «ONLY `lifeLog`» A PROOF RATHER THAN A CLAIM: **EVERY `PRE_V*`
// RUNG AT OR BELOW `PRE_V73` HELD, CHARACTER FOR CHARACTER, AND THE FIRST RUNG THAT CARRIES
// `lifeLog` IS THE FIRST THAT MOVED.** `lifeLog` became a required world key at v73, so
// `careerHashAtSchema(…, ≤ 72)` peels it away – and those rungs (`PRE_V50`…`PRE_V73`) reproduce
// byte for byte on the new tree. The twenty cells below are exactly the ones whose serialisation
// still contains the key.
//
// ⭐⭐ `rngMain` HOLDING IS THE INVARIANT-2 PROOF FOR THIS CHANGE, TAKEN PER KEY RATHER THAN ASSERTED.
// The exclusion adds no stream and changes no draw COUNT: `rollSmallTalk` still derives
// `:smalltalk:<week>`, `:smalltalk:subject:<week>` and `:smalltalk:situation:<week>`, in that order,
// and every one of them is a purpose-scoped sub-stream. `tests/condition.test.ts` is green on this
// tree – 41550 draws, `e6b0c709` – and the A arm reproduced all 106 frozen cases before the stamp.
//
// ⚠ THE MASSEUR'S ANNUAL ASK (round 43 #4) IS IN THE SAME BUNDLE AND REACHES NO CAREER HERE AT ALL,
// which the per-key diff says from the other side: not one `masseur*` key moved, and `fundsCents`
// held. No frozen career hires him inside 156 weeks – the gate is a counting W-series result and
// these three are still juniors. The seat's own pins are `tests/round43-masseur-raise.test.ts`.
//
// ⚠ APPLIED **BY NAME**, block-scoped, by a script that printed each cell's old and new value and
// REFUSED to write unless the old string occurred exactly once inside its own block – the T3 rule
// («no pass by position and no blind string replacement anywhere»), which matters here because
// `FROZEN.middleGrinder` and `PRE_R28B.middleGrinder` are the same string and move together.
// Twenty cells, fourteen distinct values: FROZEN 40d00fc3/63df8fcd -> ba88104e/1c5da19f,
// PRE_R28B the same two plus 74233294 -> 8dfaf228 and 4803a0cc -> 34c4f437, PRE_NAME_VERA
// 421e454a/5ead3ba9 -> 51184218/ae5c7088, and the six `PRE_V74`…`PRE_V79` pairs.
//
// =================================================================================================
// ⭐⭐⭐ RE-STAMPED FOR WAVE 6's T1 (14.09.2026, SCHEMA v77) – ELEVEN CONSTANTS, AND **THE PROTOCOL'S
// FIRST NESTED PEEL**: `careerHashAtSchema` now MAPS `loveEpisodes` AND RESTS EACH ROW, because for
// the first time in seventy-seven versions a schema move widens a ROW INSIDE A LIST rather than the
// world. EVERY `PRE_V*` RUNG HELD. `rngMain` IS BYTE-IDENTICAL ON ALL FIVE CAREERS. THE FROZEN MAIN
// CAPTURE IS UNMOVED.
// =================================================================================================
//
// WHAT MOVED THEM. v77 appends ONE key to `createWorld`'s literal – `spotlightHabituation`, the
// spotlight's own counter – and FOUR fields to `LoveEpisode`: `publicWeek`, `publicWrong`,
// `airedMetWeek`, `airedEndedWeek` (the spotlight, wave 6 T1;
// `docs/plans/life-wave-6-builder-2026-09.md` §2, the model `docs/specs/who-she-is-2026-09.md` §3c and
// §3c-bis, the booth's boundary `the-way-she-sounds-2026-09.md` C4). Every career here carries the
// world key at its identity value `0`, and every career here carries its `loveEpisodes` rows – all
// none of them, except one.
//
// ⚠⚠ AND THAT EXCEPTION IS THE WHOLE READING OF THIS RE-STAMP. Measured 14.09 on all five cells:
// `middleGrinder`, `selfTravelling`, `highPlayer` and `middlePlayer` carry `loveEpisodes: []` – the
// sha256 of `[]`, `4f53cda18c2b`, four times – and `FROZEN.eliteGrinder` carries exactly ONE row,
// `{id:'p:137', sinceWeek:137, endedWeek:null, knownWeek:137, wants:'open', partnerId:'p:137'}`. So
// the nested peel has ONE witness in this entire file, and she is **the cell this file has already
// lost once to a positional pass** (the T3 note below: «ONE LIVE CELL WAS MISSED BY THE POSITIONAL
// PASS AND IT WENT RED: `FROZEN.eliteGrinder`», because it has been THE SAME STRING as
// `PRE_R28B.eliteGrinder` since v73). ⚠ THE RE-STAMP WAS THEREFORE APPLIED **BY NAME**, block-scoped,
// by a script that printed each cell's old and new value: FROZEN 787028a0/c861a2c9/18a3adc6 ->
// 28b59fd5/179fded1/f5003e91, PRE_R28B the same three plus bf6fef07 -> a46324bf and 856704b4 ->
// 9fe173a7, PRE_NAME_VERA c8025717/9630cc8b/8648aa66 -> c8886dae/9cd2399e/d63af2dc. Eleven cells,
// eight distinct values, no pass by position and no blind string replacement anywhere.
//
// ⚠ PER-KEY DIFF TAKEN FIRST, as this file's protocol demands and never after the fact. Control =
// this branch's head `f588dfd9` with a clean tree; `tools/frozen-key-diff.ts` on all five cells,
// each header read back against its invocation. **2 lines move on four of them** – `schemaVersion`
// (`f74efabef12e` -> `a88a7902cb4e`) and `spotlightHabituation` APPEARING with `5feceb66ffc8` (the
// sha256 of `JSON.stringify(0)`) – and **3 on `eliteGrinder`**, whose `loveEpisodes` moves
// `d447eb28e502` -> `8c5884c15bcd` as her one row gains the four fields. 85 / 83 / 86 / 84 / 85 other
// keys byte-identical, `rngMain`, `results`, `events`, `nextEventId`, `spirit`, `bond`, `temperament`,
// `lifeLog`, `spiritShock` and the walls pair among them. The full table is over `PRE_V77`.
//
// ⚠⚠ AND `PRE_V77` IS WHAT TURNS THAT DIFF INTO AN IDENTITY, which is the only form this file
// accepts for a peel: `careerHashAtSchema(…, 76)` peels the world key AND maps the rows, and returns
// the OLD `FROZEN` constants character for character on all three careers – `eliteGrinder` included,
// which is the one value in this file that can say anything at all about the nested half. Every rung
// below it was RUN and not assumed, before a constant was touched: `careerHashAtSchema` at 75, 74, 73
// and 71 returned `PRE_V76`, `PRE_V75`, `PRE_V74` and `PRE_V72` verbatim.
//
// ⚠ THE FROZEN MAIN CAPTURE IS UNMOVED AND NOT RE-PINNED – 41550 draws / hash `e6b0c709`. It holds
// BY CONSTRUCTION: the step takes no draw on any stream at all, and its two writers put literals in
// (`createWorld` a `0`, the one `loveEpisodes.push` four nulls-and-a-false at a row's birth).
//
// ⚠⚠ TWO ARMS LIVE IN THIS FILE AND BOTH WERE RUN – the wave's other seven are in
// tests/wave6-spotlight-schema.test.ts's own ledger, where the ARM 9 entry is the one worth reading.
// Each was applied by a scripted string edit and UNDONE by the inverse edit, never `git checkout`,
// with this file's md5 checked back to pristine after each.
//
//   ARM 6  the NESTED half of the peel neutered – the `.map` returns each row unchanged, the
//          world-key half left intact – **3 RED in tests/coach-travel-edge.test.ts: the v77, v76 AND
//          v75 rungs**, every one of them through `eliteGrinder`. ⭐⭐ HIGHER THAN PREDICTED, AND THE
//          EXTRA REDS ARE THE MEASUREMENT: the nested peel was expected to be load-bearing at v77's
//          own rung alone, and it is load-bearing at every rung from 74 UP, because each of those
//          keeps `loveEpisodes` in the shape it rolls back to – so a row carrying four fields no
//          shipped version ever wrote breaks all three at once. `PRE_V74` and everything below it
//          HELD, because rung 73 drops the key entirely and a peel cannot be wrong about a key it
//          has already removed.
//   ARM 7  the `schemaVersion < 77 ? preSpotlight` branch removed from the chain, the peel itself
//          kept – **1 RED, the new v77 case ALONE**, every older rung still green. That is what a
//          rung which guards only its own version looks like, and it is v76's ARM 11 repeated.
//
...
// `schemaVersion`, `spotlightHabituation` and `eliteGrinder`'s `loveEpisodes` are the whole of what
// v77 may move, and anything else is a real behavioural change wearing a pre-signed licence.

// =================================================================================================
// ⚠⚠ ROUND 42's ECONOMY BUNDLE (15.09.2026) – **TWO CELLS MOVED. NOT RE-STAMPED HERE.**
// `rngMain` BYTE-IDENTICAL ON ALL FIVE. THE FROZEN MAIN CAPTURE UNMOVED (41550 / e6b0c709).
// =================================================================================================
//
// TWO ENGINE CHANGES reach a bench walk, both the owner's own rulings of 15.09:
//   #41  `ECONOMY.staffShare.coach` 1000/500/(0) -> **1000/1000/1000** – ten per cent of EVERY prize
//        cheque at every finish («10% безусловных отчислений с любых призовых, независимо от глубины
//        прохода»), `staffResultShareBps`' tail reading `everyBps` instead of returning a hard 0.
//   #43  `ECONOMY.sponsor.seasonCap` **deleted** (constant and reader) and `cooldownWeeks` 6 -> **4**
//        («сними потолок, а кулдаун давай 4»).
// (#42 moves `coachMarket`'s `overBudgetCents` denominator, which no bench walk reads: `hireCoach`
// never consults the budget. It cannot reach a frozen career and does not.)
//
// ⚠ PER-KEY DIFF TAKEN FIRST, as this file's protocol demands and never after the fact. Control =
// **MY OWN CHANGE NEUTRALISED IN PLACE** – the three coach rates and the two sponsor dials edited back
// by script and restored from a byte-copy afterwards, the two files' md5 checked back to pristine
// (`ef123fec…` / `5971e7ce…`), never `git checkout --`. `tools/frozen-key-diff.ts` on all five cells,
// each header read back against its invocation:
//
//   · 5/0 `FROZEN.middleGrinder`   – **0 keys of 87. Byte-identical.**
//   · 8/0 `FROZEN.eliteGrinder`    – **0 keys of 86. Byte-identical.**
//   · 6/1 `PRE_R28B.highPlayer`    – **0 keys of 86. Byte-identical.**
//   · 0/1 `FROZEN.selfTravelling`  – **37 KEYS OF 88**: academy, bestFinishByTier, careerTotals,
//     condition, drawnFirstRounds, entries, events, financeWeeks, fundsCents, gearRestWeeks,
//     injuryHistory, internationalEntryWeeks, kidFundsCents, kidRank, kidRankWta, knockHistory,
//     lastSeasonSummary, lifeLog, medicalWithdrawalWeek, milestones, nextEventId, offers,
//     peakPhysical, prevKidRank, prevKidRankWta, proEntryWeeks, results, seasonEntries,
//     seasonHistory, seasonLosses, seasonRecord, seasonStartRank, skills, spirit, trophiesByTier,
//     vacations, walkoverWeek.
//   · 5/1 `PRE_R28B.middlePlayer`  – **EXACTLY 5 KEYS OF 87**: careerTotals, events, financeWeeks,
//     fundsCents, seasonHistory.
//
// ⭐⭐ AND THE TWO MOVES WERE ATTRIBUTED RATHER THAN LUMPED, with two further single-change arms:
//
//   · 0/1's 37 keys are **ALL #43 and NONE of #41** – the coach change alone moves 0 of its keys.
//     Which is correct by construction: she is SELF-COACHED, so no coach share can ever touch her,
//     and she is the WORKING family the local-sponsor cameo actually pays. More sponsor money in
//     different weeks moves her entries, which moves her results, which moves everything.
//   · 5/1's 5 keys are **ALL #41 and NONE of #43** – and they are money keys only. `results`,
//     `skills`, `condition`, `bond` and `spirit` are byte-identical on that career: the coach's cut
//     changed what the family banked and not one match she played.
//
// ⚠⚠ `rngMain` IS BYTE-IDENTICAL ON EVERY CELL AND IN EVERY ARM – `1dbff28caca2` (5/0 and 5/1),
// `d84bcbf0c481` (0/1), the canonical fingerprints this file already records. Neither change takes a
// draw: #41 is integer arithmetic at finalize on a decided cheque, and #43 REMOVES a branch that was
// deliberately placed AFTER an unconditional `rng()` for exactly this reason. `tests/condition.test.ts`
// is green on this tree and its capture is NOT re-pinned.
//
// ⚠ NO RE-STAMP IN THIS BUNDLE, BY THE BUILDER'S BRIEF. The two moved cells are `selfTravelling`
// (19 red cases – the whole version ladder, v77 down to P5, across coach-travel-edge,
// -mid-schemas and -older-schemas) and `PRE_R28B.middlePlayer` (1 red case). 28 red in all, zero
// elsewhere in the unit project. The stamp is the architect's.
...
// =================================================================================================
//
// WHAT MOVED THEM. v76 appends SIX keys to `createWorld`'s literal in ONE append – the psychologist's
// seat (`psychologistHired`, `psychologistRung`, `psychologistFocus`, `psychologistFocusSeason`) and
// the two who-she-is §2a walls keys (`wallsLean`, `wallsFlipped`). Every career here carries all six
// and every one of them carries its identity value, because T1 ships six seats, the migration and NO
// WRITER AT ALL: `hirePsychologist` is T2, `setPsychologistFocus` is T3, and the weekly leaning pass
// with its flip hazard is T7.
//
// ⚠ PER-KEY DIFF TAKEN FIRST, as this file's protocol demands and never after the fact. Control =
// this branch's head `ccc83cbc` with a CLEAN tree, captured before a character was edited;
// `tools/frozen-key-diff.ts` on **all five careers this file names** – the first schema move to
// capture the two window-rule witnesses the 12.09 union merge restored – headers checked on all ten
// captures. ⚠ The word-splitting trap v75's own block warns about is REAL AND IT FIRED HERE TOO, in
// a new costume: this shell is zsh, which does NOT word-split an unquoted `$pair`, so a `for … set --
// $pair` loop ran five captures into one file named after an empty variable. The header check caught
// it, exactly as it caught v75's, and nothing else would have.
//
//   · 5/0 (25k middle, grinder)              – **7 lines of 85**: `schemaVersion` and the six keys
//     APPEARING. 78 keys byte-identical.
//   · 8/0 (120k wealthy, elite, grinder)     – **7 lines of 84**, the same seven. 77 byte-identical.
//   · 0/1 (8k working, self-coached, PLAYER) – **7 lines of 86**, the same seven. 79 byte-identical.
//   · 6/1 (25k middle, HIGH coach, PLAYER)   – **7 lines of 84**, the same seven. 77 byte-identical.
//   · 5/1 (25k middle, middle coach, PLAYER) – **7 lines of 85**, the same seven. 78 byte-identical.
//
// The six new hashes are the SAME SIX on every career, which is what «no writer at all» looks like
// from inside a diff: `psychologistHired` `fcbcf165908d`, `psychologistRung` `6b86b273ff34`,
// `psychologistFocus` and `psychologistFocusSeason` `74234e98afe7` (the sha256 of
// `JSON.stringify(null)`, which v75's block met once and this one meets twice), `wallsLean`
// `8c393bb9b0db`, `wallsFlipped` `1c396dc5e2a9`. `schemaVersion` goes `f369cb89fc62` ->
// `f74efabef12e`.
//
// ⚠⚠ AND WHAT DID **NOT** MOVE IS THE LOAD-BEARING HALF, on all five careers: `rngMain`, `results`,
// `events`, `nextEventId`, the wallet, `bond`, `spirit`, `condition`, `careerTotals`, `financeWeeks`,
// `lifeLog`, `loveEpisodes` and `spiritShock` are every one of them byte-identical. ⭐ `temperament`
// is in that list and it is this wave's own sentence rather than a routine line: identity is
// IMMUTABLE (who-she-is §2a's 09.09 re-cut), the walls are EXPRESSION over it – read through
// `expressedTemperamentOf` and never stored – and a bump that had quietly moved a girl's birth
// temperament is the gravest finding this wave could produce.
//
// ⚠ ELEVEN AND NOT NINE, AND THE TWO EXTRA ARE NOT THIS WAVE'S DOING. `FROZEN`, `PRE_R28B` and
// `PRE_NAME_VERA` hash the LIVE world with NO PEEL AT ALL, so a bumped `schemaVersion` moves every
// value they produce by arithmetic – v74's and v75's commits each recorded NINE. The 12.09 union
// merge then restored `highPlayer` and `middlePlayer` to `PRE_R28B` after v75 had already been
// stamped, so the live set is five plus three plus three. Every `PRE_V*` rung HELD, measured rather
// than assumed: `careerHashAtSchema(…, 74)`, `(…, 73)` and `(…, 71)` were RUN on this tree BEFORE a
// constant was touched and returned `PRE_V75`, `PRE_V74` and `PRE_V72` character for character,
// because `careerHashAtSchema` peels the six new keys ahead of all twelve of its older peels.
// `PRE_V76` is NEW rather than moved, and its three values are the VERBATIM v75 `FROZEN` constants.
//
// ⚠⚠ EVERY NEW VALUE WAS COMPUTED BY RUNNING THE EXPORTED HELPERS (`careerHash`,
// `careerHashAtSchema` at rungs 75/74/73/71, `careerHashUnderTheOldName`,
// `careerHashUnderTheWindowRule`, `windowRuleWitness`) INSIDE A THROW-AWAY VITEST HARNESS THAT WROTE
// THEM TO A FILE – never transcribed from a failure message, which elides a hash with an ellipsis –
// and the harness was deleted before the commit.
//
// ⚠ THE NEW PEEL WAS MUTATION-VERIFIED IN BOTH OF ITS HALVES AND BOTH ARMS ARE RECORDED, the standing
// ledger v75 opened here. Each was applied by a scripted string edit and UNDONE by the inverse edit,
// never `git checkout`, with this file's md5 checked back to pristine after each; the scope is all
// THREE ladder files, which is wider than v75's arm was measured over and is named so the counts are
// comparable to nothing but themselves.
//
//   ARM 10  the six-key destructure replaced by `const preWalls = world` – the six keys left ON every
//           rolled-back shape: **24 RED**, being 6 in tests/coach-travel-edge.test.ts, 8 in
//           tests/coach-travel-edge-mid-schemas.test.ts and 10 in
//           tests/coach-travel-edge-older-schemas.test.ts. EVERY rollback rung in the ladder and not
//           only the new one, because the shape each of them rolls back to would then carry six keys
//           no shipped version ever wrote – exactly the failure the v62 note further down predicted
//           in the abstract, and the same shape v75's own peel arm measured at 13 over one file.
//   ARM 11  the destructure kept, but the `schemaVersion < 76 ? preWalls` branch removed from the
//           chain, so the peel is computed and thrown away: **1 RED**, the new v76 case ALONE, every
//           older rung still green. That is what a rung which guards only its own version looks like,
//           and it is the sharper of the two arms – ARM 10 would have been caught by any of thirteen
//           cases, this one by exactly the case that was written for it.
//
// ⚠ THE FROZEN MAIN CAPTURE IS UNMOVED AND NOT RE-PINNED: 41550 draws / hash `e6b0c709`,
// tests/condition.test.ts, green on this tree. It holds BY CONSTRUCTION – the step takes no draw on
// any stream at all, `createWorld` writes six literals and the migration writes the same six – and
...
//
// ⚠ RE-STAMP, 12.09, THE SECOND OF THE DAY (round 41 P1 – «one market, different baskets»: the
// owner's «на рынке цены для всех сословий одинаковые» and his narrowing of the services corridor to
// «только на нижних тирах»). TWO careers moved and the third is byte-identical, which is the
// slice's own shape read back:
//
//   · 5/0 (25k middle, middle coach, grinder)  – **6 KEYS OF 79**: careerTotals, events,
//     financeWeeks, fundsCents, lastSeasonSummary, seasonHistory. GEAR ONLY: a middle family's
//     composite frames, strings and shoes fell to the uniform rung price, and `middle` is one of the
//     three rungs that KEPT the corridor, so its coaching, facility and medical lines are untouched.
//   · 8/0 (120k wealthy, elite coach, grinder) – **6 KEYS OF 78**, the same six
//     (fundsCents 62f148b9763c -> db57151e9e09). BOTH HALVES: gear as above, plus the elite rung
//     leaving the corridor (1.20-1.30 -> 1.00) on the coach line, the facility line that came out of
//     it and the medical bill that rides the same rung.
//   · 0/1 (8k working, SELF-COACHED, player)  – **ZERO KEYS. Byte-identical, the whole file.**
//     Predicted before the run and it is the diagonal's own identity check: the uniform `composite`
//     band IS the old working band cent for cent, and `self` keeps its corridor. A working family's
//     career does not notice this wave at all.
//
// ⚠⚠ **`rngMain` IS BYTE-IDENTICAL ON ALL THREE** – 1dbff28caca2 · aebc8101d6df · d84bcbf0c481,
// unchanged from the control. That is the stop condition, and here it is a claim about the change's
// shape rather than luck: every price this wave moved is a post-draw arithmetic change, and the one
// band that moved inside a draw (`ECONOMY.gear[*].price`) is read by `pickInt`, which spends exactly
// one `rng()` call whatever its bounds. Nothing in the wave adds, removes or reorders a draw.
//
// ⚠ THE CONTROL IS THIS CHANGE'S OWN BEFORE-STATE, captured on the clean tree BEFORE the first edit
// (CLAUDE.md's control-is-your-own-change-reverted rule; nobody else was committing in this
// worktree), all three arms with EXPLICIT flags and each header read back against the invocation –
// `# preset 5 policy 0`, `# preset 8 policy 0`, `# preset 0 policy 1`. ⚠ AND THE ZSH WORD-SPLIT THIS
// FILE HAS WARNED ABOUT TWICE CAUGHT THE FIRST ATTEMPT AGAIN: a `for pp in "5 0" "8 0" "0 1"` loop
// with `set -- $pp` produced three captures all headed `# preset 0 policy 1`. They were thrown away
// and re-taken one command at a time. **50 constants re-stamped of 75** (48 unique – FROZEN and
// PRE_R28B share both grinder values; all 25 `selfTravelling` cells are untouched). Full table:
// docs/specs/one-market-2026-09.md §5 and docs/rounds/round-41.md's P1 block.
//
// ⚠ RE-STAMP, 12.09, THE THIRD OF THE DAY (round 41 – the owner's answer to the question P1 left
// open: «единая элит-полка вверх - верно»). ONE LEVER, ONE ROW OF ONE CONSTANT:
// `ECONOMY.coach.hourlyRateCents.elite` × 1.25, the midpoint of the wealth corridor P1 took off that
// rung, so the single price everybody pays is what the WEALTHY family used to pay ($96-144 ->
// $120-180 at 12-16, and the same ×1.25 on the two age rows above it). `high` is untouched – his word
// was «элит» – and so is every rung that KEPT the corridor.
//
// ⭐ SO EXACTLY ONE CAREER MOVES, AND WHICH ONE IT IS *IS* THE MEASUREMENT. It is the narrowest move
// this apparatus has recorded on a price change:
//
//   · 8/0 (120k wealthy, ELITE coach, grinder) – **6 KEYS OF 78**: careerTotals, events,
//     financeWeeks, fundsCents (`db57151e9e09` -> `2cb8dde3c124`), lastSeasonSummary, seasonHistory.
//     Money and the money's paper trail, and nothing else: no result, rank, injury or knock key.
//   · 5/0 (25k middle, MIDDLE coach, grinder)  – **ZERO KEYS. Byte-identical, the whole file.**
//   · 0/1 (8k working, SELF-COACHED, player)   – **ZERO KEYS. Byte-identical, the whole file.**
//
// ⚠ THE TWO ZEROS WERE PREDICTED AND THEN MEASURED RATHER THAN ASSUMED, which is this file's own
// rule and the brief's explicit instruction: neither career hires at `elite`, so a change confined to
// the elite row cannot reach them – and had either moved, the rule was to stop and understand it
// before re-freezing anything. Both reproduced.
//
// ⚠⚠ **`rngMain` IS BYTE-IDENTICAL ON ALL THREE** – `aebc8101d6df` (8/0), `1dbff28caca2` (5/0),
// `d84bcbf0c481` (0/1), unchanged from the control. That is the stop condition, and here it is a
// claim about the change's SHAPE rather than luck: the band is read by `pickInt`, which spends
// exactly one `rng()` call whatever its bounds, and the corridor roll still lands on exactly 1.0 at
// this rung. A constant moved through a draw path P1 had already built; nothing adds, removes or
// reorders a draw on any stream.
//
// ⚠ THE CONTROL IS THIS CHANGE'S OWN BEFORE-STATE, captured on the clean tree BEFORE the edit
// (CLAUDE.md's control-is-your-own-change rule), all three arms with EXPLICIT flags **one command at
// a time** – never a `for … set -- $pp` loop, for the reason the block above records – and each
// header read back against the invocation: `# preset 8 policy 0`, `# preset 5 policy 0`,
// `# preset 0 policy 1`. **25 constants re-stamped of 75** (24 unique – FROZEN and PRE_R28B share the
// `eliteGrinder` value), and every one of them is an `eliteGrinder` cell: all 25 `middleGrinder` and
// all 25 `selfTravelling` values in this file reproduce untouched, measured cell by cell rather than
// promised. Full table: docs/specs/one-market-2026-09.md §3's resolution block and
// docs/rounds/round-41.md's P1 block.
//
// =================================================================================================
// ⭐⭐⭐ THE UNION RE-STAMP, 12.09.2026 – ROUND 41 x WAVE 4 MERGED, AND THIS MODULE IS WHERE THE TWO
// BRANCHES COLLIDED. Both re-stamped this file on their own facts; neither is wrong; the union tree
// is a third set of numbers that neither branch ever ran.
// =================================================================================================
//
// ⚠⚠ **`rngMain` WAS VERIFIED BYTE-IDENTICAL ON ALL THREE CAREERS BEFORE A SINGLE CELL WAS STAMPED** –
// `1dbff28caca2` (5/0), `aebc8101d6df` (8/0), `d84bcbf0c481` (0/1), the same three fingerprints both
// branches carry. That is the stop condition and it is what makes everything below a re-stamp rather
// than an investigation: VALUES moved, the stream did not. The fairness law is untouched by this
// merge and no engine file is in it.
//
// ⚠ AND THE ZSH WORD-SPLIT THIS FILE HAS WARNED ABOUT THREE TIMES BIT A FOURTH – on the very read
// that was meant to prove the streams unmoved. The first `rngMain` capture went out through a
// `for pp in "5 0" …; set -- $pp` loop, zsh did not split, `argOf` took its fallbacks and all three
// captures came back headed `# preset 0 policy 1` – three copies of one career wearing three careers'
// names. The header check caught it; the fingerprints alone would not have, because one of the three
// was genuinely 0/1's. Thrown away and re-taken one command at a time, flags written out longhand.
//
// WHAT WAS DONE, COUNTED:
//   · THE LADDER, ITERATIVELY – **24 + 24 pairs over two passes**. The rungs cannot be stamped in one
//     shot because each rung's expected value is a peel of a tree that the previous pass moved; the
//     second pass is what proves the first converged.
//   · THE LIVE CELLS, POSITIONALLY – `FROZEN.middleGrinder` -> `aef1bdc7…` and
//     `FROZEN.selfTravelling` -> `17644471…`, applied by position rather than by string because a
//     blind string replace cannot tell two registers apart when they hold the same value.
//   · `PRE_R28B`'s REGISTER – deliberately deferred to the witness pass, and it is written up in full
//     over the constant itself: the witness career DIED in this merge (`windowRuleWitness(8, 1)` lost
//     `kit-152`), the arm caught it with a sentence, the eighteen-career sweep found four careers that
//     still discriminate, and preset 6 / policy 1 replaces preset 8 / policy 1 under the honest name
//     `highPlayer`. Five cells, all measured on an engine-toggled copy, eight careers / eight
//     agreements, both directions with the control first.
//
// ⚠⚠ ONE LIVE CELL WAS MISSED BY THE POSITIONAL PASS AND IT WENT RED: `FROZEN.eliteGrinder`. It has
// been THE SAME STRING as `PRE_R28B.eliteGrinder` since v73, so deferring `PRE_R28B`'s register
// deferred this cell with it and `careerHash(8, 0)` failed against wave 4's value. **A value shared
// by two registers belongs to both, and is re-stamped when EITHER moves.** Re-stamped to `060bb6c3…`
// with the rest of the witness pass; the note beside the cell dates it.
//
// ⚠ AND THE MERGE LEFT A SECOND ARTEFACT IN THIS FILE, REMOVED IN THE SAME PASS: the union of the two
// branches' note blocks dragged in a SECOND copy of this module's import block, sitting below the
// round-41 notes. It was a strict subset of the block above (round 41 never needed `kitOfferDeadline`,
// `sponsorWindowOpensAt` or the `Offer` types, which `windowRuleWitness` does), so deleting it changes
// nothing that runs – but `vue-tsc` would have called every one of those six names a duplicate
// identifier, and vitest's esbuild transform does not, which is exactly the kind of red that reaches a
// gate after the tests have all gone green.
...
/** ⭐⭐⭐ RE-STAMPED AGAIN 15.09.2026 – ROUND 42 #41 AND #43, AND THE TWO MOVES ARE ATTRIBUTED CELL BY
 *  CELL RATHER THAN LUMPED. The builder ran the per-key protocol FIRST, control = both changes
 *  neutralised in place (md5 back to pristine, never `git checkout --`), then one further arm per
 *  change to say which did what:
 *
 *    · `selfTravelling` (0/1) – 37 keys of 88 moved, and they are **all #43** (the cameo's cadence):
 *      she is the self-coached working family the local sponsor pays, and #41 cannot reach her
 *      because she has no coach to take a cut.
 *    · `PRE_R28B.middlePlayer` (5/1) – exactly **5 keys of 87**, and they are **all #41**, MONEY KEYS
 *      ONLY: `results`, `skills`, `condition`, `bond` and `spirit` are byte-identical, so the
 *      every-cheque cut changed what the family banked and not one match she played. That is the
 *      cleanest evidence this file has ever carried that a prize-split change is a prize-split change.
 *    · 5/0, 8/0 and 6/1 – byte-identical on every rung. `middleGrinder` and `eliteGrinder` never move
 *      here, which is the same asymmetry the 15.09 cameo stamp records one block down.
 *
 *  ⚠ `rngMain` is byte-identical on every cell in every arm and `tests/condition.test.ts` (41550 /
 *  `e6b0c709`) is green – neither change took a die. What moved is money, which is what a balance
 *  change is. */
/** ⭐⭐⭐ RE-STAMPED 15.09.2026 – ROUND 42 #7, AND THIS TIME EVERY CELL MOVED. The domestic track took
 *  the professional window (52 weeks, best 6, crawling) on the owner's own ruling, and a ranking
 *  window is a whole-world fact: 43/87, 32/87, 43/87, 30/86 and 37/86 keys moved across the five
 *  cells.
 *
 *  ⚠⚠ AND THE STOP CONDITION IS THE ONE THAT MATTERS: `rngMain` is BYTE-IDENTICAL on all five, as are
 *  `schemaVersion`, `seed`, `week` and `profile`, and `tests/condition.test.ts`'s capture (41550 /
 *  `e6b0c709`) did not move. The window is a READ – it changes which events she ENTERS, which is a
 *  command and not a roll – so invariant 2 held through a change that moved half the world. That is
 *  the distinction this file exists to make visible, and it has never been this clean.
 *
 *  ⚠ The staff-share guard below was re-aimed in the same pass and its old form is worth remembering:
 *  «no staff share row in any frozen career» was true because a share needed a title or a final
 *  (#41 removed that) and the player arms had no coach (the window-rule witnesses do). A guard whose
 *  REASON dies is not a guard any more, whatever colour it prints. */
/** ⭐⭐ RE-STAMPED 15.09.2026 – v78, AND THIS ONE IS THE NARROW KIND. Three customers rode one bump
 *  (the psychologist's ceiling bonus, the fund chart's purchase rows, the sparring seat's two keys)
 *  and the per-key protocol says exactly four lines move on every cell: `schemaVersion`, and the
 *  three new keys APPEARING at their defaults – `composureBonus` 0, `sparringHired` false,
 *  `sparringRung` 1. Key counts 87→90 / 86→89 / 87→90 / 86→89 / 87→90.
 *
 *  ⭐ `skills` is byte-identical on all five, as are `rngMain`, `potential`, `assets` and the wallet –
 *  and that is the interesting part, because v78 is the first version whose new key ships WITH a live
 *  reader. What makes it inert here is that no frozen career hires the psychologist, which
 *  `walkFrozenCareer` asserts rather than assumes. `PRE_V78` was added and MEASURED: peeling the
 *  three keys reproduces v77 byte for byte on all three ladder cells. */
/** ⭐⭐ RE-STAMPED 16.09.2026 – ROUND 42 #15/#24, AND THE MOVE IS ONE LINE OF THE WORLD. The small-talk
 *  exchange writes richer rows, so `lifeLog` moved on all three careers and NOTHING ELSE DID:
 *  `rngMain`, `skills`, `potential`, `events`, `results`, the wallet and the body are byte-identical.
 *  The ladder proves it twice over – exactly the five rungs that RETAIN `lifeLog` moved (v74…v78) and
 *  both siblings that roll back past v73, where the field arrived, are green without a stamp.
 *
 *  ⚠ The builder declined to re-freeze and was right to: this file's own doctrine is that an agent
 *  bakes in nobody else's unattributed movement. The stamp is the architect's, taken here with the
 *  attribution above, and it is the LAST build bundle of the round – the measurement wave that follows
 *  re-measures rather than re-writes. */
/** ⭐⭐⭐ RE-STAMPED 16.09.2026 – ROUND 42 #34, THE PRICE OF NERVE, and this is the widest move this
 *  file has ever recorded: 33–44 keys of 91 on every one of the nine careers the per-key control
 *  walked. That is what a match-physics change looks like, and every moved key is downstream of a
 *  played match.
 *
 *  ⚠⚠ AND THE STOP CONDITION HELD THROUGH IT: 42 of 91 keys never moved on any career, `rngMain`
 *  byte-identical on all nine, and `tests/condition.test.ts`'s capture (41550 draws / `e6b0c709`)
 *  GREEN AND UNTOUCHED. #34 changes what a drawn point is WORTH; it draws nothing new. Invariant 2
 *  survived a change that moved half of every career in the game – which is the strongest statement
 *  this fixture has been able to make about the discipline it exists to protect.
 *
 *  ⚠ The kid-rank reference constants in `condition` / `injuries` / `planner` moved 89 → 90 in the
 *  same pass and are re-pinned there with the same reasoning: a re-dealt ITF field is a re-dealt
 *  table, and that constant folds the table. Their own captures are asserted BEFORE the constant is
 *  read, and they reproduce byte for byte. */
/** ⭐⭐⭐ RE-STAMPED 16.09.2026 – v79, THE CHEMISTRY WAVE C1 (`docs/specs/the-chemistry-2026-09.md`,
 *  and round 42 #48 riding the same bump). SIXTY-SIX CELLS: sixty-three moved, and the three of the
 *  new `PRE_V79` rung are new. This is a BEHAVIOUR change and not a key append, and the file
 *  distinguishes the two the way v70 taught it to: the rungs are re-anchored to the new world rather
 *  than expected to hold.
 *
 *  WHAT MOVED THEM, AND IT IS ONE THING. v79 appends two keys to `createWorld`'s literal –
 *  `sparringTravels`, then `coachPairs` – and unlike v74…v78 the second ships WITH ITS READER. Every
 *  week a coach is paid for, `accrueCoachPair` writes his row and `coachFactor` takes the accrued
 *  level as a third argument, so the development rate of every career that hires anybody moves and
 *  her build moves with it. `sparringTravels` has no reader at all and reads `false` on all five
 *  careers. `state.ts`'s own v79 block says it in one line: «the frozen careers move, and they move
 *  for a REASON rather than by a key append».
 *
 *  ⭐⭐ AND THE PER-KEY CONTROL SAYS IT IS **ONLY** THAT, which is the whole value of having run it.
 *  The SAME FOUR keys of 90 move on every coached career and no others – `skills`, `peakPhysical`,
 *  `events`, `schemaVersion` – while `coachId`, `results` and `fundsCents` are BYTE-IDENTICAL on all
 *  five. She develops at a different rate; she hires the same man, plays the same matches and banks
 *  the same money. That is the change the spec describes, and it is now the change the measurement
 *  shows, on every cell rather than on one lucky witness.
 *
 *  ⚠⚠ IT IS NOT A STYLE DRAW, AND AN EARLIER READING OF THIS BLOCK SAID IT WAS. C1a asked for `style`
 *  to be drawn per career as well, and the FIRST per-key diff of this wave is what sank it: it found
 *  `coachId` itself moving on three of the five cells, `bestFitCoachAt` was traced to breaking ties
 *  by PRICE on a shuffled shelf, and the bench priced that at a 4.7% coaching discount at every rung
 *  above budget – enough to flip `tests/economy-calibration.test.ts`'s wealthy cell from a $2,970
 *  idle-year burn to break-even, which reverses an owner principle for ~1.5 and ~3 points of two
 *  corner frequencies. It was handed back to him and NOT shipped; `buildCoachRoster`'s own header
 *  carries the three numbers. What ships draws the MANNER and nothing else, so `coachId` holds
 *  everywhere and these constants carry the chemistry alone.
 *
 *  ⭐⭐ THE TWENTY-SIX `selfTravelling` RUNG CELLS HELD, AND THAT IS THE MEASUREMENT. She is the one
 *  career here with no coach, and on her v79 IS a pure key append – ONE key of 90 moves
 *  (`schemaVersion`) and the two new ones appear at `{}` and `false`. Every `PRE_V*` rung's
 *  `selfTravelling` cell is untouched, and the new `PRE_V79` rung returns her old `FROZEN` value
 *  character for character. Only her three LIVE cells moved – `FROZEN`, `PRE_R28B` and
 *  `PRE_NAME_VERA`, the three that carry no peel – and they moved by the bump and the two appends
 *  alone.
 *
 *  ⚠ PER-KEY PROTOCOL RUN FIRST, ON ALL FIVE CAREERS, AND NEVER AFTER THE FACT. Control = this wave
 *  absent, which here is `03a83d36`, the commit immediately before the wave's own three (`53363b5f`,
 *  `df851324`, `f886a98b`) – `PRE_V57`'s own form of «my own change reverted» – built as a
 *  `git archive` into /tmp so no file in the working tree was edited to make it, which also keeps it
 *  honest in a checkout another agent is committing into. Both arms checked in the direction CLAUDE.md
 *  prescribes: the A arm carries no `coachPairs` in `src/` at all, the B arm carries the key AND its
 *  reader, and the A arm reproduced ALL EIGHTY-NINE shipped constants of this file byte for byte
 *  before one number was read off it. `tools/frozen-key-diff.ts`, headers checked against the
 *  invocation on all ten captures: **4 / 4 / 1 / 4 / 4 keys moved of 90** on 5/0, 8/0, 0/1, 6/1
 *  and 5/1.
 *
 *  ⚠ 6/1 IS THE ONE THAT LOOKS WRONG AND IS NOT, and it is worth a sentence because the next reader
 *  will check `coachId` first: hers is `null` at week 156 and her four keys moved anyway. Her
 *  `coachPairs` holds a `high-1` row – she hired him and later let him go – and a rate that moved for
 *  the weeks he was paid for does not come back when he leaves. Read `coachId` at the horizon as «who
 *  is on the books now» and `coachPairs` as «who has ever been».
 *
 *  ⭐⭐ `rngMain` IS BYTE-IDENTICAL ON ALL FIVE, and so are `seed`, `profile`, `potential`, `plan`,
 *  `season`, `week` and `temperament`. `tests/condition.test.ts`'s capture (41550 draws /
 *  `e6b0c709`) is UNMOVED and NOT re-pinned: the chemistry pass takes no MAIN draw at all, and the
 *  roster's ONE new draw – the manner, one `pickInt` a slot – sits on the existing `seed:coaches`
 *  sub-stream after every legacy draw, so coach NAMES and `rateCents` are byte-identical on all five
 *  careers, read straight off `buildCoachRoster` on both arms rather than inferred from a world hash
 *  that does not hold them.
 *
 *  ⚠ APPLIED BY NAME, BLOCK-SCOPED, BY A SCRIPT THAT PRINTED EACH CELL'S OLD AND NEW VALUE, for the
 *  reason the T3 note at the head of this file records: `FROZEN.eliteGrinder` and
 *  `PRE_R28B.eliteGrinder` have been THE SAME STRING since v73 – they are again after this pass – and
...
/** ⭐⭐⭐ RE-STAMPED 20.09.2026 – v85, THE PREGNANCY AND THE RETURN (`world.pregnancy` +
 *  `world.children`, wave 8 T1, docs/plans/life-wave-8-builder-2026-09.md §2 T1). THE SAME ELEVEN
 *  LIVE CELLS MOVED AND EVERY ROLLBACK RUNG HELD – the second bump in a row to take the narrowest
 *  shape available, and this one takes it for a stronger reason than v84's: T1 SHIPS THE SEATS AND
 *  NO WRITER AT ALL (the hazard is T2's, the pause T3's, the birth T4's), so `createWorld`'s `null`
 *  and `[]` are the only values anything on this tree can put in either key, and the live
 *  serialisation gains exactly two literals.
 *
 *  ⭐⭐ PER-KEY DIFF TAKEN FIRST, AS THE BUILDER'S VERY FIRST COMMAND – on the untouched tree at
 *  `c322301c`, before an edit, because re-freezing without it «would have been the exact defect the
 *  file exists to catch». `tools/frozen-key-diff.ts` on all three careers, both arms, headers read
 *  back against the invocation on all six captures (`# preset 5 policy 0`, `# preset 8 policy 0`,
 *  `# preset 0 policy 1` – the zsh word-split this file keeps warning about did NOT fire, because
 *  the three invocations were written out explicitly rather than looped). **Exactly THREE lines moved
 *  on every cell**, of 97 / 97 / 98 after the append (95 / 95 / 96 before it): `schemaVersion` (`44c8031cb036` -> `b4944c6ff08d`), `pregnancy`
 *  APPEARING at `74234e98afe7` and `children` APPEARING at `4f53cda18c2b`. Nothing else moved on any
 *  career – `rngMain`, `results`, `events`, `nextEventId`, `bond`, `spirit`, `loveEpisodes`,
 *  `spiritShock` and the wallet all byte-identical.
 *
 *  ⭐⭐ AND THE IDENTITY WAS MEASURED BEFORE THE CONSTANTS WERE TOUCHED: `careerHashAtSchema(·, ·, 84)`
 *  on 5/0, 8/0 and 0/1 with the bump in the tree reproduced the three shipped v84 `FROZEN` values
 *  CHARACTER FOR CHARACTER – `176e3b5a…`, `89224ccc…`, `a043d425…` – which is the identity `PRE_V85`
 *  now pins: peel the two keys, roll the number back, and the exact v84 serialisation returns.
 *
 *  ⚠⚠ THE ELEVEN RE-STAMPED CELLS are this trio, `PRE_R28B`'s five and `PRE_NAME_VERA`'s three,
 *  applied BY NAME and block-scoped, each rewrite asserting the value it found was the one the probe
 *  read – never a global search-and-replace, which «cannot tell two rungs that happen to share a cell
 *  apart». ⚠ AND THAT TRAP WAS LIVE ON THIS PASS AND CAUGHT: the first capture read
 *  `PRE_R28B.selfTravelling` off `careerHashUnderTheWindowRule` and got `dfda6fba…`, a value nothing
 *  asserts. That cell is asserted against **`careerHash`** – «the career that was never written to did
 *  not move at all», the other half of the round-28 case's claim – so it shares `FROZEN`'s value and
 *  always has. Re-read through the right helper it is `e1c04ceb…`, and the shared-cell relationship
 *  survives the bump intact. The helper is per CELL, not per register.
 *
 *  ⚠ `rngMain` CANNOT HAVE MOVED, BY CONSTRUCTION: the migration writes two literals, `createWorld`
 *  writes two literals, and the wave's own sub-streams (`seed:life:pregnancy:<week>`,
 *  `seed:life:return:<week>`) DO NOT EXIST ON THIS TREE AT ALL. The per-key diff says the same thing
 *  from the other side – `rngMain` byte-identical on all three careers – and the frozen MAIN capture
 *  (41550 / e6b0c709, tests/condition.test.ts) is untouched and NOT re-pinned. */
/** ⭐⭐⭐ RE-STAMPED 20.09.2026 AGAIN – v85's THIRD KEY, `world.comeback` (the week she came back and
 *  the freeze she came back with; the architect's ruling after gate 2 – task T2½ piece 1). THE SAME
 *  ELEVEN LIVE CELLS MOVED AND EVERY ROLLBACK RUNG HELD, `PRE_V85` INCLUDED AND UNCHANGED.
 *
 *  ⚠⚠ THIS IS THE FIRST RE-STAMP IN THIS FILE'S HISTORY THAT IS NOT A VERSION BUMP, AND THE
 *  DIFFERENCE IS WORTH READING BEFORE THE NUMBERS. Every block above re-stamped because
 *  `SAVE_SCHEMA_VERSION` moved and a world's serialisation carries it; here the version is ALREADY 85
 *  and stays 85 – v85 GREW a key rather than v86 arriving, which is licensed by one fact and one
 *  only: **nothing has shipped**. v85 exists on `life/wave-8` and nowhere else, no save in the world
 *  holds it, and §0's «this wave takes 85» stays true. ⚠ The mechanical receipt for that sentence is
 *  in the per-key diff below: `schemaVersion` DID NOT MOVE, so exactly ONE line moved per career
 *  where every earlier rung moved two or more.
 *
 *  ⭐⭐ PER-KEY DIFF TAKEN FIRST, AS THE BUILDER'S VERY FIRST COMMAND – on the untouched tree at
 *  `5b025575`, before an edit, because re-freezing without it «would have been the exact defect the
 *  file exists to catch». `tools/frozen-key-diff.ts` on all three careers, both arms, headers read
 *  back against the invocation on all six captures (`# preset 5 policy 0`, `# preset 8 policy 0`,
 *  `# preset 0 policy 1` – written out explicitly, never looped). **Exactly ONE line moved on every
 *  cell**, of 98 / 98 / 99 keys after the append (97 / 97 / 98 before it, ALL of them byte-identical
 *  across the arms): `comeback` APPEARING at `74234e98afe7`, the sha256 of `JSON.stringify(null)` and
 *  therefore the same twelve characters `pregnancy` landed on at T1 and `spiritShock` at v75 – three
 *  null keys, one hash, which is the cross-check that the tool and the peel agree about what a null
 *  key is. Nothing else moved on any career: `rngMain`, `results`, `events`, `nextEventId`, `bond`,
 *  `spirit`, `loveEpisodes`, `spiritShock`, `pregnancy`, `children` and the wallet all byte-identical.
 *
 *  ⭐⭐ AND THE IDENTITY WAS RE-VERIFIED RATHER THAN ASSUMED, which is the part the architect asked
 *  for by name: `careerHashAtSchema(·, ·, 84)` on 5/0, 8/0 and 0/1 with the third key in the tree
 *  reproduced the three shipped v84 `FROZEN` values CHARACTER FOR CHARACTER – `176e3b5a…`,
 *  `89224ccc…`, `a043d425…` – so `PRE_V85` below is untouched by this key and holds the same three
 *  constants it has held since T1. A 156-week frozen career never reaches 23, no writer for
 *  `comeback` exists on this tree, and the prediction is now a measurement.
 *
 *  ⚠⚠ THE ELEVEN RE-STAMPED CELLS are this trio, `PRE_R28B`'s five and `PRE_NAME_VERA`'s three,
 *  applied BY NAME and block-scoped, each rewrite asserting the value it found was the one the probe
 *  read – never a global search-and-replace, which «cannot tell two rungs that happen to share a cell
 *  apart». ⚠ THE SHARED CELL IS LIVE AGAIN AND WAS READ THROUGH THE RIGHT HELPER: `PRE_R28B`'s three
 *  are asserted against `careerHashUnderTheWindowRule` for the two grinders and against **`careerHash`**
 *  for `selfTravelling`, so that cell shares `FROZEN`'s value and always has – captured that way, and
 *  the relationship survives this append intact (`cd5f9e0a…`, `3b9d1bed…`, `af93ff21…` on both).
 *
 *  ⚠ `rngMain` CANNOT HAVE MOVED, BY CONSTRUCTION: the migration writes one more literal,
 *  `createWorld` writes one more literal, and `seed:life:return:<week>` – the stream T6's writer will
 *  eventually use – DOES NOT EXIST ON THIS TREE AT ALL. The per-key diff says the same thing from the
 *  other side, and the frozen MAIN capture (41550 / e6b0c709, tests/condition.test.ts) is untouched
 *  and NOT re-pinned. */
// =================================================================================================
// ⭐⭐⭐ RE-STAMPED 22.09.2026 – v86, THE DYNASTY: A PURE KEY APPEND, ELEVEN LIVE CELLS MOVED AND
// EVERY ROLLBACK RUNG HELD, `PRE_V85` INCLUDED AND UNCHANGED.
// =================================================================================================
//
// WHAT MOVED THEM. `createWorld` writes one more key – `dynasty`, the record of whose daughter she is
// (docs/specs/the-dynasty-2026-09.md §3, his go of 22.09) – and it writes `null` on every career in
// this file, because `openCareer` hands it no fifth argument and never will. A key in the literal is
// a key in `JSON.stringify`, so every register that hashes the LIVE world re-stamps: `FROZEN`'s three,
// `PRE_R28B`'s five (its trio shares these cells, the shared-cell rule) and `PRE_NAME_VERA`'s three.
//
// ⚠⚠ AND THE RECEIPT IS STRONGER THAN A PER-KEY DIFF, WHICH IS WHY NO SECOND WORKTREE WAS BUILT.
// `careerHashAtSchema(·, ·, 85)` was RUN on this tree and returned `cd5f9e0a…`, `3b9d1bed…`,
// `af93ff21…` – the three values `FROZEN` held on `main` before this wave, character for character.
// The peel drops exactly one key, so reproducing the old serialisation exactly can only happen if
// NOTHING ELSE IN THE WORLD MOVED A BYTE – `rngMain`, `results`, `events`, the wallet, the body, all of
// it. A two-arm diff proves «these keys differ»; this proves «these keys and no others», over the
// whole object, in one run. `PRE_V86` below holds those three values and its block carries the same
// argument at length.
//
// ⚠ `PRE_V85` WAS RE-VERIFIED RATHER THAN ASSUMED, the discipline that rung's own block asks for:
// `careerHashAtSchema(·, ·, 84)` on this tree returns `176e3b5a…`, `89224ccc…`, `a043d425…`, its
// three shipped constants. Every rung below it holds for the same reason – the peel gained one line
// at its HEAD and not one line anywhere else.
//
...
export const FROZEN = {
   ...
   *
   *  ⚠⚠ RE-STAMPED 12.09.2026 BY THE UNION MERGE (round 41 x wave 4) – AND IT WAS THE ONE LIVE CELL
   *  THE MERGE'S OWN RE-STAMP MISSED, which is worth recording beside the value rather than in the
   *  block at the head of the file. `careerHash(8, 0)` reads `060bb6c3f3b2…` on the union tree; this
   *  cell still held `f0b701d6…`, wave 4's value, and the case at
   *  tests/coach-travel-edge.test.ts:202 was RED on it. ⚠ WHY IT WAS MISSED IS STRUCTURAL AND NOT
   *  CARELESSNESS: this cell and `PRE_R28B.eliteGrinder` have been THE SAME STRING since v73, so a
   *  per-cell re-stamp that deliberately defers `PRE_R28B`'s register defers this one with it. A
   *  shared value belongs to two registers and is re-stamped when EITHER moves. ⚠ AND IT IS A STAMP,
 ...
 *  one is written. */
/** ⭐⭐⭐ THE SAME THREE CAREERS AS THEY HASHED UNDER v76 – the identity the v77 re-freeze rests on,
 *  and the wave-5 pattern (`PRE_V76` directly below) repeated one version up. These three values are
 *  the VERBATIM v76 `FROZEN` constants, character for character, and they are reproduced here by
 *  peeling exactly what v77 appended – the world key `spotlightHabituation`, AND the four fields the
 *  version puts on every `LoveEpisode` ROW (`publicWeek`, `publicWrong`, `airedMetWeek`,
 *  `airedEndedWeek`) – and rolling the number back. So «nothing but v77's own fields moved» is an
 *  identity over the whole serialisation rather than a claim about a diff.
 *
 *  ⭐⭐⭐ AND THIS RUNG IS THE ONE THING THAT PROVES **THE NESTED PEEL**, which is what makes it worth
 *  more than the twelve flat rungs below it. Every earlier version added keys to the WORLD, so its
 *  rung is restored by object rest and the claim is «the destructure lists the right names». v77's
 *  four fields are one level down, inside the entries of `world.loveEpisodes`, and the only career in
 *  this file that holds an entry at all is `eliteGrinder` – ONE row, hers, from week 137. Her value
 *  here reproducing the old `FROZEN.eliteGrinder` CHARACTER FOR CHARACTER is the proof that the map
 *  restores each row's six v74 fields in their original order; on the other two careers the nested
 *  half of the peel is a no-op over an empty list and says nothing at all. ⚠ RUN, not assumed:
 *  `careerHashAtSchema(8, 0, 76)` was computed on this tree by a throw-away vitest harness that wrote
 *  it to a file, and it returned `c861a2c9…` – the string that stood over `FROZEN.eliteGrinder` from
 *  the v76 wave until this one re-stamped it.
 *
 ...
 *  about whether a leak inside 156 weeks would move these hashes. ⚠⚠ And ruling D of 14.09 measured
 *  that it CANNOT: peak `fameAt` across 156 weeks is 0.00 / 0.00 / 3.43 / 1.85 / 3.25 on the five
 *  cells against a proposed news bar of 30, so this corpus is an order of magnitude away from the
 *  states wave 6 adds, on every career at every week. No behavioural claim in this wave may cite a
 *  frozen constant as its evidence.
 *
 *  ⚠⚠ THE FROZEN MAIN CAPTURE IS UNMOVED AND NOT RE-PINNED: 41550 draws / hash `e6b0c709`,
 *  tests/condition.test.ts, green on this tree. It held BY CONSTRUCTION – this step takes no draw on
 *  any stream at all: `createWorld` writes one literal `0`, the one `loveEpisodes.push` writes four
 *  more literals at a row's birth, and the migration writes the same five.
 *
 *  ⚠ PER-KEY DIFF FIRST, as this file's protocol demands and never after the fact. Control = this
 *  branch's head `f588dfd9` with a clean tree; `tools/frozen-key-diff.ts` on all five careers,
 *  headers checked on all ten captures:
 *
 *    · 5/0 (25k middle, grinder)              – **2 lines of 87**: `schemaVersion`, and
 *      `spotlightHabituation` APPEARING with `5feceb66ffc8`. 85 keys byte-identical.
 *    · 8/0 (120k wealthy, elite, grinder)     – **3 lines of 86**: those two, and `loveEpisodes`
 *      moving `d447eb28e502` -> `8c5884c15bcd` as her ONE row gains the four fields. 83 byte-identical.
 *    · 0/1 (8k working, self-coached, PLAYER) – **2 lines of 88**, the same two. 86 byte-identical.
 *    · 6/1 (25k middle, high coach, PLAYER)   – **2 lines of 86**, the same two. 84 byte-identical.
 *    · 5/1 (25k middle, middle coach, PLAYER) – **2 lines of 87**, the same two. 85 byte-identical.
 *
 *  `rngMain`, `results`, `events`, `nextEventId`, `bond`, `spirit`, `temperament`, `lifeLog`,
 *  `spiritShock`, `wallsLean`, `wallsFlipped` and the wallet are untouched on all five – which is the
 *  load-bearing half, and the one this rung then restates as an identity instead of as a diff. ⭐ The
 *  THIRD line on `eliteGrinder` is this version's whole shape in one number: a career that holds an
 *  attachment is the only career a row-level schema move can reach, and there is exactly one of them.
 *
 *  ⚠ ELEVEN LIVE CONSTANTS MOVED, the same eleven v76 cost and for the same arithmetic: `careerHash`,
 *  `careerHashUnderTheWindowRule` and `careerHashUnderTheOldName` hash the LIVE world with NO peel, so
 *  a bumped `schemaVersion` moves every value they produce. Every `PRE_V*` rung HELD, RUN and not
 *  assumed: `careerHashAtSchema` at 75, 74, 73 and 71 returned `PRE_V76`, `PRE_V75`, `PRE_V74` and
 *  `PRE_V72` character for character before a constant was touched. ⚠⚠ AND THE RE-STAMP WAS APPLIED
 *  **BY NAME**, block-scoped, with each cell's old and new value printed – because
 ...
 *  v77 LIVE hashes, character for character, and they are reproduced by peeling exactly what v78
 *  appends – the three world keys `composureBonus`, `sparringHired`, `sparringRung`, and
 *  `entries` off every `OwnedAsset` row – and rolling the number back. So «nothing but those four
 *  things moved» is an IDENTITY over the whole serialisation rather than a claim about a diff.
 *
 *  ⚠⚠ RUN AND NOT ASSUMED, and this time the control was a separate WORKTREE rather than a memory of
 *  the old values. `careerHashAtSchema(p, po, 77)` was captured on a clean checkout of `697180be`
 *  BEFORE the first edit and re-captured here afterwards; all three cells returned the same string
 *  both times, and so did rung 76 (`PRE_V77` above reproduces unchanged), which is what says the peel
 *  was inserted at the right end of the chain rather than merely somewhere in it.
 *
 *  ⚠ PER-KEY DIFF FIRST, as this file's protocol demands and never after the fact.
 *  `tools/frozen-key-diff.ts` on all five cells, both arms, headers and key counts verified on all
 *  ten captures. **EXACTLY FOUR LINES MOVE ON EVERY CELL, and they are the same four everywhere:**
 *
 *    · `schemaVersion`   `a88a7902cb4e` -> `349c41201b62`
 *    · `composureBonus`  APPEARING with `5feceb66ffc8` – the sha256 of `JSON.stringify(0)`
 *    · `sparringHired`   APPEARING with `fcbcf165908d` – the sha256 of `false`
 *    · `sparringRung`    APPEARING with `6b86b273ff34` – the sha256 of `1`
 *
 *  Key counts 87->90 (5/0), 86->89 (0/1), 87->90 (8/0), 86->89 (5/1), 87->90 (6/1). Every other key
 *  is byte-identical on every cell, `rngMain`, `results`, `events`, `potential`, `fundsCents` and
 *  `assets` among them. ⚠ Every hex string in this block was COPIED OUT OF THE CAPTURE FILES, never
 *  typed from memory – this file's v75 block records the one time that rule was broken.
 *
 *  ⭐⭐ AND `skills` IS BYTE-IDENTICAL ON ALL FIVE, WHICH IS THE ONE LINE THAT MATTERS HERE. v74, v75,
 *  v76 and v77 could each say «the move is inert» cheaply, because each shipped a seat with NO READER
 *  AT ALL. v78 cannot: `composureBonus` ships WITH its reader, and `growWeek` really does ask
 *  `composureCeilingOf` for composure's ceiling on every week of every career. What makes it inert
 *  HERE is a property of these careers rather than of the tree – the bonus can only move while the
 *  psychologist works the `'coolhead'` focus, `walkFrozenCareer` asserts `psychologistHired === false`
 *  on every one of them, so the bonus is 0, the effective ceiling is `potential.composure` to the bit
 *  and her build is untouched. That is measured above and not argued: her `skills` hash did not move.
 *  ⚠ A policy that started hiring him would go red at that assertion first, with a sentence, rather
 *  than drifting five hashes for a reason nobody could name.
 *
 *  ⚠ `assets` IS BYTE-IDENTICAL TOO, AND FOR A DULLER REASON: it is `[]` on all five careers – 156
 *  weeks, she is 16.6, and no bench policy buys a shop rung – so v78's nested peel maps an empty
 *  array and proves nothing about itself here. THE CORPUS THAT WITNESSES IT IS THE GOLDEN SAVES,
 *  where fourteen fixtures carry 75 real asset rows; `tests/round42-v78-schema.test.ts` carries the
 *  crafted, mutation-verified one. Said plainly because v77's nested peel had exactly one witness in
 *  this file and a reader could reasonably expect v78's to have some.
 *
 *  ⚠⚠ THE FROZEN MAIN CAPTURE IS UNMOVED AND NOT RE-PINNED: 41550 draws / hash `e6b0c709`,
 *  tests/condition.test.ts, green on this tree. It holds BY CONSTRUCTION – nothing this bundle adds
 *  takes a draw on any stream: `createWorld` writes three literals, `buyAsset` writes an object of
 *  numbers it already had, the migration writes literals and an empty array, and
 *  `composureBonusAfterWeek` is a constant, a clamp and a compare.
 *
 *  ⚠ ELEVEN LIVE CONSTANTS MOVED, the same eleven v76 and v77 cost and for the same arithmetic:
 *  `careerHash`, `careerHashUnderTheWindowRule` and `careerHashUnderTheOldName` hash the LIVE world
 *  with NO peel, so a bumped `schemaVersion` alone moves every value they produce. **THEY ARE NOT
 *  RE-STAMPED HERE.** The bundle's brief left the stamp to the architect, so `FROZEN`, `PRE_R28B` and
 *  `PRE_NAME_VERA` stand at their v77 values and the three `coach-travel-edge*` files are RED ON
 *  PURPOSE, exactly as round 42 #7 left them a day earlier. The measured replacements are in that
 *  bundle's report; whoever applies them applies them BY NAME, block-scoped, with each cell's old and
 ...
 *  on (wave F1, `world.form`), and the FIRST RUNG IN THIS FILE WHOSE THREE CELLS ARE NOT THE PREVIOUS
 *  VERSION'S `FROZEN` CONSTANTS. That difference is the wave, so it is stated before anything else.
 *
 *  ⚠⚠ A PEEL DROPS A KEY; IT DOES NOT REPLAY A CAREER WITHOUT THE MECHANIC. Every rung from
 *  `PRE_V50` up has been able to claim «these are the previous version's live constants, character
 *  for character», because every version since v50 appended keys nothing in a 156-week career could
 *  read – or, at v79, keys only a coached career could (its `selfTravelling` cell is the last
 *  verbatim value in this file, and its own note says so). `world.form` is different in kind: its one
 *  reader is `composureEff` at `MatchPlayer` build time, EVERY frozen career plays matches and has
 *  matchless weeks, so every one of them stepped on court at a different composure and played
 *  different matches. Rolling the version back to 79 and dropping the key therefore returns the
 *  SHAPE a v79 save has, over a career v79 could not have produced. That is what this rung asserts
 *  and the only thing it can honestly assert.
 *
 *  ⭐ WHAT IT STILL PROVES, and it is the reason to keep the rung rather than retire it: that `form`
 *  is the ONLY key the version appended, and that it is the LAST key of `createWorld`'s literal. Drop
 *  it, roll the number, and the serialisation is byte-for-byte a v79 serialisation of this world – so
 *  a v80 save that lost the key would still load, and nothing else about the shape moved.
 *
 *  ⚠⚠ PER-KEY PROTOCOL RUN FIRST, ON ALL THREE CAREERS, AND THE CONTROL WAS THIS TREE WITH THE
 *  WAVE'S CHANGE NEUTRALISED **IN PLACE** – an early return at the head of `accrueFormWeek` leaving
 *  `form` at 0, removed by a reverse edit afterwards and never by `git checkout --`. The reader was
 *  confirmed PRESENT on the B arm (`formComposureDelta` has three call sites in `world/player.ts`)
 *  and INERT on the A arm, which is CLAUDE.md's null-arm check taken in both directions.
 *  `tools/frozen-key-diff.ts --policy 1` on the three canonical cells:
 *
 *    · 5/0 (25k middle, grinder)          – **3 keys of 94**: `form` itself, `events` and
 *      `nextEventId`. ⭐ AND THAT PAIR IS THE COACH'S EYE AND NOTHING ELSE – one `info` row, no
 *      money row, no scoreline, no ranking: her form moved and never far enough to turn a match. The
 *      narrowest cell in this file, and the one that says the mechanic is a mood rather than a lever.
 *    · 8/0 (120k wealthy, elite, grinder) – **34 keys of 94**, `coachId` and `spirit` among them.
 *    · 0/1 (8k working, self-coached, PLAYER) – **33 keys of 94**.
 *      Both wide cells are the same causal chain and it is the one the wave claims: a changed
 *      composure turns a match, a turned match moves the results ledger, the ledger moves the
 *      ranking, the ranking moves which events she can enter, and an entry is money. Nothing here is
 *      a new writer – `git grep 'world.form ='` finds ONE line, in `world/form.ts`.
 *
 *  ⭐⭐ `rngMain` IS BYTE-IDENTICAL ON ALL THREE CAREERS – `d84bcbf0c481` (0/1), `1dbff28caca2`
 *  (5/0), `aebc8101d6df` (8/0), the three canonical fingerprints this file has carried for fourteen
 *  waves, reproduced exactly. That is invariant 2's STOP condition met per key rather than asserted:
 *  the wave adds ZERO draws on any stream (O4), `seed:form:<week>` stays reserved and unused, and the
 *  frozen MAIN capture (41550 / e6b0c709) is untouched by construction.
 *
 *  ⚠ THE SPARRING SEAT REACHES NONE OF THIS. `sparringHired` is false in every frozen career and
 *  `walkFrozenCareer` never hires into the seat, so wave F2 is invisible here by construction – the
 ...
 *  kind: **its three cells are the verbatim v80 `FROZEN` constants**, character for character, which
 *  the ladder has not been able to say since v78.
 *
 *  ⚠⚠ AND THE REASON IS NOT THAT THE ROUND WAS SMALL. Round 44 grows the small-talk catalogue from 8
 *  situations to 51 and stamps a delivery frame on every `'small-talk'` row – a behaviour change and
 *  a schema key in one wave. What makes the rung an identity anyway is a CALENDAR fact, and it is
 *  worth stating because it will not hold for ever: **a frozen career is 156 weeks from its own start,
 *  so the girl in it never leaves `school`** – and every one of the 43 corpus situations declares
 *  `after-school`, `college`, `independent` and no `school` at all. The pool these five careers draw
 *  from did not grow by one row. The day a frozen walk is lengthened past school, this rung stops
 *  being an identity and the 84 rungs under it re-anchor with it.
 *
 *  ⚠ SO EXACTLY ONE THING MOVED IN THEIR SERIALISATION AND IT IS THE NEW KEY: `frame` on each
 *  small-talk `lifeLog` row. The per-key diff at the head of this file names `lifeLog` and
 *  `schemaVersion` on all five cells and nothing else, and the peel drops exactly that key – so
 *  `careerHashAtSchema(p, q, 80)` reproduces the career the file already held.
 *
 *  ⚠ WHICH IS ALSO WHY 84 OF THE 95 CELLS IN THIS FILE WERE ALREADY CORRECT. Only the eleven that
 ...
 *  the rung above it managed and this one does not (round 42 #51, ruled 17.09;
 *  `docs/specs/the-coachs-raise-2026-09.md`). **Its three cells are NOT the verbatim v81 `FROZEN`
 *  constants**, and every `PRE_V*` rung beneath it is re-stamped with it.
 *
 *  ⚠⚠ WHY, IN ONE SENTENCE: v82 changes what a hired coach COSTS. The shipped till re-derived his
 *  hourly rate every week from her age band and her ranking; v82 bills the LABOUR that was agreed at
 *  hire plus the court at today's price. Dropping `coachDeal` gives back the v81 SHAPE and can never
 *  give back the v81 CAREER, because the money the family spent is different.
 *
 *  ⚠⚠ AND THE BUILD PREDICTED AN IDENTITY HERE AND WAS WRONG BY ONE WEEK, which is recorded because
 *  the per-key diff is the only reason it was caught. The prediction: «156 weeks ends inside
 *  `coachAgeBand` 0 with no WTA rank, so the agreed labour equals the market's, the ceiling leaves no
 *  room and no ask can fire». `ageAtWeek` returns WHOLE YEARS – she is 16 from week 104 and turns 17
 *  at week 156 exactly – and `walkFrozenCareer`'s final tick runs AT 156. So the last week of every
 *  coached frozen career is billed across an age-band step (where v81 re-drew the man from a dearer
 *  row and v82 does not), and 156 is also `3 x 52`, so the annual ask fires on the same week.
 *
 *  ⚠ THE PER-KEY DIFF, taken FIRST and with the control being this change reverted IN PLACE:
 *    · 5/0 (25k middle, grinder)           – 7 of 93: `careerTotals`, `coachDeal`, `events`,
 *                                            `financeWeeks`, `fundsCents`, `nextEventId`, `schemaVersion`
 *    · 8/0 (120k wealthy, elite, grinder)  – the same 7 of 93
 *    · 6/1 (25k middle, high, player)      – 6 of 92: the same, without `nextEventId`
 *    · 5/1 (25k middle, middle, player)    – 6 of 93: the same, without `nextEventId`
 ...
 *      else, and it could not be otherwise: there is no coach, so there is no fee to fix.
 *
 *  ⭐ `rngMain` IS BYTE-IDENTICAL ON ALL FIVE and reproduces the three canonical fingerprints this
 *  ladder has carried for eighteen waves – 5/0 `1dbff28caca2` · 8/0 `aebc8101d6df` · 0/1
 *  `d84bcbf0c481`. The ask is a weighted mean over state the tick has already written and the fee is
 ...
 *  wedding, wave 7 – T1; `docs/plans/life-wave-7-builder-2026-09.md` §0). **Its three cells ARE the
 *  verbatim v82 `FROZEN` constants**, measured rather than promised, and every `PRE_V*` rung beneath
 *  it HELD – the peel drops the two new row fields ahead of all its older peels, so not one older
 *  constant moved by a character.
 *
 *  ⚠⚠ WHY AN IDENTITY, IN ONE SENTENCE: v83 appends `latchedWeek` and `partnerName` to every
 *  `LoveEpisode` row, both null at birth, and a frozen career is 156 weeks from its own start – the
 *  girl never reaches 23, so no wedding hazard, no `'engaged'` beat, no name and no cost can fire
 *  here, and T1 in any case ships NO WRITER for either value (the hazard is T2, the latch and the
 *  naming are T3). Dropping two null fields and rolling the number returns the exact v82
 *  serialisation.
 *
 *  ⚠ THE PER-KEY DIFF, taken FIRST, control = this branch's head `1b9eb2b1` with a clean tree:
 *    · 5/0, 0/1, 6/1, 5/1                  – **1 key moved: `schemaVersion`** (`a46e37632fa6` ->
 *                                            `bbb965ab0c80`), everything else byte-identical
 *    · 8/0 (120k wealthy, elite, grinder)  – 2: `schemaVersion`, and `loveEpisodes`
 *                                            (`8c5884c15bcd` -> `444ca2a71789`) as its one row
 *                                            (`p:137`, v77's own witness cell) gains the two null
 *                                            fields – the key append itself, never a behaviour
 *
 *  ⭐ `rngMain` IS BYTE-IDENTICAL ON ALL FIVE and reproduces the three canonical fingerprints this
 *  ladder has carried – 5/0 `1dbff28caca2` · 8/0 `aebc8101d6df` · 0/1 `d84bcbf0c481`. Zero MAIN
 ...
 *  pregnancy and the return – `world.pregnancy` + `world.children`, wave 8 T1,
 *  `docs/plans/life-wave-8-builder-2026-09.md` §2 T1). **Its three cells ARE the verbatim v84
 *  `FROZEN` constants**, measured rather than promised: `careerHashAtSchema(·, ·, 84)` was RUN on
 *  this tree with the bump in it and reproduced all three character for character – a pure key
 *  append, exactly as §0 predicted – and every `PRE_V*` rung beneath it HELD, because the peel drops
 *  the two new world keys ahead of all its older peels.
 *
 *  ⚠⚠ WHY AN IDENTITY, IN ONE SENTENCE, AND IT IS THE STRONGER OF THE TWO REASONS AVAILABLE: T1
 *  SHIPS THE SEATS AND NO WRITER AT ALL – the hazard that sets `pregnancy` is T2's, the pause T3's,
 *  the `children.push` T4's – so `createWorld`'s `null` and `[]` are the only values anything on this
 *  tree can put there, however long a walk runs. Dropping two literal keys and rolling the number
 *  returns the exact v84 serialisation. ⚠ The career-shaped reason holds too and is named here
 *  because it is the one that will OUTLIVE this tree: `walkFrozenCareer` stops at 156 weeks, age
 *  16.6, the hazard's window opens at 24 and marriage is the door – so this rung stays an identity
 *  after T2..T4 land, unless `FREEZE_WEEKS` grows past the latch.
 *
 *  ⭐ `rngMain` CANNOT HAVE MOVED, BY CONSTRUCTION: the migration writes two literals, `createWorld`
 *  writes two literals, and the wave's own draws (`seed:life:pregnancy:<week>`,
 *  `seed:life:return:<week>`) are purpose-scoped sub-streams that do not exist on the T1 tree at all
 *  – so the frozen MAIN capture (41550 / e6b0c709) is untouched and the three canonical fingerprints
 *  (5/0 `1dbff28caca2` · 8/0 `aebc8101d6df` · 0/1 `d84bcbf0c481`) stand un-re-pinned.
 *
 *  ⚠ PER-KEY DIFF TAKEN FIRST, as this file's protocol demands and never after the fact – the
 *  builder's FIRST command, on the untouched tree at `c322301c`, headers checked on all six
 *  captures. **Exactly THREE lines moved on every cell** of 97 / 97 / 98 after the append (95 / 95 / 96 before
 *  it, and 94 / 94 / 95 byte-identical across the two arms): `schemaVersion`
 *  (`44c8031cb036` -> `b4944c6ff08d`), `pregnancy` APPEARING at `74234e98afe7` (the sha256 of
 *  `JSON.stringify(null)` – the same string v75's `spiritShock` landed on, which is a small
 *  cross-check that the tool and the peel agree about what a null key hashes to) and `children`
 *  APPEARING at `4f53cda18c2b` (the sha256 of `[]`). Nothing else moved on any career – `rngMain`,
 *  `results`, `events`, `nextEventId`, `bond`, `spirit`, `loveEpisodes`, `spiritShock` and the wallet
 *  all byte-identical. `spiritShock`'s widening to `'breakup' | 'postpartum'` is type-level and
 *  cannot appear in a hash at all, which the unmoved `spiritShock` line is the receipt for.
 *
 *  ⭐⭐⭐ UNCHANGED BY v85's THIRD KEY, AND RE-VERIFIED RATHER THAN LEFT ALONE (20.09, after gate 2 –
 *  `world.comeback`, task T2½ piece 1). Three cells that do not move are the easiest thing in this
 *  file to leave unmeasured, so they were MEASURED: `careerHashAtSchema(·, ·, 84)` was RUN on the
 *  tree carrying the third key and reproduced all three values below character for character. The
 *  peel now drops THREE keys ahead of `prologueTrace` instead of two, `createWorld` writes a third
 *  `null`, and rolling the number back still returns the exact v84 serialisation – a pure key append
 *  for the second time in one version. ⚠ The prediction that licensed it – a 156-week frozen career
 ...
 *  CHARACTER, AND THEY ARE THE VALUES `FROZEN` HELD BEFORE WAVE 10 (v86, the dynasty;
 *  docs/specs/the-dynasty-2026-09.md §3).
 *
 *  ⚠⚠ THIS RUNG IS THE RECEIPT FOR «A PURE KEY APPEND», AND IT IS A MEASUREMENT RATHER THAN A CLAIM.
 *  `createWorld` now writes one more key – `dynasty`, `null` on every career this file walks, because
 *  `openCareer` hands it no fifth argument and never will – so `JSON.stringify` of the LIVE world
 *  gained a key and all eleven live cells re-stamped. `careerHashAtSchema(·, ·, 85)` was then RUN on
 *  this tree and returned `cd5f9e0a…`, `3b9d1bed…`, `af93ff21…` – the three values `FROZEN` held on
 *  `main` – which can only happen if the peel removed the ONE new key and nothing else in the world
 *  moved a byte. That is a stronger statement than a per-key diff between two trees, and it needed no
 *  second worktree to make.
 *
 *  ⚠ AND `PRE_V85` DIRECTLY ABOVE IS UNTOUCHED, re-verified the same way rather than assumed:
 *  `careerHashAtSchema(·, ·, 84)` on this tree returns `176e3b5a…`, `89224ccc…`, `a043d425…`, its
 *  three shipped constants. Every rollback rung below it holds for the same reason – the peel gained
 *  a line at its head and none of the others changed.
 *
 *  ⚠ THE PREDICTION THAT LICENSED IT is `dynasty`'s own law in world.ts: `createWorld`'s literal is
...
/** ⭐⭐⭐ THE SAME THREE CAREERS AS THEY HASHED UNDER v87 – AND THE FIRST RUNG IN THIS LADDER WHOSE
 *  PEEL DOES NOTHING AT ALL, because there is nothing to peel. v88 (the parting, wave 12 – T1) is
 *  THREE UNION WIDENINGS AND NOT ONE NEW KEY: `SpiritShockKind` + `'divorce'`, `MilestoneType` +
 *  `'divorce'`, `LifeBeatKind` + `'divorced'`. None of the three is a FIELD, so the serialised world
 *  is the same shape it was at v87 and `careerHashAtSchema`'s ternary chain gains no rung – its tail
 *  (`schemaVersion < 87 ? preWeight : world`) answers 87 and 88 alike, and the only thing that
 *  differs between the two hashes is the version number the last line stamps in.
 *
 *  ⭐⭐ SO THIS REGISTER IS NOT A RECONSTRUCTION – IT IS THE LIVE v87 WORLD WITH ITS OWN NUMBER ON
 *  IT, and that makes the identity below the strongest form this file can produce. Every rung above
 *  proves «the peel removed exactly the new keys»; this one proves «the wave added none», which is
 *  a claim about the WAVE rather than about the peel. A single persisted field slipped in anywhere
 *  – a `divorcedWeeks` list, a flag on `LoveEpisode`, a counter – and all three cells below go red
 *  at once with no comment able to explain it away.
 *
 *  ⚠ MEASURED BEFORE THIS BLOCK WAS WRITTEN, as the protocol demands and never after the fact:
 *  `careerHashAtSchema(·, ·, 87)` on this tree with the bump in it returns all three values below
 *  CHARACTER FOR CHARACTER – they are what `FROZEN` held on `main` before this wave. The per-key
 *  control was captured as the builder's FIRST command on the untouched tree, all three careers,
 *  headers read back against the invocation (`# preset 5 policy 0`, `# preset 8 policy 0`,
 *  `# preset 0 policy 1` – the zsh word-split trap this file warns about fired on the first attempt
 *  and produced three copies of preset 0, which the headers caught and which is why they are read).
 *  **Exactly ONE line moves on the golden fixture and on every career: `schemaVersion`.**
 *
 *  ⚠ AND THE ELEVEN LIVE CELLS RE-STAMP ANYWAY, which is not a contradiction but the arithmetic:
 *  the version number is inside the hash, so `FROZEN`'s three, `PRE_R28B`'s five (its trio shares
 *  these cells, the shared-cell rule) and `PRE_NAME_VERA`'s three all move while the world under
 ...
 *  `dynasty.motherCareer.collegeTitles` – and it is NESTED TWO LEVELS DEEP INSIDE A NULLABLE RECORD
 *  that **every frozen career carries as `null`**. So the field exists and the serialised world still
 *  gains no key, `careerHashAtSchema`'s ternary chain gains no rung – its tail
 *  (`schemaVersion < 87 ? preWeight : world`) answers 87, 88 and 89 alike – and the only thing that
 *  differs between the two hashes is the version number the last line stamps in.
 *
 *  ⭐⭐ THE CLAIM THIS RUNG MAKES IS THEREFORE STRONGER THAN «THE PEEL WORKS» AND NARROWER THAN v88's.
 *  v88's block could say «the wave added no field»; this one says «the wave added a field and put it
 *  somewhere a frozen career cannot reach», which is a claim about WHERE a schema move landed. Move
 *  `collegeTitles` up to the world, or give a frozen career a dynasty, and all three cells below go
 *  red at once with no comment able to explain it away.
 *
 *  ⚠ MEASURED BEFORE THIS BLOCK WAS WRITTEN, as the protocol demands and never after the fact. The
 *  per-key control was captured as the builder's FIRST command on the untouched tree, all three
 *  careers, headers read back against the invocation (`# preset 5 policy 0`, `# preset 8 policy 0`,
 *  `# preset 0 policy 1`). On the tree WITH the bump: **exactly ONE key of 102/102/103 moves on every
 *  career, `schemaVersion`** – none added, none removed, key ORDER identical, `rngMain` byte-identical
 *  – and `careerHashAtSchema(·, ·, 88)` returns the three values below CHARACTER FOR CHARACTER, which
 *  are what `FROZEN` held on this branch before the bump. The golden fixture says the same thing from
 *  the other side: `migrateSave(v88.json)` moves ONE key of 104.
 *
 *  ⚠ AND THE ELEVEN LIVE CELLS RE-STAMP ANYWAY, which is not a contradiction but the arithmetic – the
 *  version number is inside the hash, so `FROZEN`'s three, `PRE_R28B`'s five (its trio shares these
 ...
 *  AN IDENTITY ON ONE CAREER AND A RE-ANCHORING ON THE OTHER TWO. Every `PRE_V*` block above this
 *  one opens by saying «these are the VERBATIM constants of the version below, character for
 *  character». That sentence is TRUE HERE OF EXACTLY ONE CELL, and saying so plainly is the whole
 *  value of the rung (16.09.2026, the chemistry wave C1 – `docs/specs/the-chemistry-2026-09.md`).
 *
 *  ⭐⭐ `selfTravelling` IS THE VERBATIM v78 VALUE – `14d1195a…`, the old `FROZEN.selfTravelling`,
 *  character for character. She is the one career in this file with NO COACH, so for her the whole of
 *  v79 really is a key append: the per-key control says `schemaVersion` moved and `coachPairs` and
 *  `sparringTravels` appeared at `{}` and `false`, and NOTHING ELSE OF 90 KEYS moved. Peel the two,
 *  roll the number back, and her serialisation returns exactly – `rngMain`, `results`, `events`, the
 *  wallet, the body, `skills`, all of it.
 *
 *  ⚠⚠ `middleGrinder` AND `eliteGrinder` ARE RE-ANCHORED TO THE NEW WORLD AND ARE **NOT** THE OLD v78
 *  VALUES, which is the second kind of move this file distinguishes and has distinguished since v70's
 *  `drawnFirstRounds`: «not a schema field, but a change to what a career IS». Both of them hire a
 *  coach, so both of them now have a relationship – `accrueCoachPair` writes a row every week he is
 *  paid for and `coachFactor` takes the level as its third argument – and a peel that drops
 *  `coachPairs` cannot undo a development rate the remaining shape has already spent 156 weeks
 *  living at. No amount of care would have kept those two green, and pretending otherwise by
 *  weakening the peel is the one thing this rung must not do.
 *
 *  ⭐ SO THE RUNG STILL DOES ITS JOB, in the form v70 left it: a later wave that moves one of these
 *  careers through anything but `SAVE_SCHEMA_VERSION` goes red HERE beside a red freeze, and the
 *  PAIR is what names the kind of change. And it now carries one extra sentence the ladder has never
 *  been able to say – a career with no coach must come back BYTE FOR BYTE. If `selfTravelling` ever
 *  goes red on this rung, something has reached a family that hired nobody.
 *
 *  ⚠ PER-KEY DIFF TAKEN FIRST, as this file's protocol demands and never after the fact – the full
 *  table and the control's provenance are in `careerHashAtSchema`'s own v79 block. In short: five
 *  careers, `tools/frozen-key-diff.ts` on both arms, headers checked on all ten captures, 4 / 4 / 1 /
 *  4 / 4 keys moved of 90, and `coachId`, `results`, `fundsCents`, `rngMain`, `seed`, `profile`,
 *  `potential`, `plan`, `season`, `week` and `temperament` byte-identical on every one. What re-anchors
 *  the two grinders is the CHEMISTRY and only the chemistry: four keys, and they are her build, not
 *  her coach and not her wallet.
 *
 *  ⚠ NO STYLE DRAW IS INVOLVED, and an earlier draft of this rung said one was. C1a's per-career
 *  `style` shuffle was measured, found to move `coachId` itself and to discount coaching by 4.7% at
 *  every rung above budget, and was handed back to the owner rather than shipped. `buildCoachRoster`
 *  draws the manner alone, so every frozen career hires the same man she always did.
 *
 *  ⚠⚠ THE FROZEN MAIN CAPTURE IS UNMOVED AND NOT RE-PINNED: 41550 draws / hash `e6b0c709`,
 *  tests/condition.test.ts. It holds BY CONSTRUCTION – the chemistry pass takes no MAIN draw at all
 *  (`affinityFor` and `nextChemistryPhase` each build a `rngFromSeed` generator and throw it away),
 ...
 *  the VERBATIM v75 `FROZEN` constants, character for character, and they are reproduced here by
 *  peeling exactly the SIX keys v76 appended (`psychologistHired`, `psychologistRung`,
 *  `psychologistFocus`, `psychologistFocusSeason`, `wallsLean`, `wallsFlipped`) and rolling the number
 *  back – so «nothing but those six keys moved» is an identity over the whole serialisation rather
 *  than a claim about a diff.
 *
 *  ⚠ SIX KEYS WHERE EVERY RUNG SINCE v72 HAS PEELED ONE, and the precedent for a GROUP is `PRE_V72`'s
 *  own three: they arrived in one append, in `createWorld`'s order, and object rest preserves the
 *  relative order of everything it keeps – so one destructure restores the v75 shape exactly. The six
 *  are two different kinds of thing riding one bump (four staff-seat keys, two walls keys) and that
 *  costs this rung nothing: what it asserts is the SHAPE, and a shape does not care why its keys
 *  arrived together.
 *
 *  ⚠ THE PEEL REMOVES SIX KEYS AND NEVER A VALUE, and on this rung that is a fact about THE TREE
 *  rather than about these careers – v74's and v75's own note, verbatim in its situation. T1 ships six
 *  seats, the migration and **no writer at all**: `hirePsychologist` is T2, `setPsychologistFocus` is
 *  T3, and the weekly leaning pass with its flip hazard is T7. So every career here carries `false`,
 *  the middle rung `1`, two `null`s, `{open: 0, reg: 0}` and `{open: false, reg: false}` not because
 *  the walk is too short to reach a hire or a flip but because there is nothing in the tree that could
 *  write another value – which is also why this rung says nothing yet about whether a hire or a season
 *  of walls inside 156 weeks would move these hashes. ⚠ AND THE SEAT COULD NOT REACH THEM ANYWAY, for
 *  a reason worth writing down now so T2 is not blamed for it later: the seat is gated on
 *  `activeLadderOf(world) === 'wta'`, these careers stop at week 156 with the kid at 16.6, and no
 *  frozen policy hires anybody. The WALLS are the half to watch – their weekly pass is deterministic
 *  and runs on the bond band, which every career here has – so T7 is where that question gets
 *  answered, by this rung reproducing or not.
 *
 *  ⚠⚠ THE FROZEN MAIN CAPTURE IS UNMOVED AND NOT RE-PINNED: 41550 draws / hash `e6b0c709`,
 *  tests/condition.test.ts, green on this tree. It held BY CONSTRUCTION – this step takes no draw on
 *  any stream at all: `createWorld` writes six literals and the migration writes the same six.
 *
 *  ⚠ PER-KEY DIFF FIRST, as this file's protocol demands and never after the fact. Control = this
 *  branch's head `ccc83cbc` with a clean tree; `tools/frozen-key-diff.ts` on **all five careers this
 *  file names** – the two window-rule witnesses included, which no earlier schema move ever captured –
 *  headers checked on all ten captures:
 *
 *    · 5/0 (25k middle, grinder)              – **7 lines of 85**: `schemaVersion`, and the six keys
 *      APPEARING. 78 keys byte-identical.
 ...
 *  from inside a diff: `psychologistHired` `fcbcf165908d`, `psychologistRung` `6b86b273ff34`,
 *  `psychologistFocus` and `psychologistFocusSeason` `74234e98afe7` (the sha256 of
 *  `JSON.stringify(null)` – v75's own, met here twice), `wallsLean` `8c393bb9b0db`, `wallsFlipped`
 *  `1c396dc5e2a9`; `schemaVersion` goes `f369cb89fc62` -> `f74efabef12e`. `rngMain`, `results`,
 *  `events`, `nextEventId`, `bond`, `spirit`, `temperament`, `condition`, `careerTotals`,
 *  `financeWeeks`, `lifeLog`, `loveEpisodes`, `spiritShock` and the wallet are untouched on all five –
 *  which is the load-bearing half, and the one this rung then restates as an identity instead of as a
 *  diff. ⚠ `temperament` byte-identical is this wave's own sentence and not a routine line: identity
 *  is IMMUTABLE (who-she-is §2a), the walls are expression over it, and a schema move that had
 *  quietly moved a girl's birth temperament would be the gravest finding the wave could produce.
 *
 *  ⚠⚠ ELEVEN LIVE CONSTANTS MOVED, NOT THE NINE v74 AND v75 EACH COST, and the two extra are not this
 *  wave's doing. `careerHash`, `careerHashUnderTheWindowRule` and `careerHashUnderTheOldName` hash the
 *  LIVE world with NO peel at all, so a bumped `schemaVersion` moves every value they produce by
 *  arithmetic – and the 12.09 union merge restored two more careers to `PRE_R28B` (`highPlayer` and
 *  `middlePlayer`, the window-rule witnesses) after v75 had already been stamped. Nine plus those two
 *  is eleven. The rule is unchanged and is the one wave-4 §0.4 states correctly: a NEW key PEELS at
 *  its own rung, so the LADDER holds – and it did, measured rather than assumed: `careerHashAtSchema`
 *  at 74, 73 and 71 were RUN and returned `PRE_V75`, `PRE_V74` and `PRE_V72` character for character
 *  before a constant was touched.
 *
 *  ⚠ AND `PRE_R28B` IS STILL BYTE-IDENTICAL TO `FROZEN` ON ITS THREE ORIGINAL CAREERS, which v75's
 *  block flagged and did not fix. It is still true here (the three values are the same strings), it is
 *  still nothing T1 may repair under its own brief, and it is still worth a sentence: the round-28
 *  #17-b case catches a moved career on those three but no longer distinguishes the WINDOW rule from
 *  the LETTER rule there. ⭐ What it DOES distinguish is `highPlayer` and `middlePlayer`, which is
...
/** ⭐⭐ RE-STAMPED 16.09.2026 – v79, THE CHEMISTRY WAVE C1 (`docs/specs/the-chemistry-2026-09.md`).
 *  ALL THREE CELLS MOVED, for the reason `PRE_R28B`'s stamp gives one block down:
 *  `careerHashUnderTheOldName` hashes the LIVE world with no peel, so the schema bump and the two
 *  appended keys move it on every career before the chemistry is even asked. What this constant
 *  asserts is UNCHANGED and was re-measured rather than assumed – restore `kidName: 'Vera'` before
...
/** ⚠⚠⚠ RE-AIMED AND RE-STAMPED 12.09.2026 BY THE UNION MERGE (round 41 x wave 4) – ALL FIVE CELLS
 *  MOVED, ONE CAREER WAS REPLACED, AND THE ARM WORKED EXACTLY AS T1b BUILT IT TO. This is the T1b
 *  block above happening a second time, nine days later, to a different career and for the same
 *  reason – and this time it took ONE test run and a sentence instead of nine silent days.
 *
 *  ⭐⭐ THE WITNESS DIED AGAIN, AND THE ARM SAID SO. `windowRuleWitness(8, 1)` – the career T1b
 *  restored – no longer holds `kit-152`: her inbox at the horizon is `kit-47` and `kit-151`, both on
 *  slot 0, where the two rules agree by arithmetic. The case went RED on its OWN witness precondition
 *  («the witness letter exists and landed MID-window (slot 1…): expected undefined to be 152») rather
 *  than on a hash, which is the entire point of the re-aim: a career that stops discriminating is now
 *  a red test with a sentence, not a green one proving nothing.
 *
 *  ⭐⭐ WHAT MOVED HER, BISECTED RATHER THAN GUESSED – and it is NOT the junior ad letter. Four trees
 *  were walked with the same probe and the same 156 weeks, then the 59 commits between them binary-
 *  searched on one question («does 8/1 hold kit-152?»), six runs:
 *
 *    · merge base `3dda7566`                – kit-152 present, slot 1, open. `kidRankWta` 310.
 *    · wave 4 `af6007da`                    – IDENTICAL inbox. `kidRankWta` 310. The witness is alive
 *      on the wave-4 side, so the private life did not touch her.
 *    · round 41 `bdab3c64`                  – kit-152 ABSENT. `kidRankWta` 539.
 *    · the union tree (this one)            – kit-152 ABSENT. `kidRankWta` 539. Same as round 41.
 *
 *  **THE COMMIT IS `bea3d58e` «feat(economy): round 41 P1 – one market, different baskets».** Its
 *  parent `b07f01ba` still holds the letter; it does not. So the mover is a PRICE change, reaching a
 *  career through her wallet: this is a `PLAYER`-policy career whose entry rules read the money, her
 *  accepted entries in the sponsor window went from ONE (`entry-3-w157-w35-0`) to FIVE (weeks 151-155),
 *  her standing at the window moved with them, and `windowLadder(standing)` / `offerChanceFor(standing,
 *  tier)` are read fresh on every week of it – so a different rung cleared and the slot-1 letter was
 *  never raised. ⚠ THE A1 JUNIOR AD LETTER (`043d49e1`, items 15+27) IS EXONERATED BY MEASUREMENT, not
 *  by argument: it lands earlier in the bisect with the witness still intact, and 8/1 receives no `ad`
 *  letter at all on any of the four trees. (0/1, 3/1, 5/1 and 6/1 do – `ad-drinks-138/149/151` – which
 *  is that item arriving, in careers that are not this one.)
 *
 *  ⚠⚠ AND THE REASON NOBODY SAW IT COMING IS IN THIS FILE, three blocks up. Round 41's own P1 re-stamp
 *  took its per-key diff on THREE careers (5/0, 8/0, 0/1) and its elite-shelf re-stamp on the same
 *  three. Wave 4's T5, one branch across, took its diff on FIVE – «the three this file freezes plus 5/1
 *  and 8/1, because `windowRuleWitness` reaches the PLAYER-policy pair». **The witness lived on the two
 *  careers the round-41 control did not include.** That is the same shape as `ac2b5de3` in the T1b block
 *  above: a guard whose subject is chosen by her RANKING is re-rolled by every balance change. The rule
 *  that follows is one line: a per-key control that is narrower than the set of careers this file
 *  ASSERTS is not a control for this file.
 *
 *  ⭐ SO THE REPAIR IS (a) RESTORE AGAIN, AND THE THIRD OUTCOME IS REFUTED FOR THE SECOND TIME. All
 *  eighteen preset/policy pairs were walked to week 156 on the union tree and printed. **FOUR hold a
 *  mid-window kit letter where the two rules disagree – 3/1, 4/1, 5/1 and 6/1**, every one of them
 *  under `POLICIES[1]`, which is the same policy-shaped finding T1b recorded (it found five: 3/1, 4/1,
 *  5/1, 6/1, 8/1 – the identical set minus the career that just died). Nothing was invented and nothing
 *  hand-tuned: the replacement is an existing bench pair walked by this file's own `walkFrozenCareer`,
 *  whose assertions it passes unchanged.
 *
 *    · `highPlayer` = preset 6 / policy 1 (25k middle · HIGH coach · player) REPLACES `elitePlayer`.
 *      Its witness is `kit-152` again, and the numbers are character for character the ones the old
 *      witness carried: week 152, slot 1, written deadline **156**, window deadline **155**, `open` at
 *      the horizon instead of expired. It is the highest coach rung still discriminating, which is why
 *      it and not 3/1 or 4/1 – it keeps the pair one wealth/coach rung apart, as the old pair was.
 *    · `middlePlayer` = preset 5 / policy 1 is UNCHANGED AS A SUBJECT and its letter did not move:
 *      `kit-100`, week 100, slot 1, expired at the horizon on `decidedWeek` 105 where the window rule
 *      would have lapsed it on 104. Only its HASH moved, with the rest of the tree. ⭐ It is now a
 *      DOUBLE witness – it also holds `kit-152` (slot 1) and `kit-153` (slot 2, deadline 157 by the
 *      letter and 155 by the window) – which is extra strength and not a substitute for the pair.
 *
 *  ⚠ THE CELL WAS RENAMED RATHER THAN RE-POINTED IN SILENCE. `elitePlayer` named preset 8; preset 6 is
 *  a middle family with a high coach, so keeping the name would have made the constant lie about its
 *  own career – exactly the rot this block exists to stop. ⚠ AND 8/1 WAS NOT KEPT AS A FOURTH
 *  «this career did not move» PIN, deliberately: a non-discriminating hash pin is what this case was
 *  for nine days, and adding another would be re-committing the defect under a new name.
 *
 *  ⭐⭐ EVERY CONSTANT HERE IS A **MEASURED** VALUE FROM A TREE THAT ACTUALLY RAN THE OLD RULE, by the
 *  identical protocol T1b used. `src/engine/offers.ts:965` (`raiseKitOffers`) and `:1059`
 *  (`raiseKitRenewal`) – still those two lines, still the only two `d1cec974` changed – were put back
 *  to `sponsorWindowClosesAt(week)` in a COPY of this mid-merge tree (a merge in progress is not a
 *  commit, so it cannot be a worktree; the copy was verified byte-identical on `offers.ts` before the
 *  edit). **BOTH DIRECTIONS, CONTROL FIRST**: the B arm reproduced all eight careers' live hashes
 *  before the toggle, the A arm was taken, the two lines were put back, and the B arm re-ran
 *  BYTE-IDENTICAL to its first capture – which is CLAUDE.md's own «restoring B is not `git checkout --
 *  src`» hazard closed by re-measurement rather than by assumption. **EIGHT CAREERS, EIGHT AGREEMENTS
 *  between the engine-toggled hash and this file's reconstruction**:
 *
 *      5/0 aef1bdc7…  8/0 060bb6c3…  0/1 17644471…  8/1 9a5cf713…   (no mid-window letter – unchanged)
 *      6/1 aa076754…  5/1 b2549083…  3/1 78a40ca1…  4/1 edb8db6c…   (the four that discriminate)
 *
 *  ⚠ 8/1 APPEARING IN THE UNCHANGED ROW IS THE VERDICT TAKEN FROM THE OTHER SIDE, and it is worth more
 *  than the sweep that found it: with the ruling un-shipped in the ENGINE, the career that used to
 *  witness it hashes exactly what it hashes with the ruling in. That is «this career no longer
 *  discriminates» measured by the engine rather than read off an inbox listing.
 *
 *  ⚠ THE THIRD KIT-WRITING SITE WAS LEFT ALONE AND IT IS NAMED HERE SO NOBODY RE-DISCOVERS IT:
 *  `offers.ts:2491` in `apparelBondLetter` also writes `kitOfferDeadline(week)`. It is round 39 #17,
 *  born eleven rounds AFTER the ruling, so a pre-ruling tree never had it; the toggle is the two
 *  documented lines and nothing else. The eight agreements are what say the choice was right – a
 *  letter written by that site in any of these careers would have shown up as a disagreement.
 *
 *  ⚠ NO ENGINE FILE IN THIS TREE WAS TOUCHED. The toggle lived in the copy, which is discarded; `git
 *  status` on this merge shows fixtures and tests only. And every value below was RUN through the
 *  exported helpers into a file and compared character for character, never transcribed from a failure
 *  message, which elides a hash with an ellipsis.
 *
 *  ⚠ THE FROZEN MAIN CAPTURE IS UNMOVED AND NOT RE-PINNED: 41550 draws / hash `e6b0c709`,
 *  tests/condition.test.ts, RE-RUN GREEN on the union tree rather than assumed – nothing engine-side
...
/** ⭐⭐ RE-STAMPED 16.09.2026 – v79, THE CHEMISTRY WAVE C1 (`docs/specs/the-chemistry-2026-09.md`).
 *  ALL FIVE CELLS MOVED, and they had to: these five hash the LIVE world with no peel at all, so a
 *  bumped `schemaVersion` and two appended keys move every value they produce by arithmetic before
 *  any behaviour is considered. The behaviour moved too on the four with a coach behind them –
 *  `coachPairs` ships with its reader, so a relationship accrues and the development rate follows it.
 *  WHICH COACH they hire did NOT move: `coachId` is byte-identical on all five in the per-key control,
 *  and so are `results` and `fundsCents`. `selfTravelling` is the exception in kind rather than in
 *  outcome: her cell moved by the schema bump and the two appends ALONE (one key of 90), which is why
 *  the `PRE_V79` rung returns her old value verbatim while nothing here can. The full table and the
 *  control's provenance are over `FROZEN`.
 *
 *  ⚠ THE TWO WITNESSES STILL WITNESS, checked rather than assumed on the re-stamped tree: `kit-152`
 *  still lands on `highPlayer`'s window week 48 at slot 1 with deadlines 156 / 155 and still `open` at
 *  the horizon, `kit-100` still lapses on `middlePlayer` at 105 / 104, and `underTheWindowRule` still
...
export const PRE_R28B = {
   ...
   *  rewrites: `kit-152` lands on window week 48 (slot 1), so its deadline is 156 by the letter and
...
function walkFrozenCareer(
  ...
  // ⚠⚠ RE-AIMED BY ROUND 42 #41 AND #7, AND THE OLD FORM WAS TRUE FOR A REASON THAT HAS GONE. The
  // sentence above still names it: «a share needs a wta cheque, which the grinders never win inside
  // 156 weeks, or a coach, which the player arm does not have». #41 removed the first leg – the coach
  // now takes 10% of EVERY cheque at every finish, not only a title or a final – and #7's rolling
  // window then let the coached PLAYER arms (the window-rule witnesses, 6/1 and 5/1) collect prize
  // money inside the same 156 weeks where they had collected none. So the guard holds where its
  // reasoning still holds – a career with NO COACH cannot pay a coach – and where there is a coach it
  // asserts the honest modern thing instead: every share row is an expense in a week she was paid.
  const coached = world.coachId !== null
  if (!coached) {
    expect(
      world.events.some((e) => e.text.includes("share of the prize money") && e.type === 'expense'),
      'no staff share row in a career with no coach to pay',
    ).toBe(false)
  } else {
    // ⚠ THE EXPENSE ROWS ONLY, which is what the old guard filtered on too: the same sentence also
    // appears on `info` rows, where the feed TELLS her about the cut rather than charging it. A guard
    // that forgot the filter called a narrative line an accounting defect – caught here, by its own
    // first run.
    for (const row of world.events.filter(
      (e) => e.text.includes("share of the prize money") && e.type === 'expense',
    )) {
      expect((row.amountCents ?? 0) < 0, 'a charged staff share leaves the family, in cents').toBe(true)
    }
  }
...
export function careerHashAtSchema(presetIndex: number, policyIndex: number, schemaVersion: number): string {
  ...
  // ⚠⚠ RE-AIMED FOR v76 (the psychologist's year, wave 5 – T1), NOT WEAKENED, AND IT PEELS AHEAD OF
  // ALL TWELVE BELOW because these six are the newest keys and the peel order is reverse order of
  // arrival. SIX IN ONE DESTRUCTURE, which this file has done once before and for the identical
  // reason: they arrived in ONE append, in `createWorld`'s own order, and object rest preserves the
  // relative order of everything it keeps – the wave-1 three (`spirit`/`bond`/`temperament`) are the
  // precedent, six lines down.
  //
  // ⚠ ALL SIX ARE WRITTEN BY `createWorld` ITSELF, so every career here carries every one of them –
  // and each carries its OWN identity value (`false`, the middle rung `1`, two `null`s, a zero
  // leaning and an unflipped pair), which on this rung is a property of THE TREE rather than of these
  // careers, exactly as v75's `null` was: T1 ships six seats, the migration and NO WRITER AT ALL
  // (`hirePsychologist` is T2, the focus command T3, the weekly leaning pass and the flip hazard are
  // T7), so nothing on this tree could put another value in any of them however long the walk ran.
  // The peel removes six keys and never a value, and `PRE_V76` asserts it.
  //
  // ⚠ MEASURED BEFORE THIS LINE WAS WRITTEN, as the protocol demands and never after the fact:
  // `tools/frozen-key-diff.ts` on ALL FIVE careers this file now names (the two window-rule witnesses
  // included, which no earlier schema move captured), control = this branch's head `ccc83cbc` with a
  // clean tree, headers checked on all ten captures. Exactly SEVEN lines move on each – `schemaVersion`
  // (`f369cb89fc62` -> `f74efabef12e`) and the six keys APPEARING, with the same six hashes on every
  // career: `psychologistHired` `fcbcf165908d`, `psychologistRung` `6b86b273ff34`, `psychologistFocus`
  // and `psychologistFocusSeason` `74234e98afe7` (the sha256 of `JSON.stringify(null)`, v75's own),
  // `wallsLean` `8c393bb9b0db`, `wallsFlipped` `1c396dc5e2a9`. Everything else is byte-identical –
  // 78 / 77 / 79 / 77 / 78 other keys, `rngMain`, `results`, `events`, `nextEventId`, `spirit`, `bond`,
  // `temperament`, `lifeLog`, `loveEpisodes`, `spiritShock` and the wallet among them.
  // ⚠⚠⚠ RE-AIMED FOR v77 (the spotlight, wave 6 – T1), NOT WEAKENED, AND IT IS **THE PROTOCOL'S FIRST
  // NESTED PEEL**. Every re-aim above this one peels a KEY OFF THE WORLD with object rest; v77 appends
  // one such key (`spotlightHabituation`, the newest, so it peels ahead of all thirteen below) and FOUR
  // fields that are not on the world at all – `publicWeek`, `publicWrong`, `airedMetWeek` and
  // `airedEndedWeek` live on the ENTRIES of `world.loveEpisodes`. Object rest cannot reach inside an
  // array, so the peel MAPS the list and rests each row. That is the whole of the new mechanism, and it
  // is written out rather than folded into the destructure above so the next reader sees it.
  //
  // ⭐⭐ THE KEY ORDER SURVIVES IT, WHICH IS THE ONLY PROPERTY THAT MATTERS HERE. Object rest preserves
  // the relative order of everything it KEEPS, at both levels: each row comes back with its six v74
  // fields in their original order, and `{ ...preHabituation, loveEpisodes: … }` re-uses the key's
  // EXISTING slot rather than appending a new one (an overwrite in an object literal does not move a
  // key). So `JSON.stringify` produces precisely the v76 serialisation, which is what the identities
  // below assert byte for byte rather than argue.
  //
  // ⚠⚠ AND IT IS A NO-OP ON FOUR OF THE FIVE CAREERS AND REAL ON EXACTLY ONE, measured 14.09 rather
  // than assumed: `FROZEN.middleGrinder`, `FROZEN.selfTravelling`, `PRE_R28B.highPlayer` and
  // `PRE_R28B.middlePlayer` all carry `loveEpisodes: []` (hash `4f53cda18c2b`, the sha256 of `[]`), and
  // `FROZEN.eliteGrinder` carries ONE row – `{id:'p:137', sinceWeek:137, endedWeek:null, knownWeek:137,
  // wants:'open', partnerId:'p:137'}`, the attachment wave 3's hazard gives her at 137. She is the ONE
  // witness this peel has, on all five careers, at every rung from 74 up. ⚠ And she is the cell this
  // file has already lost once to a positional re-stamp (the T3 note at the head, «ONE LIVE CELL WAS
  // MISSED BY THE POSITIONAL PASS»), so v77's re-stamp was applied BY NAME.
  //
  // ⚠ MEASURED BEFORE THIS LINE WAS WRITTEN, as the protocol demands and never after the fact:
  // `tools/frozen-key-diff.ts` on all five careers, control captured on this branch's head
  // `f588dfd9` with a clean tree, headers checked on all ten captures. Exactly TWO lines move on the
  // four empty-list careers – `schemaVersion` (`f74efabef12e` -> `a88a7902cb4e`) and
  // `spotlightHabituation` APPEARING with `5feceb66ffc8` (the sha256 of `JSON.stringify(0)`) – and
  // THREE on `eliteGrinder`, whose `loveEpisodes` moves `d447eb28e502` -> `8c5884c15bcd` as its one
  // row gains the four fields. 85 / 83 / 86 / 84 / 85 other keys byte-identical on the five careers:
  // `rngMain`, `results`, `events`, `nextEventId`, `spirit`, `bond`, `temperament`, `lifeLog`,
  // `spiritShock`, the walls pair and the wallet among them. ⚠ Every hex string in this block was
  // COPIED OUT OF THE CAPTURE FILE, never typed from memory – this file's v75 block records the one
  // time that rule was broken and an MD5 was written where a sha256 belonged.
  // ⚠⚠ RE-AIMED FOR v78 (16.09 – round 42 #35's `composureBonus`, round 42 #45's sparring pair and
  // round 41 #22's `OwnedAsset.entries`), NOT WEAKENED, AND IT PEELS AHEAD OF ALL FOURTEEN BELOW
  // because these are the newest keys and the peel order is reverse order of arrival. THREE world
  // keys in one destructure – the wave-1 three and the wave-5 six are the precedent – and then the
  // SECOND NESTED PEEL this file has ever carried, over `assets` rather than `loveEpisodes`.
  //
  // ⭐⭐ AND THE NESTED HALF IS THE ONE THAT ACTUALLY BITES HERE, which is the difference from v77.
  // That version's list is EMPTY on four of the five careers, so its nested map was very nearly a
  // no-op on this file's corpus. `assets` is empty on ALL FIVE – a frozen career is 156 weeks long,
  // she is 16.6, and no bench policy buys a shop rung – so the map runs zero times and the peel is
  // exact by construction rather than by luck. Kept anyway, and kept in the right order, because the
  // invariant that keeps the next wave out of trouble is «peel every key appended after the version
  // being rolled back to», and an exception argued from today's careers is how that invariant rots
  // (the `brandStrengthSeed` block twelve rungs down makes the same argument for a key no live
  // career can EVER write).
  //
  // ⚠⚠ `composureBonus` IS THE FIRST KEY THIS LADDER HAS PEELED THAT SHIPS WITH A LIVE READER, and
  // that is worth a sentence because every rung from v72 down peeled a seat nothing could touch.
  // `growWeek` really does read it, through `composureCeilingOf`. What makes the peel exact anyway
  // is that the bonus can only move on a week the psychologist works the `'coolhead'` focus, and
  // `walkFrozenCareer` ASSERTS `psychologistHired === false` on every one of these worlds – so the
  // value is 0, the effective ceiling is `potential.composure` to the bit, and her build is what it
  // was. That assertion is the guard; if a policy ever started hiring him, this rung would go red
  // with a sentence beside it instead of five hashes drifting for a reason nobody could name.
  //
  // ⚠ MEASURED BEFORE THIS LINE WAS WRITTEN, as this file's protocol demands and never after the
  // fact: the whole-world hash at rung 77 was captured on the CLEAN tree at `697180be` before any
  // edit, and re-captured here. All three ladder cells reproduce byte for byte – see `PRE_V78`.
  // ⚠⚠⚠ RE-AIMED FOR v79 (16.09 – the chemistry wave C1's `coachPairs` and round 42 #48's
  // `sparringTravels`; `docs/specs/the-chemistry-2026-09.md`), NOT WEAKENED, AND IT PEELS AHEAD OF
  // ALL FIFTEEN BELOW because these two are the newest keys and the peel order is reverse order of
  // arrival. They arrived in ONE append – `createWorld`'s literal lists `sparringTravels` and then
  // `coachPairs` – so ONE destructure restores the v78 shape, on the precedent of the wave-1 three,
  // the wave-5 six and v78's own three: object rest preserves the relative order of everything it
  // keeps, whichever order the destructure names them in.
  //
  // ⚠⚠⚠ AND THIS IS THE FIRST RUNG IN THE LADDER WHOSE VERSION CHANGES WHAT A CAREER IS, so the peel
  // CANNOT return the old constants on a career that has a coach – and it is not meant to. v78's
  // `composureBonus`, one rung up, was the first key here that shipped with a live reader, and what
  // made it inert was that no frozen career hires the psychologist. `coachPairs` ships with a reader
  // that fires EVERY WEEK a coach is paid for: `accrueCoachPair` writes the row and `coachFactor`
  // takes the level as its third argument, so her development rate moves and her build moves with
  // it. v70's `drawnFirstRounds` block, further down this chain, is the precedent and states the
  // rule – «not a schema field, but a change to what a career IS» – and every rollback identity
  // below is RE-ANCHORED to the new world rather than expected to hold.
  //
  // ⭐⭐ WHAT THE PEEL STILL BUYS IS THE PAIR THAT SAYS WHICH KIND OF CHANGE THIS WAS, and it has one
  // witness. On the ONE career here with no coach – `selfTravelling`, 8k self-coached – the whole of
  // v79 IS a key append: the per-key diff is `schemaVersion` plus the two keys appearing, `{}` and
  // `false`, and NOTHING ELSE of 90 moves. So this rung is an exact identity on her and reproduces
  // her old `FROZEN` value character for character, while the two coached grinders are re-anchored.
  //
  // ⚠ MEASURED BEFORE THIS LINE WAS WRITTEN, as this file's protocol demands and never after the
  // fact. Control = this wave absent, which here is the tree at `03a83d36`, the commit immediately
  // before the wave's own three – `PRE_V57`'s own form of «my own change reverted» – taken as a
  // `git archive` into /tmp so nothing in the working tree was touched to build it. The A arm was
  // checked to carry no `coachPairs` in `src/` at all and the B arm to carry both the key and its
  // reader; and the A arm reproduced ALL EIGHTY-NINE shipped constants of this file byte for byte
  // before one number was read off it. `tools/frozen-key-diff.ts` on all five careers, headers
  // checked against the invocation on all ten captures:
  //
  //     · 5/0  middleGrinder     4 of 90 moved, 86 held, 2 new
  //     · 8/0  eliteGrinder      4 of 90 moved, 86 held, 2 new
  //     · 0/1  selfTravelling    1 of 90 moved, 89 held, 2 new – `schemaVersion` ALONE
  //     · 6/1  highPlayer        4 of 90 moved, 86 held, 2 new
  //     · 5/1  middlePlayer      4 of 90 moved, 86 held, 2 new
  //
  // ⭐⭐ AND THE FOUR ARE THE SAME FOUR EVERY TIME – `events`, `peakPhysical`, `skills`,
  // `schemaVersion` – which is what makes this table worth more than its row count. `coachId`,
  // `results` and `fundsCents` are BYTE-IDENTICAL on all five: she hires the same man, plays the same
  // matches and banks the same money, and only her BUILD moves. A development rate that moved and
  // nothing else is exactly what the spec describes this version as doing.
  //
  // ⭐⭐ `rngMain` IS BYTE-IDENTICAL ON ALL FIVE, and so are `seed`, `profile`, `potential`, `plan`,
  // `season`, `week` and `temperament`. Zero MAIN draws were added: `affinityFor` and
  // `nextChemistryPhase` each build their own `rngFromSeed` generator from a string and throw it
  // away, and the roster's ONE new draw – the manner, one `pickInt` a slot – sits on the EXISTING
  // `seed:coaches` sub-stream AFTER every legacy draw, so coach NAMES and `rateCents` are
  // byte-identical on all five careers, read straight off `buildCoachRoster` on both arms rather
  // than inferred from a world hash that does not contain them.
  //
  // ⚠⚠ THERE IS NO STYLE DRAW HERE, AND THIS BLOCK SAID THERE WAS UNTIL THE WAVE WAS CUT BACK – worth
  // recording, because the per-key protocol is what caught it and that is the argument for the
  // protocol. C1a also asked for `style` to be drawn per career; the first run of this table showed
  // `coachId` itself moving on three of the five cells (`middle-1` -> `middle-2` on the 25k careers,
  // `null` -> `high-2` on 6/1), which traced to `bestFitCoachAt` breaking ties by PRICE on a shuffled
  // shelf – a 4.7% coaching discount at every rung above budget, and `economy-calibration`'s wealthy
  // cell flipping from a $2,970 idle-year burn to break-even. That reverses an owner principle to buy
  // ~1.5 and ~3 points on two corner frequencies, so the shuffle went back to him and did not ship.
  // `buildCoachRoster` draws the manner alone, and `coachId` holds on every cell above.
  //
  // ⚠ 6/1 READS AS A CONTRADICTION AND IS NOT: her `coachId` is `null` at week 156 and four keys moved
  // anyway. `coachPairs` holds a `high-1` row – she hired him and later let him go – and the weeks he
  // was paid for are weeks her rate moved. `coachId` is who is on the books now; `coachPairs` is who
  // has ever been.
  // ⭐⭐⭐ v80 (wave F1) – `form` PEELS FIRST, because it is the newest key and the peel order is
  // reverse order of arrival. It is the LAST key of `createWorld`'s literal (placed there for exactly
  // this), so dropping it leaves every other key's position untouched and the eighteen older shapes
  // come back exactly.
  //
  // ⚠⚠ AND THE PEEL CANNOT UNDO WHAT THE KEY DID, WHICH IS WHY EVERY RUNG BELOW MOVED WITH THIS
  // WAVE RATHER THAN HOLDING. v79's `loveEpisodes` note says the same thing from the other side: a
  // peel drops a KEY, it does not replay a career without the mechanic. Form reaches `composureEff`
  // at `MatchPlayer` build time on every frozen career, so every one of them played different
  // matches – the per-key diff (below) names 33 / 3 / 34 keys of 94 moving on the three cells, with
  // `rngMain` byte-identical on all three. ⚠⚠ SO `PRE_V80`'s THREE VALUES ARE **NOT** THE VERBATIM
  // v79 `FROZEN` CONSTANTS – the first rung in this file of which that is true, and its own block
  // says why at length. Every rung under it is re-stamped too, which is what a BEHAVIOUR change costs
  // this file and what a key append does not.
  // ⭐⭐⭐ v81 (ROUND 44) – `LifeBeatRecord.frame` PEELS FIRST, because it is the newest key, and it is
  // a NESTED peel like v77's `loveEpisodes` and v78's `assets[].entries`: the key is on a `lifeLog`
  // ROW rather than on the world. The spread-then-override idiom below is what makes a nested peel
  // exact – a key that already exists in the spread keeps its original insertion position when it is
  // re-assigned – and the `undefined` arm is not caution: a world with no `lifeLog` at all must keep
  // having none, because writing `[]` would be a byte this shape never held.
  //
  // ⚠⚠ AND THE PEEL CANNOT UNDO WHAT THE ROUND DID, WHICH IS WHY EVERY RUNG BELOW MOVED AGAIN. Round
  // 44 lands the 43-situation corpus, so the pool a career draws from grows from 8 situations to 51
  // and every `lifeLog` row's `detail` changes with it; `court-four`'s four rows merge into one and
  // `reachableSituations` answers a different question. Dropping the `frame` key gives back the v80
  // SHAPE, never the v80 CAREER. The per-key diff (in the dated block at the head of this file) names
  // what moved on each cell, and `rngMain` is byte-identical on all of them – a situation draw is a
  // purpose-scoped sub-stream and cannot touch MAIN.
  // ⭐⭐⭐ v82 (ROUND 42 #51) – `coachDeal` PEELS FIRST, because it is the newest key and the peel
  // order is reverse order of arrival. A plain world key and the LAST of `createWorld`'s literal, so
  // the one-line destructure below restores the v81 SHAPE exactly.
  //
  // ⚠⚠ AND IT RESTORES THE SHAPE, NEVER THE CAREER, WHICH IS WHY EVERY RUNG BELOW MOVED AGAIN. The
  // item changes what a hired coach COSTS: v81 re-derived his rate every week, so a career crossing
  // `coachAgeBand`'s first step re-drew him from a dearer row, and v82 bills the fee that was agreed.
  // ⚠ A FROZEN CAREER CROSSES THAT STEP ON ITS LAST WEEK AND THE BUILD PREDICTED OTHERWISE. `ageAtWeek`
  // returns whole years, she turns 17 at week 156 exactly, and this walk's final tick runs AT 156 –
  // which is also `3 x 52`, so the annual ask fires on the same week. The per-key diff in the dated
  // block at the head of this file names the seven keys on each coached cell and the two on the
  // self-coached one; `rngMain` is byte-identical everywhere, because the ask draws nothing.
  // ⭐⭐⭐ v83 (THE WEDDING, WAVE 7 – T1) – THE LATCH AND THE NAME PEEL FIRST, because they are the
  // newest keys and the peel order is reverse order of arrival. A NESTED peel like v77's, over the
  // SAME list: the two fields live on the ENTRIES of `world.loveEpisodes`, so the peel maps the list
  // and rests each row, and the spread-then-override keeps `loveEpisodes` in its original slot (the
  // v78 `entries` block below states the property).
  //
  // ⭐⭐ AND IT IS AN IDENTITY IN BEHAVIOUR, v81's OWN KIND, for a CALENDAR reason stated in the
  // wave brief's §0: a frozen career is 156 weeks from its own start, the girl in it never reaches
  // 23, so no wedding hazard, no `'engaged'` beat, no name and no cost can fire here – T1 ships two
  // seats, the migration, the birth nulls in `rollArrival`'s literal and NO WRITER for either value
  // (the hazard is T2, the latch write and the naming are T3). The peel removes two null fields and
  // never a value, and `PRE_V83` asserts the verbatim v82 constants come back.
  //
  // ⚠ MEASURED BEFORE THIS LINE WAS WRITTEN, as the protocol demands and never after the fact:
  // `tools/frozen-key-diff.ts` on ALL FIVE careers, control captured on this branch's head
  // `1b9eb2b1` with a clean tree (the wave's own change absent), headers checked on all ten
  // captures. Exactly ONE line moves on four of the five – `schemaVersion` (`a46e37632fa6` ->
  // `bbb965ab0c80`) – and TWO on `eliteGrinder`, whose `loveEpisodes` moves `8c5884c15bcd` ->
  // `444ca2a71789` as its one row (`p:137`, the v77 note's own witness cell) gains the two null
  // fields. 94 / 93 / 95 / 93 / 94 other keys byte-identical on the five careers: `rngMain`,
  ...
  // ⚠⚠ RE-AIMED FOR v85 (the pregnancy and the return, wave 8 – T1), NOT WEAKENED, AND IT PEELS
  // AHEAD OF ALL TWENTY-THREE BELOW because `pregnancy` and `children` are the newest keys and the
  // peel order is reverse order of arrival. They are the SECOND GROUP to arrive in one version since
  // the wave-5 six – peeled in ONE destructure because they arrived in one append, which is also why
  // the order INSIDE the destructure is cosmetic: object rest copies what it keeps in the source's
  // own insertion order, so `children` first and `pregnancy` first produce the identical shape. The
  // order that IS load-bearing is this GROUP's position, ahead of `prologueTrace`, and the order of
  // the two keys in `createWorld`'s literal, which is where the serialisation gets its shape.
  //
  // ⭐⭐ AND IT IS AN IDENTITY IN BEHAVIOUR, v84's OWN KIND AND FOR THE STRONGER OF THE TWO REASONS
  // THIS FILE DISTINGUISHES. Both keys are written by `createWorld` itself, so every career here
  // carries them – and it carries them `null` and `[]`, which on this rung is a property of THE TREE
  // rather than of these careers, exactly as v75's `spiritShock` was: T1 ships the seats, the
  // migration and NO WRITER AT ALL (the hazard is T2's, the pause T3's, the birth T4's), so nothing
  // on this tree could put a value there however long the walk ran. ⚠ The career-shaped reason holds
  // too and is the weaker one, worth naming so a later reader does not lean on it alone: 156 weeks
  // stops at age 16.6, the hazard's window opens at 24, and marriage is the door – so even once T2
  // lands, this peel stays a no-op on these five cells unless `FREEZE_WEEKS` grows past the latch.
  // The peel removes the version's keys and never a value, and `PRE_V85` asserts the verbatim v84
  // constants.
  //
  // ⚠ MEASURED BEFORE THIS LINE WAS WRITTEN, as the protocol demands and never after the fact:
  // `tools/frozen-key-diff.ts` on all three careers, control captured on this branch at `c322301c`
  // with a clean tree as the builder's FIRST command, headers checked on all six captures
  // (`# preset 5 policy 0`, `# preset 8 policy 0`, `# preset 0 policy 1`). Exactly THREE lines move
  // on each – `schemaVersion`, `pregnancy` APPEARING with hash `74234e98afe7` (the sha256 of
  // `JSON.stringify(null)`, the same string v75's `spiritShock` landed on) and `children` APPEARING
  // with `4f53cda18c2b` (the sha256 of `[]`) – with 94 / 94 / 95 other keys byte-identical, `rngMain`,
  // `results`, `events`, `nextEventId`, `bond`, `spirit`, `loveEpisodes` and the wallet among them.
  //
  // ⭐⭐⭐ AMENDED 20.09 – v85 GREW A THIRD KEY AFTER GATE 2 (`comeback`, the week she came back and
  // the freeze she came back with; the architect's ruling – task T2½ piece 1), AND THE PEEL TAKES ALL
  // THREE IN THE SAME ONE DESTRUCTURE. The group's POSITION is what is load-bearing – ahead of
  // `prologueTrace`, because these are still the newest keys – and the order INSIDE the destructure
  // is cosmetic for the reason the v85 block above already argues: object rest copies what it keeps
  // in the source's own insertion order. `createWorld` appends `comeback` THIRD, after `children`,
  // which is where the live serialisation gets its shape.
  //
  // ⚠ THE SAME PROTOCOL AGAIN AND NOT A PROMISE THAT IT WOULD HOLD: `tools/frozen-key-diff.ts` was
  // re-run on all three careers as the builder's FIRST command on the untouched tree at `5b025575`,
  // headers read back against the invocation on all six captures. **Exactly ONE line moves on every
  // cell** – of 97 / 97 / 98 keys before the append and 98 / 98 / 99 after, with ALL 97 / 97 / 98 of
  // the old ones byte-identical: `comeback` APPEARING at `74234e98afe7`, which is the sha256 of
  // `JSON.stringify(null)` and therefore the same twelve characters `pregnancy` landed on at T1 –
  // two null keys hash alike, the small cross-check that the tool and the peel agree about what a
  // null key is. `rngMain`, `results`, `events`, `nextEventId`, `bond`, `spirit`, `loveEpisodes`,
  // `spiritShock`, `pregnancy`, `children` and the wallet are all among the unmoved.
  //
  // ⭐ ONE LINE AND NOT TWO, WHICH IS THE ARITHMETIC AND NOT A WEAKER RESULT: `schemaVersion` DID NOT
  // MOVE, because it could not – the version number is already 85 and this key GREW v85 rather than
  // taking 86. Every schema rung in this file's history moved two lines for this reason; this one is
  // the first that moves one, and that is the receipt for «v85 grew» rather than «v86 arrived».
  //
  // ⚠ AND THE IDENTITY WAS RE-VERIFIED RATHER THAN ASSUMED, which is the whole of why `PRE_V85`
  // below is untouched: `careerHashAtSchema(·, ·, 84)` was RUN on this tree with the third key in it
  // and reproduced the three shipped v84 `FROZEN` values character for character. `PRE_V85` holds the
  // same three constants it has held since T1, and the eleven LIVE cells re-stamp instead.
  // ⚠⚠ RE-AIMED FOR v86 (the dynasty, wave 10), NOT WEAKENED, AND IT PEELS AHEAD OF EVERYTHING BELOW
  // because `dynasty` is the newest key and the peel order is reverse order of arrival. It is written
  // by `createWorld` itself, so every career here carries it – and carries it `null`, because
  // `walkFrozenCareer` opens a career through `openCareer`, which hands `createWorld` no fifth
  // argument and never will: a frozen career is a generation-zero career by construction.
  //
  // ⚠⚠ SO THE LIVE REGISTERS MOVED AND EVERY ROLLBACK RUNG HELD, which is the shape of a pure key
  // append and is stated here as a measurement rather than as a hope: `PRE_V86` below holds the
  // three v85 constants CHARACTER FOR CHARACTER – they are the values `FROZEN` held before this
  ...
  const { coachDeal: _coachDeal, ...preDeal } = preLatch
  const preFrame =
    preDeal.lifeLog === undefined
      ? preDeal
      : {
          ...preDeal,
          lifeLog: preDeal.lifeLog.map((row) => {
            const { frame: _frame, ...preRow } = row
            return preRow
          }),
        }
  const { form: _form, ...preForm } = preFrame
  const { coachPairs: _coachPairs, sparringTravels: _sparringTravels, ...prePairs } = preForm
  const {
    composureBonus: _composureBonus,
    sparringHired: _sparringHired,
    sparringRung: _sparringRung,
    ...preSparring
  } = prePairs
  const preEntries = {
    ...preSparring,
    // ⚠ THE SPREAD-THEN-OVERRIDE KEEPS `assets` WHERE IT WAS, which is the property v77's nested peel
    // relies on one block down and the only reason a nested peel can be exact at all: a key that
    // already exists in the spread keeps its original insertion position when it is re-assigned.
    assets: preSparring.assets.map((owned) => {
      const { entries: _entries, ...preEntry } = owned
      return preEntry
    }),
  }
  const { spotlightHabituation: _habituation, ...preHabituation } = preEntries
  const preSpotlight = {
    ...preHabituation,
    loveEpisodes: preHabituation.loveEpisodes.map((episode) => {
      const {
        publicWeek: _publicWeek,
        publicWrong: _publicWrong,
        airedMetWeek: _airedMet,
        airedEndedWeek: _airedEnded,
        ...preAired
      } = episode
      return preAired
    }),
  ...
  const {
    psychologistHired: _psyHired,
    psychologistRung: _psyRung,
    psychologistFocus: _psyFocus,
