...
| `applauseShort` | `applause-short-1.mp3`, `applause-short-2.mp3` (random variant)    | a game ends; a non-tiebreak set ends; a non-final match ends (×1); at ×2 also game-end/set-end (incl. tiebreak sets) and match-end (incl. the final); at ×4 only match-end |
| `applauseFinal` | `applause-final.mp3`                                               | the tournament final's match ends; the champion finale screen (if the final wasn't watched) – ×1 only (×2/×4 use `applauseShort` at match-end instead) |
