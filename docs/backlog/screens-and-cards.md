...
| 7 | **No HISTORY of played matches – ⚙ NARROWED 26.08 after he pushed back on my first wording.** I filed this as «the calendar shows no past matches at all»; he asked whether the college league's matches had appeared now that it runs the tour's own flow. Verified: they HAVE – the freeze toast says so («the matches are in the news feed, and they can be watched»), and the college year card carries a `MatchReplay` per national-team rubber. The real gap is narrower and older: the Season tab draws a bracket ONLY for the latest resolved tournament week, the Calendar is forward-looking with no played match on it at all, and the feed's window is what holds everything else. So a match is watchable until it scrolls away, and then it is gone for good – a career has no readable record of its own tennis | [round-26.md](../rounds/round-26.md) #6/#7 | a design call: where history lives (a season archive, the Trophies tab, or the bracket card growing a past) | M | Later |
