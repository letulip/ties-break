...
- **Выбор стартового возраста** [Ф6, детский пролог; НЕ добавлять мёртвое поле сейчас]: онбординг
  ...
  формируют потолки.
- **Арт-стадии по возрасту** (канон владельца): jun <12, young 12-15, teen 16-22, adult 23-28, milf
  29+. ВАЖНО: текущий старт 14 = young (не jun); нынешние jun-аватарки = заглушка. TODO (отдельная
  задача): провязать портрет = стадия(ageYears)+эмоция(состояние); онбординг-кадр «первый раз на
  корт» оставить jun. Эмоции: norm/happy/sad/serious/injury/tired + вехи
  bride/funeral/graduated/retired/farewell.
