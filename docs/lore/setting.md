...
| Tournament finale / pre-match splash | 512px `-fs8` painting | up to **180px** wide, 12px radius |
...
(42 of them). These are the real reference for style-matching – not the shipped WebP. The build
pipeline (`npm run art`, `scripts/optimize-art.mjs`) resizes to ≤512px longest side and encodes WebP,
stepping quality 82 → 79 → 76 → 75 until the file is under 120 KB. The older PNG-era set was a flat
q82. A new painting is added by dropping a JPEG into that folder and re-running the pipeline.

**Full paintings** – `public/images/fem-euro-brunnet/fem-euro-brunnet-{band}-{emotion}.webp`,
512 × 512, 36–79 KB. **37 clean-named files ship:**
...
The 24 that the code can actually request (four bands × six emotions) all exist. The other 13 are
later-life content nothing reaches yet – about 1.6 MB of it, deliberately kept out of the service
worker's precache for that reason.

**Finale paintings** – the same frames with a legacy `-fs8` suffix. 31 of these ship, but the code
only ever asks for **three emotions**: `happy` (champion), `serious` (runner-up) and `sad` (any
earlier exit), for whichever band she is in. Everything else in the `-fs8` set is inert. The suffix is
a historical filename artefact from the pngquant era and means nothing about quality – the two
encodes of the same frame land within a few hundred bytes of each other. `adult-happy-fs8` is
generated by an explicit alias rule in the pipeline, because the old set never had one and a champion
aged 23+ used to 404 on her own title splash.

Two naming inconsistencies to know about before adding files: the `-fs8` set spells teen and milf
injury as `injured`, while every clean-named file spells it `injury`; and `angry` exists as a JPEG
master and an `-fs8` WebP for all five bands but has **no clean-named WebP and no code path at all** –
it is not one of the six emotions the game can resolve.
