...
| `seeded-careers.spec.ts` | seven of the thirteen fixtures boot into the state their manifest describes – the other six (`soft`, `breakup`, `belated`, `engaged`, `expecting`, `parting`) are booted and asserted by the specs of the waves that added them, because each exists for a state this walk has no way to name | 1, 2 |
...
| `elite-gate.spec.ts` | the top coaching rung is earned: a career below the gate's bar meets four locked Elite rows that say how far short she is, cannot be pressed, and read the same number to a screen reader – and a career past the bar meets the same screen with nothing locked and, since round 42 #42, no money refusal on it either – a rung costing more than her whole week is still a rung she may press, with the week's cost in its own name and the figures on the Team-budget tile | 5, 1 |
| `psychologist.spec.ts` | a psychologist goes on the payroll: the professional gate has opened on the card, the keyed confirm quotes the default rung's price back, the year's work appears only once there is somebody to work it, one press puts his salary on the ledger to the cent, and the card still holds him and his year a week later | 1, 5 |
| `spotlight.spec.ts` | the cameras were on her last week and the feed says so in plain words – the only thing a player is ever told about the spotlight, written inside a tick nobody watches, on a horizon one week behind the week it prints in – and against it a psychologist is hired and given the fifth year of work, «The public life», which the hired line then carries for the rest of the season | 1, 5 |
...
| `dynasty.spec.ts` | a finished career takes the door on its epilogue, the childhood is walked again under her mother's LOCKED surname with the origins card not asked, and the career that comes out was born on the line's own seed – `${mother}:dynasty:1`, a string that exists nowhere in the UI and can only have crossed the hand-off in memory. ⚠ `world.dynasty` itself reaches no screen and the e2e project may not import the engine (`tsconfig.e2e.json`'s own rule), so the record's persistence is pinned one layer down and this spec proves everything the record CAUSES that a player can see | 1, 5 |
| `weight.spec.ts` | the one switch in the game is turned ON in settings and the press round-trips through the worker; a RELOAD brings the answer back out of IndexedDB, which is where a v87 schema field either persists or does not and is the sharpest thing only this layer can say; turned off again a driven year (▶▶ 52) raises no weight beat. ⚠ It cannot wait for a bereavement to ARRIVE – 0.08% a week is four careers in a hundred over a year – so the arrival is walked in the engine and the bench, and this file owns the route, the persistence and the absence | 1, 2, 5 |
| `parting.spec.ts` | the week a MARRIAGE ends: a married career comes back out of IndexedDB, one press raises the divorce's own card rather than the break-up's, he answers it through the real UI, and the week leaves BOTH rows on the feed – the news row and the album's line. The only week in the game that writes to both channels, which is a claim about a PAGE and therefore invisible to every engine test, all of which read arrays | 1, 2, 5 |
...
| `AlbumScreen.vue` | no tab – Home's recent-memory card | `album` (the door, the fetch, the pan, `Back`, and the re-fetch on a second visit) | `component/album-mobile`, `component/album-wide`, `component/album-home-door`, unit (`albumBook`) | **e2e** – the only view in the app assembled ON DEMAND rather than handed down in the weekly snapshot (spec §8b), so it is the only one where something has to GO AND GET its data: every mounted test above is handed a book by a fixture, and none of them can say the `{type:'album'}` arm answers. The three layouts, the pager, the three widths and the copy rule stay component-owned |
...
| The album – the career as a book, assembled when the section opens | [the-album-2026-09.md](the-album-2026-09.md) | yes – `album` | **e2e** – the mechanic IS the on-demand assembly (§8b: «Сборка альбома – по требованию, не в недельном снимке»), and an on-demand view is precisely what a mounted test cannot have: it is handed a book, so it can never say the query answers. Everything that is a PICTURE stays component-owned – the three layouts, the pager's arithmetic, the three widths, the base prefix on a painting and the rule that no layer but the corpus writes a sentence. The assembly's own rules (chapters where something was lived AND earned, the §4 ladder, the caps) are unit-owned in `tests/albumBook.test.ts` |
