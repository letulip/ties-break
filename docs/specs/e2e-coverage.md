...
| `MoneyScreen.vue` | no tab – Home's budget card | `week-advance`, `tournament`, `sponsor-inbox`, `responsive`, `parity` (presence at four widths) | `component/round20-ui` | **e2e** – second screen off the same snapshot: the only proof the ledger is fed by the tick, and the only proof a signature becomes a contract |
