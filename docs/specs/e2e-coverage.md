...
| `elite-gate.spec.ts` | the top coaching rung is earned: a career below the gate's bar meets four locked Elite rows that say how far short she is, cannot be pressed, and read the same number to a screen reader – and a career past the bar meets the same screen with nothing locked and the money's own separate refusal still on it | 5, 1 |
