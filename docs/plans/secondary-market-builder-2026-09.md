...
- `OwnedAsset.listedWeek?: number`; `SAVE_SCHEMA_VERSION` 89 → 90; append-only migration (the
...
- MoneyScreen: «Sell» on a thing opens the §2g dialog – the quote's weeks and corridor, the academy
  one-lot warning line when the row is academy, buttons List / Sell now / keep. A listed row shows
  the badge and «Withdraw». All strings DRAFT.
...
- Mounted tests (`tests/component/`): the dialog's dismiss control inside 375×667 **with the
  ...
  carries the warning line and a car's does not.
