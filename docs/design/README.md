...
Пакет содержит **20 экранов**:
- **Онбординг и создание персонажа:** N. Welcome, O. Identity, P. Country, Q. Family & Coaching,
  R. Play Style, S. Summary.
- **Игровой цикл:** A. Home, B. Season Planner, C. Kid Profile, D. Weekly Story,
  E. Tournament (Preview), F. Match Day (Live), I. Live Match (Court), G. Family Budget,
  H. Calendar (Week View), T. Coach Market.
- **Турнирная сетка и итоги:** J. Championship Draw (R16), K. Championship Draw (Final),
  L. Champion, M. Runner-up.

Порядок реализации: онбординг (N–S) → Home (A) → календарь и планирование (H, B) →
матч (E, F/I) → итоги (J–M, D) → экономика (G, T).

**Имена в макетах.** Игровой цикл (A–M) собран на персонаже **Bianca Tran**, онбординг и
Coach Market (N–T) — на **Naomi Steiner** (имя из брифа онбординга). Это одна и та же роль
«дочь игрока»; в проде имя приходит из онбординга. Не считать расхождением.
...
- **Конфетти на экране L** — 18 абсолютно позиционированных прямоугольников. В проде это частицы
  (появление сверху, падение, вращение), а не статичные div-ы; позиции из макета — только для композиции.
- **Радар-диаграммы стилей (экран R)** — реальные SVG-полигоны с пятью осями; копировать точно,
  значения осей брать из игровых данных стиля.
- **Кубок и медаль (L, M)** — арт-слоты, не SVG (см. Assets).
...
### J. Championship Draw (R16) / K. Championship Draw (Final)
**Purpose:** турнирная сетка по раундам; отслеживание своей дочери и чужих матчей.

**Layout:** header (back / тайтл по центру / share) → строка чипов покрытия и даты →
таб-строка раундов → сетка матчей (`flex:1`) → карточка-нотификация → CTA. Таб-бара нет.

- **Header:** back-arrow 22px, тайтл 17px/800 −0.02em по центру, иконка share 20px.
  Ниже по центру: чип покрытия (pill border `rgba(255,255,255,.1)`, точка 10px `#e2822f`, «clay»
  12.5px/500 `#c3ccd2`) и дата «Apr 12–18, 2032» 12.5px/500 `#8e9ba4`, gap 10px.
- **Раунды:** `grid repeat(4,1fr); gap 8px`, pill `padding:9px 0`, radius 999px.
  Активный — `#cfe152` / текст `#111a10` 13px/800; неактивный — `#17212b` / `#c3ccd2` 13px/600.
- **Матч (J):** карточка `flex:1` (высота делится поровну), radius 12px, `#141d26`,
  border `rgba(255,255,255,.06)`, padding 7px 10px, `margin-right:22px` (место под коннекторы).
  Строка игрока: сид 10.5px/600 `#6d7a83` (ширина 11px, пусто у несеяных) → имя → сеты.
  Победитель: имя 13px/700 `#cfe152` + галочка 12px, счёт 12.5px/700 `#f2f6f8`.
  Проигравший: имя 13px/500 `#8e9ba4`, счёт 12.5px/500 `#6d7a83`. Сет-ячейка — `width:11px;text-align:center`
  (несыгранный сет — «—»). **Матч своей дочери** — фон `rgba(207,225,82,.06)`, border `1px #cfe152`;
  подсветка помечает участие, а не победу (в макете B. Tran проигрывает).
- **Коннекторы:** абсолютный столбец 12px у правого края списка, линии `1.4px rgba(255,255,255,.18)` —
  две горизонтальные culy-полки + вертикаль на пару матчей.
- **Финал (K):** вместо списка — по центру иконка кубка 34px `#d9a441` + подпись «THE FINAL»
  11px/800/0.14em, единственная карточка матча (padding 13px 14px, radius 14px, лаймовая рамка,
  имена 15px/800, счёт 15px, делитель 1px между игроками) и строка «Semifinals: …» 12.5px/500
  между двумя hairline 26×1px.
- **Нотификация:** карточка radius 15px `#141d26`: иконка (разбитое сердце 27px `#ef6a3a` — вылет своей;
  плитка 38×38 radius 11px `rgba(217,164,65,.14)` + кубок 21px `#d9a441` — финал готов),
  тайтл 14px/700, текст 12.5px/500 line-height 1.4.
- **CTA:** pill `#cfe152`, «Next round →» / «Watch the final →», 14.5px/800 `#111a10`, стрелка 17px.

### L. Champion / M. Runner-up
**Purpose:** экран итога турнира для своей дочери. Одна карточка-«постер» + путь по раундам.

**Layout:** header (как в J) → карточка результата (`flex:1`, `overflow:hidden`, margin 8/14/22,
radius 22px) → внутри карточки всё содержимое, включая CTA. Таб-бара нет.

- **Фон карточки:** `radial-gradient(120% 68% at 50% 0%, #16283b 0%, #111a24 52%, #101821 100%)`
  (у M холоднее: `#152532` вместо `#16283b`), border `rgba(255,255,255,.06)`.
- **Конфетти (только L):** 15 прямоугольников 6–10×6–14px + 3 круга 4–5px, цвета `#cfe152`,
  `#dbe45a`, `#d9a441`, `#8a7040`, `#4a5560`, повороты −52°…+56°, opacity 0.5–0.9,
  в абсолютном слое `inset:0` с `overflow:hidden` и `pointer-events:none`.
  У M вместо конфетти — 6 тусклых точек 4–5px `#8fb2d6` / `#c3ccd2`, opacity 0.22–0.4.
- **Порядок содержимого:** арт-слот (L: 138×82 кубок; M: 104×92 медаль) → статус
  («CHAMPION» / «RUNNER-UP» 12px/700, letter-spacing **0.24em**, uppercase; L — `#c3ccd2`, M — `#a8b3ba`) →
  имя 33px/800 −0.025em (L — `#cfe152`, M — `#f2f6f8`) → фото `flex:1; min-height:0; max-height:198px`
  (M: 192px), radius 15px, border `rgba(207,225,82,.4)` / `rgba(255,255,255,.14)`,
  `box-shadow:0 12px 30px rgba(0,0,0,.45)` → строка соперника 14.5px/500 («def. **I. Aigner** in the Final» —
  имя лаймом; у M «lost to **I. Aigner**» — имя `#e9eef1`) → счёт по сетам 19px/800, gap 16px →
  чипы покрытия и турнира → очки «+75 pts» / «+48 pts» 19px/800 лаймом (у M тоже лайм — очки всегда прирост) →
  **строка раундов** (`margin-top:auto`) → CTA «Continue».
- **Строка раундов:** `grid repeat(4,1fr)`, padding 11px 2px, radius 14px, фон `rgba(255,255,255,.03)`,
  border `rgba(255,255,255,.06)`, колонки разделены hairline. Ячейка: раунд 11.5px/700 `#c3ccd2`
  (проигранный — 11.5px/800 `#ef4b3a`), «def. **Имя**» / «lost to **Имя**» 10.5px, счёт 10.5px/500,
  иконка-кружок 17px (чек `#cfe152` / крест `#ef4b3a`).
- **Важно про высоту:** карточка = `flex:1` с `overflow:hidden`, поэтому фото — единственный
  растягивающийся элемент (`flex:1` + `max-height`); при добавлении контента уменьшать его, а не CTA.
  Иначе кнопка «Continue» уезжает под обрез.

### N–S. Онбординг и создание персонажа
**Purpose:** 6 шагов от эмоционального интро до старта карьеры.

**Общий каркас:** индикатор шагов (flex:none) → заголовок → контент → футер `Back` / `Next`.
Таб-бара нет. Боковой gutter онбординга — **22px** (не 14px, как в игровом цикле).

- **Индикатор шагов:** 6 кружков, между ними линии 14×1.5px `rgba(255,255,255,.12)`.
  Активный — 22px, `#cfe152`, число 11px/800 `#111a10`; неактивный — 18px,
  border `1px rgba(255,255,255,.14)`, число 10px/600 `#6d7a83`. Padding `28px 20px 0`.
- **Заголовок шага:** 20px/800 −0.01em **uppercase** `#f2f6f8` + подзаголовок 13.5px/500 `#8e9ba4`
  (padding `24px 22px 0`, отступ подзаголовка 6px). Исключение — N (см. ниже).
- **Футер:** `Back` — pill `#17212b` border `rgba(255,255,255,.1)`, 14px/700 `#c3ccd2`, padding 14px 28px;
  `Next` — pill `#cfe152`, 14.5px/800 `#111a10`, padding 14px 26px + **иконка мяча** 17px
  (круг r8.5 + две дуги, stroke 1.6). Между футером и контентом — распорка `flex:1;min-height:0`.
- **N. Welcome:** заголовок 26px/800 uppercase в две строки, вторая («Together.») лаймом;
  три абзаца 14.5px/500 line-height 1.5 `#a8b3ba`, gap 14px; арт `flex:1` radius 16px
  с нижним скримом до `rgba(15,23,32,.45)`; CTA «Begin» (pill лайм, 15px/800 + мяч 19px);
  под ним «Skip for now» 13px/500 `#8e9ba4` с подчёркиванием `rgba(142,155,164,.4)`, offset 3px.
- **O. Identity:** поля gap 16px. Лейбл 12.5px/600 `#a8b3ba` + отступ 8px. Инпут: padding 14px 15px,
  radius 12px, `#17212b`, border `rgba(255,255,255,.09)`, значение 15px/600 `#f2f6f8`.
  Кнопка «кости» 46×46 radius 12px с лаймовой иконкой (две разные раскладки точек — намеренно).
  Пол — `grid 1fr 1fr; gap 12px`, выбранный: фон `rgba(207,225,82,.07)`, border `1px #cfe152`,
  иконка и текст лаймом 14px/700; невыбранный — `#17212b`, `#8e9ba4`/600.
  Селект месяца: иконка календаря + значение + шеврон 18px. Инфо-плашка: `#141d26`, иконка (i) 17px,
  текст 12.5px/500 line-height 1.42.
- **P. Country:** поиск (padding 14px 15px, radius 12px, placeholder 14.5px/500 `#6d7a83`, лупа 18px) →
  eyebrow «POPULAR» лаймом → `grid repeat(3,1fr); gap 11px`: плитка padding 13px 6px, radius 13px,
  `#17212b`, флаг **34×23 radius 3px** + подпись 10.5px/600 `#c3ccd2` → eyebrow «ALL COUNTRIES» `#8e9ba4` →
  строка «Browse all countries ›» (radius 13px, `#141d26`, 14px/600).
- **Q. Family & Coaching:** eyebrow «FAMILY BACKGROUND» → 3 карточки выбора (padding 14px 15px,
  radius 14px, иконка 25px, тайтл 14.5px/700, строка бюджета 12.5px/500 с **суммой 700 лаймом**,
  подпись 12px/500); выбранная — `rgba(207,225,82,.07)` + border `#cfe152` + чек-кружок 19px.
  Затем eyebrow «COACHING» и `grid 1fr 1fr`: карточка padding 16px 12px, иконка 28px,
  тайтл 13.5px/700 по центру, описание 12px/500 line-height 1.4.
- **R. Play Style:** 4 карточки `flex:1` (высота делится поровну), padding 12px 14px, radius 14px:
  арт-слот 42×58 radius 8px → название 14.5px/700 + описание 12px/500 + 2 чипа
  (padding 4px 10px, radius 999px, 10.5px/600; у выбранной — `rgba(207,225,82,.14)` / лайм) →
  **радар 56×56**: внешний пятиугольник `rgba(255,255,255,.13)`, внутренний 50% `rgba(255,255,255,.08)`,
  полигон данных fill 0.3–0.32 + stroke 1.4px. Цвета: baseliner `#ef4b3a`, counterpuncher `#cfe152`,
  big serve `#5b9bd5`, all-court `#9b7fd4`.
- **S. Summary:** арт `flex:1; max-height:306px` radius 16px border `rgba(207,225,82,.28)` →
  список из 6 строк (padding 11px 14px, hairline между, иконка 17px `#8e9ba4`, лейбл 12.5px/500,
  значение 13.5px/700 справа; стиль игры — лаймом; у страны — флаг 22×15 после значения) →
  рукописная строка Caveat 19px `rgba(207,225,82,.85)` с иконкой сердца →
  CTA «Start career» (pill лайм 15px/800 + мяч 19px).

### T. Coach Market
**Purpose:** выбор тренера на любом тире и под любой стиль игры, с честной привязкой к бюджету.

**Концепция (три решения, отличающие экран от «списка карточек»):**
1. **Тир — это раздел, а не фильтр.** Тренеры сгруппированы по тирам сверху вниз; у группы —
   заголовок с цветной точкой, числом тренеров и вилкой цен. Градация читается прокруткой,
   бейдж тира на карточке не нужен (минус одна сущность на каждой карточке).
2. **Стиль — один дропдаун вместо пяти чипов.** По умолчанию подставлен стиль ребёнка;
   снять/сменить — один тап. Рядом «Sort: Best fit». Итого один ряд чипов (тиры) + один ряд контролов
   вместо двух рядов по пять чипов.
3. **Главный сигнал на карточке — посадка (fit), а не список тегов.** Одна пилюля посадки +
   специализации текстом через «·». Отвечает на вопрос «подойдёт ли он моей дочери», а не «что он умеет».

**Layout:** header → карточка бюджета → чипы тиров → ряд контролов → список (`flex:1`) → tab bar.
Gutter контента 14px, header 16px.

- **Header:** кружок back 34px `rgba(255,255,255,.07)`, тайтл «Coach Market» 19px/800 −0.02em,
  подстрока «Naomi Steiner · **Counterpuncher** · 32 coaches» 12.5px/500 (стиль — 700 лаймом), лупа 21px.
- **Карточка бюджета:** radius 14px `#141d26`; «Coaching budget» 11.5px/600 `#a8b3ba` и
  «**$130** /week free» (13px/800 лаймом + 11px/600 `#8e9ba4`); бар 6px radius 3px,
  трек `rgba(255,255,255,.09)`, лаймовая заливка 46%; легенда из двух пунктов
  (квадрат 7px + текст 10.5px/500): «$110 committed», «$240 weekly cap».
- **Чипы тиров:** pill с **счётчиком** (лейбл 12px + число 10px). Активный — `#cfe152`,
  текст `#111a10`/800, счётчик `rgba(17,26,16,.55)`; неактивный — `#17212b`,
  border `rgba(255,255,255,.08)`, лейбл `#c3ccd2`/600, счётчик `#6d7a83`.
  Значения: All 32 · Budget 11 · Middle 9 · High 8 · Elite 4.
- **Контролы:** два pill-дропдауна (лейбл 11.5px/500 `#8e9ba4` + значение 11.5px/700 + шеврон 13px).
  Активный фильтр стиля — фон `rgba(207,225,82,.07)`, border `rgba(207,225,82,.5)`, значение лаймом.
- **Заголовок тира:** точка 7px цвета тира + «BUDGET TIER» 10px/800/0.1em в цвете тира +
  «11 coaches» 10.5px/500 `#6d7a83`; справа вилка «$40–70 /wk» 10.5px/600 `#8e9ba4`.
  Цвета тиров: Budget `#8fb2d6`, Middle `#cfe152`, High `#e2822f`, Elite `#9b7fd4`.
- **Карточка тренера:** `flex:1; min-height:0; max-height:82px`, padding `9px 12px 9px 9px`,
  radius 14px, `#141d26`. Фото 46×46 radius 12px → имя 14.5px/700 → строка: пилюля посадки
  (padding 3px 8px, 10px/700) + специализации 11px/500 `#8e9ba4` с `text-overflow:ellipsis` →
  правая колонка: цена 14.5px/800 + «/wk» 10.5px/600 и действие.
  **Пилюля посадки:** Great fit — `rgba(207,225,82,.14)` / лайм; Good fit — `rgba(255,255,255,.07)` / `#c3ccd2`;
  Off-style — прозрачная, border `rgba(255,255,255,.12)`, `#8e9ba4`.
  **Три состояния действия:** доступен — «Hire ›» 12px/800 лаймом; текущий тренер — карточка
  `rgba(207,225,82,.07)` + border `#cfe152`, чек 14px + «Current» 11.5px/700 лаймом;
  не по бюджету — карточка `rgba(20,29,38,.5)` + **border dashed** `rgba(255,255,255,.09)`,
  фото opacity 0.45, имя и цена `#6d7a83`…`#8e9ba4`, замок 12px + «$20 over»
  (показывать именно нехватку в деньгах, не «дорого»).
- **Свёртка старших тиров:** строка внизу списка (radius 14px, фон `rgba(255,255,255,.02)`):
  две точки цветов тиров + «High & Elite — **12 coaches** from $180/wk» + шеврон.
- **Tab bar:** системный, но 5-й пункт — **Market** (иконка корзины), активен.
  На этом экране «More» заменён на «Market».
...
- Конец матча своей дочери → **J** (сетка обновилась) либо **L/M**, если это был финал.
- J/K: чипы раундов переключают раунд без ухода с экрана; «Next round →» проигрывает следующий круг
  и обновляет сетку; тап по матчу → его отчёт. K: «Watch the final →» → **F** или **I**.
- L/M: «Continue» → возврат на Home с обновлённым рейтингом и пополненным альбомом воспоминаний.
- Онбординг: N «Begin» → O; «Skip for now» → генерирует персонажа случайно и уходит сразу на A. Home.
  O–R: «Next» активен только при заполненном шаге; кости в O — рандом имени; S «Start career» → A. Home.
  «Back» сохраняет уже сделанный выбор.
- T: чип тира скроллит к своей группе (а не фильтрует список в ноль); дропдаун стиля меняет расчёт
  посадки и порядок; «Hire ›» → лист подтверждения с недельной стоимостью и влиянием на бюджет;
  карточка не по бюджету → тап открывает объяснение, чего не хватает; свёртка снизу → раскрывает
  High и Elite.
...
- L: конфетти — частицы падают сверху 1.6s ease-out со случайным вращением, кубок появляется
  `scale` 0.8→1 + fade 320ms, имя — fade-up 12px с задержкой 120ms, строка раундов — по колонке
  каскадом (60ms шаг). M: без конфетти, медаль появляется тем же `scale`, но без каскада.
- J: продвижение победителя в следующий раунд — подсветка строки лаймом 240ms, затем коннектор
  прорисовывается к следующей карточке.
- Онбординг: смена шага — slide-left/right 24px + fade 220ms; индикатор шага перекатывается
  (активный кружок 18→22px, 180ms). Радар в R при выборе стиля — `stroke-dashoffset` + fill fade 300ms.
- T: смена фильтра стиля — пересортировка списка с FLIP-анимацией 260ms; «Hire» — карточка
  становится текущей, лаймовая рамка проявляется 200ms, бар бюджета доезжает до нового значения 400ms.
...
- `draw` (J/K): rounds[{ id: 'R16'|'QF'|'SF'|'F', matches[{ players: [{name, seed, sets[], isWinner,
  isPlayerKid}], status }] }], `activeRound`, `playerStatus` ('in' | 'out' | 'champion' | 'runnerUp').
- `tournamentResult` (L/M): outcome ('champion'|'runnerUp'), opponent, finalSets[],
  points, surface, path[] { round, opponent, sets, won }.
- `onboarding` (N–S): step (1–6), kid { firstName, lastName, gender, birthMonth },
  country, background ('wealthy'|'middle'|'working'), coaching ('self'|'hired'),
  playStyle ('baseliner'|'counterpuncher'|'bigServe'|'allCourt'), `startingBudget` (производное от background).
- `coachMarket` (T): coaches[{ id, name, tier, pricePerWeek, styles[], specialties[], fit }],
  `tierCounts`, `filters` { style, sort }, `weeklyCap`, `committed`, `currentCoachId`.
  `fit` — производное от совпадения стиля тренера со стилем ребёнка ('great'|'good'|'off');
  доступность — производное от `weeklyCap − committed` (не хранить отдельно).
...
20. **DrawMatchRow** — матч сетки: сиды, имена, галочка победителя, сеты, флаг «своя дочь».
21. **RoundTabs** — переключатель раундов R16/QF/SF/F.
22. **ResultPoster** — «постер» итога (L/M): статус, имя, фото, счёт, чипы, очки, строка раундов, CTA;
    props `outcome`. Внутри — правило «растягивается фото, не кнопка».
23. **RoundPath** — строка из 4 колонок «раунд → соперник → счёт → чек/крест».
24. **StepIndicator** — индикатор шагов онбординга (n из 6).
25. **ChoiceCard** — карточка выбора с состоянием selected (используется в Q, R и в T как строка тренера).
26. **StyleRadar** — радар стиля игры на 5 осей, props `values`, `color`.
27. **FlagBadge** — флаг 34×23 / 24×16 / 22×15 (в проде — реальный набор иконок).
28. **TierSectionHeader** — заголовок группы тира: точка, название, счётчик, вилка цен.
29. **FitPill** — пилюля посадки (great / good / off).
30. **BudgetMeter** — карточка недельного бюджета: сумма, бар, легенда.
31. **PriceAction** — правая колонка карточки тренера: цена + одно из трёх состояний
    (hire / current / over budget).
...
| `trophy-art` | L | кубок с лаврами (прозрачный фон) | 138×82 |
| `champion-photo` | L | момент победы | 198×198 |
| `silver-medal-art` | M | серебряная медаль (прозрачный фон) | 104×92 |
| `runnerup-photo` | M | момент финала | 192×192 |
| `onboarding-hero` | N | родитель и ребёнок на корте, сумерки | 346×~300 |
| `style-baseliner`, `style-counterpuncher`, `style-bigserve`, `style-allcourt` | R | поза игрока по стилю | 42×58 |
| `summary-portrait` | S | героиня у входа в клуб | 346×306 |
| `coach-velez`, `coach-park`, `coach-reid`, `coach-marin`, `coach-raman` | T | портреты тренеров | 46×46 |
...
`I-live-match-court.png`, `J-draw-r16.png`, `K-draw-final.png`, `L-champion.png`, `M-runner-up.png`,
`N-onboarding-welcome.png`, `O-onboarding-identity.png`, `P-onboarding-country.png`,
`Q-onboarding-family.png`, `R-onboarding-playstyle.png`, `S-onboarding-summary.png`,
`T-coach-market.png`.
...
## Files
- `design/Home Screen.dc.html` — все 20 экранов, открывается прямо в браузере (двойной клик).
