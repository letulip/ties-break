...
| `02-onboarding-tour.png` | `OnboardingTour.vue` – coach-mark tour, step 1 | Auto-opens over Home for a device that has never ANSWERED the tour, while the career is still at week 0; also on demand from More. | The card itself is representative, though the shot predates 16.08 (five steps, now eleven – see `docs/specs/onboarding-tour.md`). The Home behind it is necessarily a brand-new W1 career (empty budget, no news, nothing entered): the week-0 bound is what keeps the marks off a mature career, and the `firstEverCareer` one-shot the old caption named is gone – it lost the tour outright for any player whose first session was interrupted. |
