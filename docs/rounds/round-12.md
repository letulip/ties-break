...
> **STATUS, re-audited 09.08 (backlog #88): all 13 open boxes were stale and all 13 are now ticked.**
> Waves A and B shipped and were never ticked back; the two design items were ruled and built later,
> under other names – R12-2/13/17 as the **ladder floor** (08.08) and R12-17b as the **living
> field** + the **junior conveyor**. Every tick below names where it landed. Nothing in this round
> is open.
...
- [x] **R12-15 dead Play button after an injury (worst item).** Injury before a tournament, "no
  ...
  reason (the R10-16 doctrine).
      → **SHIPPED** – the arrival gate. `git grep R12-15`: `src/engine/world/medical.ts` (§"THE
      ARRIVAL GATE", and the dead click reproduced on a named seed), the walkover week banked on
      `src/engine/world.ts` and halted in `advanceWeeks`, `src/composables/weekAhead.ts`'s header,
      `src/App.vue`, `src/shared/protocol.ts`. The button stopped guessing.
- [x] **R12-3 playable while injured AND outgrown.** A 1-week injury; next week a regional she has
  ...
  band and the injury on the play week — this is the R10-5 "one rule, many surfaces" family again.
      → **SHIPPED** with R12-15, same gate. `git grep R12-3`: `src/engine/world/medical.ts`
      (`arrivalStatus`, which reports `outgrown` alongside the verdict so every surface can SAY so),
      `src/engine/world.ts`, `src/engine/world/snapshot.ts`, `src/shared/protocol.ts`.
- [x] **R12-16 anger fires on every loss once the streak crossed.** Design intent was a mood, not a
  ...
  already pinned — keep it.)
      → **SHIPPED** – `src/shared/avatarEmotion.ts` ("ANGER IS A MOMENT, NOT A MASK"; the `===`
      comparison against the threshold, never `>=`).
- [x] **R12-S1 (screenshot) season 2 wrap-up says "from #1".** Both careers show a season-start rank
  ...
  first counting week instead of week 1.
      → **SHIPPED** – `world.seasonStartRank`, schema **v17**. `src/engine/world.ts` (the field, the
      season-0 capture and the season-boundary bank at step 0a00), `src/engine/migrations.ts` (the
      v17 migration), `src/engine/world/milestones.ts` (the "#89 ↓88 FROM #1" note names the
      screenshots).
- [x] **R12-S2 (screenshot) "Best result: best Champion".** The row label already says "Best result";
  the value duplicates "best". Value should read "Champion" / "Runner-up" / "Semifinalist".
      → **SHIPPED** – `src/engine/world/milestones.ts` (the label already says "Best result", so the
      value dropped its own "best").
- [x] **R12-6 two Nationals on adjacent weeks (twice in one season, incl. the last two weeks).** The
  ...
  is the known cost of any calendar change and must be done deliberately, in its own commit.
      → **SHIPPED** as a per-tier `minGap`. `src/engine/season/types.ts` (the field),
      `src/engine/season/calendar.ts` (the national and j300 rules, and the slot claimer that
      enforces the distance). The re-base landed with it –
      `tests/fatigue-bench-policy-104w.test.ts` records the C3 anchor coming BACK "because R12-6
      spread the adjacent Nationals".
- [x] **R12-4/11 injured ON a family vacation, twice.** `rollInjury` runs every healthy week with the
  ...
  the physio/recovery buffs). Keep it nonzero — holidays do sprain ankles — but rare.
      → **SHIPPED** – `ECONOMY.availability.injuryVacationFactor` in `src/engine/economy.ts`,
      applied as a post-draw multiply in `src/engine/world/injury.ts` ("a booked family week is the
      opposite pole of the load axis"). Nonzero, as asked.
- [x] **R12-5b practice/sparring offered during a 5-week layoff.** The planner sheet still renders
  ...
  the tournament lock. (The "back W70" absolute-week half of this item was fixed in `fix/world-trio`.)
      → **SHIPPED** – `src/engine/world/medical.ts` (`availabilityStatus` ranks the layoff above the
      doctor, and the planner's body gate reads the same predicate) surfaced by
      `src/components/PlanWeekSheet.vue`: the layoff's hard no is rendered BEFORE a click instead of
      thrown after one.

## 🟡 Design decisions — need the owner, prepared in wave A as knobs

- [x] **R12-2/13/17 the outgrown-band trap.** At 112–118 pts only regional is open (local closes at
  ...
  to reality; (c) is the smallest patch for the trap he actually hit.
      → **RULED AND SHIPPED 08.08, under a different name: the LADDER FLOOR.** The owner chose
      option (b) in the round-14 triage – «не делать нижний порог вообще, пусть играет, просто по
      приоритету более актуальный турнир показывать» – and it was built and measured as
      `docs/specs/ladder-floor-2026-08.md` (status: SHIPPED). The lower bound stopped refusing and
      became a sorting key; on the owner's own save the weeks with nothing enterable went 27 of 46 →
      6, with the acceptance cuts untouched. ⚠ It carries a **measured cost** to the climb, which
      the owner accepted deliberately – read that spec's §3 before quoting this as a free win.
- [x] **R12-17b why the long regional losing streak at 8k?** Partly the coach gap (self-coached =
  ...
  reads as a slump rather than a trap).
      → **THE NAMED FIX SHIPPED, and it never carried this tag** – which is exactly why this box sat
      open. `docs/specs/living-field.md` is partly landed: **rivals develop** – schema **v20** gave
      the cohort an age and a ceiling (`src/engine/season/cohort.ts`, `ageCohort`;
      `src/engine/migrations.ts` v19→v20: "until now they grew about 1.5 a year for ever, so no
      career could catch the ladder"); **the field turns over** – the junior conveyor
      (`docs/specs/junior-conveyor.md`, `src/engine/season/conveyor.ts`, `renewCohort` wired in
      `world.ts`); **and the professional contour exists** – the FIELD ring for the W track (01.08,
      `feat/living-field`, that spec's §8) and `docs/specs/population-1600-2026-08.md`. The trap
      half is R12-2/13/17 above, closed by the ladder floor. STILL AHEAD by the spec's own
      admission: §§2-3, the full three-ring build.
      ⚠ `docs/specs/round15-triage.md`'s audit row says R12-17b has "no trace anywhere". That is
      true of the TAG and false of the work – the distinction this whole re-audit exists to make.

## 🟢 Presentation — wave B

- [x] **R12-1/14 exam weeks must SAY exams.** Tournament cards on exam weeks lose "+ Plan week" with
  ...
  school — but the card must say why SHE cannot go.)
      → **SHIPPED** – `src/components/screens/SeasonScreen.vue` (the exam-week flag, the "Exams" row
      label, and the card's own "Exams this week" pill in place of a silently missing button) plus
      `src/style.css`. "Exams" is the owner's own word, and the code says so.
- [x] **R12-8b injury chip on week plaques.** A small red "injury" marker on calendar/planner week
  ...
  the off-season card just silently offers nothing).
      → **SHIPPED** – the red `injury` chip on the calendar's week rows and the kept-booking rows
      (`src/components/screens/SeasonScreen.vue`, reading the layoff window off the snapshot), and
      the refusal reason rendered before the click in `src/components/PlanWeekSheet.vue`.
- [x] **R12-12 score on its own line.** On the Season screen's this-week tournament plaque the score
  goes to a SECOND line (asked before — round-11's one-line fix was for the practice row; this is
  the tournament plaque).
      → **SHIPPED** – `src/components/screens/SeasonScreen.vue` ("TWO lines – the sentence on top,
      the scoreline on its own line beneath"), the comment naming it the owner's SECOND ask.
