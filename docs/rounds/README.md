...
| [round-26.md](round-26.md) | 26 – four years of college, played through (2026-08-24) | 27 | 23 shipped, **4 open**. ⚠ Its own #1 came back as round 29 #6: the span pill was re-gated on his rule but never introduced |
| [round-27.md](round-27.md) | 27 – the college mini-round (2026-08-27) | 18 | 12 shipped, **6 open** – four of them parked on him, #8 is not |
| [round-28.md](round-28.md) | 28 – a full career replayed, 17 items (2026-08-28) | 18 | 11 shipped, **7 open**; round 29 re-files three of them |
| [round-29.md](round-29.md) | 29 – the pro years, and the economy wave that grew out of them (2026-08-28/30) | 66 | The largest round in the folder: the advertising portfolio, the manager's commission, the fund, merch, the earning academy, full offline. 63 shipped, **3 open**. ⚠ Its #17 produced [AUDIT-2026-08-28.md](AUDIT-2026-08-28.md), which sat unmerged until round 33 |
| [round-30.md](round-30.md) | 30 – an intermediate round, 17 items + the CI split (2026-08-30) | 28 | all shipped. CI cut from 18m32s to 8–9 min across three jobs; injury severity, recurrence and the fitted age curve |
| [round-31.md](round-31.md) | 31 – the draw, the decline, and what a card may promise (2026-08-31) | 13 | all shipped or answered. ⚠ **Numbering note**: only item 1 uses a `- [x]` box; items 2–13 are `##` sections, so a box count under-reads this ledger. The draw stopped re-rolling (20 of 24 → 0), the tier ladder was un-inverted, the age curve learned the college fork, and `physicalShare` finally reached the screen |
| [round-32.md](round-32.md) | 32 – the off-season shoot, the results view, and the brand's economy (2026-08-31) | 5 | all shipped. Fame entered the brand multiple; the brand's memory moved into its INCOME after he stopped the first pass at «123x annual earnings» |
| [round-33.md](round-33.md) | 33 – the tidy-up wave, and the screen that was never two screens (2026-09-01) | 4 | 3 shipped, **1 open** (the Stats header tiles, waiting on his choice of label). ⭐ Answered the question four rounds failed to: there is no tournament screen – Home's plate opens `ThisWeekScreen` in another mode |
| [round-34.md](round-34.md) | 34 – a full career on the round-33 build (2026-09-02) | 21 + his save read (#22) | 19 shipped, 3 answered; **3b** (whether the «1 день вместе» ask should carry more of a request) is `[?]` on him. The domestic points reset came back as his own round-23 season-to-date ruling with an unpriced half – the tier gates re-close every January, and latching a cleared floor is left as his balance call; bundles H and I re-normalised the ceiling read and the growth yardstick, their consequences reported, not adjusted. ⚠ **This row and the six below it did not exist until 09.09** – the fifth lag, counted under this table |
| [round-35.md](round-35.md) | 35 – the prologue, played end to end for the first time since it merged (2026-09-03) | 15 | 13 shipped, 2 answered – **3 and 6**, the key-moments clock questions, which went into NO bundle and were caught only by the final tally; the foot of the file records that as a process failure with its fix (count the boxes against the bundles BEFORE gating). The prologue weekend got the game's own tournament beats without mounting `TournamentFlow` – the blockers are named one by one, not waved at – and **#15** overturned the round-34 CI verdict: vitest's 20 s budget is PER TEST, and one `it` was migrating all 71 golden fixtures |
| [round-35-shop.md](round-35-shop.md) | 35, the shop – his design pass off his mockup, frames V–AA (2026-09-03) | 10 | 8 shipped, 2 answered; same `round/35` branch as the prologue half, different surface, no collision. His own instruction «есть нюансы, делаем не точно так» governs the whole file: three places the mockup and his words disagree are recorded on the items they belong to, and his words won each one. The shop became two levels (the shelf home, then the category pages), the six cards are TALL at the paintings' own 332/512, and the card names are the switcher's own labels so the two surfaces cannot drift |
| [round-36.md](round-36.md) | 36 – the interfaces on tablet and desktop (2026-09-04) | 5 phases | **phase 1 shipped**: the Playwright parity harness (all ten screens at 375/768/900/1280, the list derived from `src/components/screens/`) and the container on his breakpoint ladder. ⭐⭐ Its finding corrects the spec's own premise: `--app-max-width: 880px` is the FRAME – the column a player reads is `main.app-content`, capped at **520**, so the app on a 1280 screen was a 520px column and widening only the frame would have moved nothing. **Phase 2 shipped**: the tablet band – Home's hero off the square onto `--hero-aspect` (768/400, and the tournament preview reads the same token), Season two cards to a row with the third a swipe away, the coach market two to a row, and three boxes that had never grown (the recap painting stuck at 390px from 520 up, and two floating CTAs still at 520). 0 of 6,963 boxes moved at 375/520/576; 709 moved at 768. `docs/specs/responsive-decisions-2026-09.md` opened with twelve rows, four of them his to answer. **Phase 3 shipped**: the desktop shell – the bottom bar STANDS UP into a full-height sticky rail (one navigation, never a second one, because a duplicate would fail his own criterion by name), `#app` becomes a two-column grid past 1024, Home takes frame AC's two columns by dissolving its notecard wrapper with `display: contents`, and both heroes share a cap as well as a ratio. 0 of 6,963 boxes moved at 375/520/576; 141 moved at 768/900 and every one is Home's season ladder, which he ruled should open itself there; 2,100 boxes and 1,023,303px at 1280. ⭐⭐ The harness changed with it: it opens every disclosure at every width first, so its claim is «the same things are REACHABLE», which is his own wording. Nine more decision rows, twenty-one in all. Phases 4–5 open. ⚠ Rounds 34 and 35 had no row in this table until 09.09 |
| [round-37.md](round-37.md) | 37 – his six from the stand on the review wave, then ten more across the same two days (2026-09-05/06) | 6 + 10 | 15 shipped, 1 answered – **5** stayed an answer: the coach pictures had already widened 62 → 66px that morning (`D89`), and how much of the mask's remaining 6px is affordable is a measurement, stated as one. **#7** found the live-match screen's grid collapsing its `auto` rows to zero on short desktops – his «часть интерфейса вообще не видна» exactly, fixed in one declaration – and **#16** is his own ruling written down as law: «любые отрицательные отступы - это антипаттерн» |
| [round-38.md](round-38.md) | 38, the balance wave – his three from the merged build, and everything they pulled in (2026-09-06/07) | 3 + 33 follow-ons | Nothing open: 21 shipped, 15 answered. His #1 became `docs/specs/travel-class-2026-09.md` by his own word (written, deliberately NOT built); #2's answer is that the brand has never read the table – it reads fame, on a two-year half-life. Waves A (the snapshot cache) and B (`tabSeen.ts` out of `App.vue`) shipped inside the round, and Wave C closed whole: C4's one closed form SHIPPED, C1 and C3 closed together by **#17** (his pick off the measured table – `plateauRate` x3 and the fit span past the whole coach ladder, Borg's coach as the argument), C2 closed as answered (its 93.3% premise had been measured on the protected arm only) |
| [round-39.md](round-39.md) | 39 – Ines's career on the merged round-38 build (2026-09-08) | 15 (24 boxes) | 15 shipped, 9 answered, nothing open. **14a/14b** left the round open by design – the trigger could not be guessed, invariant 5 – and were ticked 09.09 against round 40's wave-c, which measured the plateau card into her DOUBT rather than a forecast. Item 17's apparel bond is the round's spine (waves G/G2/G3): the guaranteed renewal notice, no reprice, no rank floor, and the money moved off the rival's letter onto the confirm on his own objection. ⚠ **This file was found physically damaged on 09.09**: two paste collisions had doubled ~290 lines and eaten two figures (both restored from the intact duplicate) and destroyed item 12's heading (reconstructed in place, marked) |
| [round-40.md](round-40.md) | 40 – the prologue's choices and what the promo film exposed (2026-09-08) | 7 + r39's 14a/14b | 5 of his 7 shipped, 2 answered by measurement – **5** (the recorded 1.87 span had simply rotted: the real figure is 2.44, and wave F proved the Local Open answer cannot reach the arrival at all) and **6** (variant B measured on `r40/wave-g` and nothing moved – no dial, no card, not one byte of `src/`). The prologue's selectors became the ball-dot radio off his own visual, the handover reads realisation instead of arrival, the two seeds became one girl carried through – and r39's retirement voice landed as one derivation with three voices (`lastWinterIn`) |
| [round-41.md](round-41.md) | 41 – the long playtest: money, rankings and the screens' fit (2026-09-12) | 27 + 1 | 25 shipped (his same-day #28 build ring included, plus his five follow-up rulings: A1, «в этот же раунд», P1 with the tiered corridor, the elite shelf, the Slam-debut fame floor with the brand math settled on the Alice reference), 2 answered (**3** – the capstone deliberately stayed at 4 while the lifetime went to 3, his parked question re-surfaced with the 16.7%-vs-8% pricing tension; **21** – wealth pricing is canon twice over, the GEAR table's 5–8× spread flagged as the real optical culprit), **22** parked for v76 with its design written (purchase marks need a persisted per-buy record; the degraded version was refused for printing money the family never paid). The first round run through the PROJECT's own `/fix-round` (registered from `~/.claude/skills` this day), six recon + six executor bundles cut by collision surface, and the gate earned its keep twice: eleven unit reds (ten of them the owner's own A1 moving prize arithmetic; one the 30.07 back-control law catching a text button) and five e2e reds (a new `overflow: hidden` clipping the Begin pill on short desktops). Alongside it, PR #135's unit-heavy was diagnosed and cut green in two passes (goldenSaves ×3 + economy calibration out; then coach-travel-edge 66.7 s projected and college-birthday 59.2 split) – the 2036 win-rate «drop» measured into a promotion story (78% of matches at WTA-250+, her per-band rate UP everywhere) |
...
until 13.08, rounds **19, 20, 21 and 22** had none until 19.08, rounds **23, 24 and 25** had none until
24.08, and rounds **34, 35 (both halves), 37, 38, 39 and 40** had none until 09.09 – five times now,
which is its own lesson about a navigation table nobody is required to update.
