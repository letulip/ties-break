...
npm run test:sim   # sim project (~65s) — NOTE: exits 1 even when green, known bug
...
- `npm run test:sim` exits 1 with all tests passing (review finding, P6 item d). Do not read that exit code as failure.
- The `▶▶ 52 (dev)` button in More ships in production and bypasses knock/tournament guards (P6 item c).
